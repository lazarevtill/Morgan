...
The owner writes most queries in Russian, and FTS5's ``unicode61`` tokenizer does no stemming,
so keyword search is weakest exactly there. A keyword-only recall while embeddings are down
(not built) would rely on it alone. ``recall.done`` records which language each query was in,
so such a recall could be told apart by it.
"""
