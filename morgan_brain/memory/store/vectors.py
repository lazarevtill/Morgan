...
timed per-project queries only: 42 ms partitioned against 59 ms. Measured for migration step 6
on a copy of the same 3,010-memory, 4,096-wide archive, the partition costs what that did not
measure: an all-projects query -- the scope the relevance floor was fitted on -- takes 75 ms
against 60 ms; each project gets vector chunks of its own, 1,024 vectors wide whatever it
holds, so twenty projects of one memory each fill 337.5 MB against 17.0 MB; and sqlite-vec
0.1.9 refuses an ``UPDATE`` of a partition key, the statement migration step 5 renames a
