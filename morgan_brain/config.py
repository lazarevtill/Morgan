...
def default_data_dir() -> str:
    ...
    the checkout was only ever seen from the checkout. A brain that is supposed to
...
def user_config_file() -> Path:
    ...
    the repository keeps its local dev overrides). Same reason as ``default_data_dir``:
