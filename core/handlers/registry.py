...
class BaseHandler:
    ...
    async def handle(
        self, command: Command, context: ConversationContext
    ) -> Dict[str, Any]:
...
class HandlerRegistry:
    ...
    def __init__(self, core_service):
        ...
        self.logger.info(
            f"Handler registry initialized with {len(self.handlers)} handlers"
        )
    ...
    async def process_command(
        ...
        try:
            ...
            handler_name = (
                command.action.split(".")[0] if "." in command.action else "general"
            )
            ...
            self.logger.info(
                f"Command processed: {command.action} -> {result['success']}"
            )
            return result
