...
class ClientConfig(BaseModel):
    ...
    retry_delay_seconds: int = Field(
        default=2, ge=0, description="Delay between retries"
    )
        ...
        try:
            async with self._session.request(
                ...
                if response.status >= 400:
                    ...
                    if (
                        response.status >= 500
                        and retry_count < self.config.retry_attempts
                    ):
                        self.logger.warning(
    ...
    async def connect(self) -> None:
        ...
        try:
            self._ws = await websockets.connect(
                ws_url, extra_headers=headers, ping_interval=20, ping_timeout=10
            )
            self._set_status(ConnectionStatus.CONNECTED)
            ...
            raise NetworkConnectionError(
                ...
                host=self.config.server_url,
                context={"ws_url": ws_url, "error_type": type(e).__name__},
    ...
    async def receive_messages(self) -> AsyncIterator[Dict[str, Any]]:
        ...
        try:
            ...
            raise NetworkConnectionError(
                ...
                recovery_suggestions=["Try reconnecting", "Check network stability"],
            )
