...
async def test_client_api_only_communication(config, message, metadata):
    ...
    for forbidden in forbidden_imports:
        assert not any(
            forbidden in item.lower() for item in module_dict
    ...
    with patch("aiohttp.ClientSession") as mock_session_class:
        ...
        assert (
            mock_session_class.called
        ), "Client must use HTTP session for communication"
    ...
    assert ws_url.startswith("ws://") or ws_url.startswith(
        "wss://"
    ), "WebSocket client must use WebSocket protocol"

    # Verify no direct database or model access
    assert not hasattr(
        http_client, "vector_db"
    ), "Client should not have direct vector DB access"
    assert not hasattr(
        http_client, "embedding_model"
    ), "Client should not have direct embedding access"
    assert not hasattr(
        http_client, "llm_client"
    ), "Client should not have direct LLM access"
    assert not hasattr(
        ws_client, "vector_db"
    ), "WebSocket client should not have direct vector DB access"
    assert not hasattr(
        ws_client, "embedding_model"
    ), "WebSocket client should not have direct embedding access"
    assert not hasattr(
        ws_client, "llm_client"
    ), "WebSocket client should not have direct LLM access"
        ...
        if mock_session.request.called:
            ...
            if call_args:
                url_used = (
                    ...
                    else call_args[1].get("url", "")
                )
                # The URL should start with the configured server URL
                assert config.server_url in url_used or url_used.startswith(
                    config.server_url
...
async def test_client_error_handling(config, error_type):
    ...
    with patch("aiohttp.ClientSession") as mock_session_class:
        ...
        if error_type == "connection":
            ...
            mock_response.json = AsyncMock(
                return_value={"error": "NOT_FOUND", "message": "Resource not found"}
            )
            mock_session.request = AsyncMock(return_value=mock_response)
        ...
        if error_type in ["connection", "timeout", "server_error", "not_found"]:
            assert (
                error_raised
            ), f"Client should raise an exception for {error_type} errors"
...
@pytest.mark.asyncio
@settings(max_examples=100, deadline=None)
@given(config=client_config_strategy(), message=message_strategy())
async def test_client_cleanup_isolation(config, message):
    ...
    with patch("aiohttp.ClientSession") as mock_session_class:
        ...
        assert (
            http_client.status == ConnectionStatus.DISCONNECTED
        ), "Client status should be DISCONNECTED after close"

        # Verify no lingering references
        assert (
            http_client._session is None or http_client._session.closed
    ...
    with patch("websockets.connect") as mock_ws_connect:
        ...
        assert (
            ws_client.status == ConnectionStatus.DISCONNECTED
        ), "WebSocket status should be DISCONNECTED after close"

        # Verify no lingering references
        assert (
            ws_client._ws is None or ws_client._ws.closed
