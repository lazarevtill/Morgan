...
def markdown_content_strategy(draw):
    ...
    # Bold text
    if draw(st.booleans()):
        bold_text = draw(st.text(min_size=1, max_size=50, alphabet=st.characters(
            whitelist_categories=("Lu", "Ll", "Nd", "Zs")
        )).filter(lambda x: x.strip()))
    ...
    # Italic text
    if draw(st.booleans()):
        italic_text = draw(st.text(min_size=1, max_size=50, alphabet=st.characters(
            whitelist_categories=("Lu", "Ll", "Nd", "Zs")
        )).filter(lambda x: x.strip()))
    ...
    # Code blocks
    if draw(st.booleans()):
        code = draw(st.text(min_size=1, max_size=100, alphabet=st.characters(
    ...
    if draw(st.booleans()) or not elements:
        plain_text = draw(st.text(min_size=1, max_size=200, alphabet=st.characters(
            whitelist_categories=("Lu", "Ll", "Nd", "Zs", "Po")
        )).filter(lambda x: x.strip()))
...
def test_markdown_rendering(content, title):
    ...
    with patch('morgan_cli.ui.console', test_console):
        ...
        try:
            ...
            assert len(rendered_output) > 0, "Rendering should produce output even on error"
    ...
    if words:
        ...
        found_words = sum(1 for word in words[:5] if word.lower() in rendered_output.lower())
    ...
    if title:
        ...
        assert title in rendered_output or title.lower() in rendered_output.lower(), \
    ...
    if "**" in content:
        ...
        assert len(rendered_output) > len(content) * 0.5, \
...
def test_error_message_clarity(error_message, error_type, details, suggestions):
    ...
    assert error_message in rendered_output or error_message.lower() in rendered_output.lower(), \
        "Error message should be visible in output"
    
    # Verify error type is shown if provided
    if error_type:
        assert error_type in rendered_output or error_type.lower() in rendered_output.lower(), \
    ...
    if details:
        for key, value in details.items():
            ...
            key_present = key in rendered_output or key.lower() in rendered_output.lower()
            value_present = str(value) in rendered_output or str(value).lower() in rendered_output.lower()
    ...
    if suggestions:
        for suggestion in suggestions:
            assert suggestion in rendered_output or suggestion.lower() in rendered_output.lower(), \
    ...
    stack_trace_count = sum(1 for indicator in stack_trace_indicators if indicator in rendered_output)
    
    # Allow a few matches (like "Error:" which is expected), but not full stack traces
    assert stack_trace_count < 3, \
    ...
    assert "❌" in rendered_output or "Error" in rendered_output or "error" in rendered_output, \
    ...
    assert len(rendered_output) >= min_expected_length * 0.8, \
...
@given(
    ...
    error_msg=st.text(min_size=1, max_size=100, alphabet=st.characters(
        whitelist_categories=("Lu", "Ll", "Nd", "Zs", "Po")
    )).filter(lambda x: x.strip())
)
def test_connection_error_display(server_url, error_msg):
    ...
    assert "server" in rendered_output.lower() or "connection" in rendered_output.lower(), \
        "Connection error should mention server or connection"
    
    # Verify suggestions are present
    assert "check" in rendered_output.lower() or "verify" in rendered_output.lower(), \
...
@given(
    field_errors=st.dictionaries(
        ...
        values=st.text(min_size=1, max_size=100, alphabet=st.characters(
            whitelist_categories=("Lu", "Ll", "Nd", "Zs", "Po")
        )).filter(lambda x: x.strip()),
        min_size=1,
        max_size=5
...
def test_validation_error_display(field_errors):
    ...
    for field, error in field_errors.items():
        ...
        field_visible = field in rendered_output or field.lower() in rendered_output.lower()
        error_visible = error in rendered_output or error.lower() in rendered_output.lower()
        assert field_visible or error_visible, \
            f"Validation error should show field error: {field}={error}"
    
    # Verify it's labeled as validation error
    assert "validation" in rendered_output.lower() or "invalid" in rendered_output.lower(), \
...
def test_timeout_error_display(timeout_seconds):
    ...
    assert str(timeout_seconds) in rendered_output, \
        "Timeout error should show timeout duration"
    
    # Verify it's labeled as timeout
    assert "timeout" in rendered_output.lower() or "timed out" in rendered_output.lower(), \
        "Should be clearly labeled as timeout error"
    
    # Verify suggestions are present
    assert "try" in rendered_output.lower() or "check" in rendered_output.lower(), \
...
def test_server_error_display(status_code, message, details):
    ...
    assert str(status_code) in rendered_output, \
        "Server error should show status code"
    
    # Verify message is shown
    assert message in rendered_output or message.lower() in rendered_output.lower(), \
        "Server error should show error message"
    
    # Verify details are shown if provided
    if details:
        for key, value in details.items():
            key_visible = key in rendered_output or key.lower() in rendered_output.lower()
            value_visible = str(value) in rendered_output or str(value).lower() in rendered_output.lower()
            assert key_visible or value_visible, \
                f"Server error should show details: {key}={value}"
    
    # Verify appropriate suggestions based on status code
    if status_code >= 500:
        assert "server" in rendered_output.lower(), \
            "5xx errors should mention server-side issue"
    elif status_code == 404:
        assert "not found" in rendered_output.lower() or "404" in rendered_output, \
            "404 errors should mention not found"
    elif status_code == 400:
        assert "input" in rendered_output.lower() or "request" in rendered_output.lower(), \
...
def test_error_with_empty_details():
    ...
    assert "Test error" in rendered_output, "Should render error even with empty details"
