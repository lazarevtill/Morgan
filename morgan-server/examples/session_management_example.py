    ...
    try:
        ...
        async def process_request(session, request_num):
            async with manager.track_request(session.session_id) as tracked:
                print(f"  Processing request {request_num} for session {tracked.metadata['index']}")
                await asyncio.sleep(0.1)
        ...
        for session in sessions:
            retrieved = await manager.get_session(session.session_id)
            print(f"  Session {retrieved.metadata['index']}: "
...
async def example_session_cleanup():
    ...
    try:
        # Create multiple sessions
        session1 = await manager.create_session(user_id="user-1", metadata={"name": "Session 1"})
        session2 = await manager.create_session(user_id="user-2", metadata={"name": "Session 2"})
        session3 = await manager.create_session(user_id="user-3", metadata={"name": "Session 3"})
