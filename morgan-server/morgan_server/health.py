...
class HealthCheckSystem:
    ...
    async def check_component_health(
        self, name: str, checker: Any, timeout_seconds: float = 2.0
    ) -> HealthCheckResult:
        """
        ...
        try:
            ...
            if hasattr(checker, "health_check"):
                ...
                result = await asyncio.wait_for(
                    checker.health_check(), timeout=timeout_seconds
                )
                ...
                if hasattr(checker, "ping"):
                    await asyncio.wait_for(checker.ping(), timeout=timeout_seconds)
                else:
                    is_connected = await asyncio.wait_for(
                        checker.is_connected(), timeout=timeout_seconds
                    )
                    if not is_connected:
                        return HealthCheckResult(
                            name=name, status="down", error="Component not connected"
                        )

                latency_ms = (time.time() - start_time) * 1000
                return HealthCheckResult(name=name, status="up", latency_ms=latency_ms)
            ...
            latency_ms = (time.time() - start_time) * 1000
            return HealthCheckResult(
                name=name, status="down", latency_ms=latency_ms, error=str(e)
            )
            ...
            for result in check_results:
                if isinstance(result, HealthCheckResult):
                    ...
                    results["unknown"] = HealthCheckResult(
                        name="unknown", status="down", error=str(result)
                    )
