...
class ServerConfig(BaseSettings):
    ...
    llm_api_key: Optional[str] = Field(
        default=None,
        description="API key for LLM service (optional for self-hosted)"
    )
    llm_model: str = Field(
        default="gemma3",
    ...
    llm_endpoint: str = Field(
        ...
        description="Self-hosted LLM endpoint URL"
    )
    ...
    embedding_device: str = Field(
        default="cpu",
        description="Device for local embedding model (cpu, cuda, mps)"
    )
    ...
    def validate_embedding_device(cls, v: str) -> str:
        ...
        if v not in valid_devices:
            raise ValueError(f"Embedding device must be one of: {', '.join(valid_devices)}")
        return v
