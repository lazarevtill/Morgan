...
class MilestoneCelebration(BaseModel):
    ...
    achieved_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Achievement timestamp")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional milestone data")
...
class ChatResponse(BaseModel):
    ...
    empathy_level: Optional[float] = Field(None, ge=0.0, le=1.0, description="Empathy level")
    personalization_elements: List[str] = Field(default_factory=list, description="Applied personalization")
    milestone_celebration: Optional[MilestoneCelebration] = Field(None, description="Milestone info")
...
class MemoryStats(BaseModel):
    ...
    oldest_conversation: Optional[datetime] = Field(None, description="Oldest conversation timestamp")
    newest_conversation: Optional[datetime] = Field(None, description="Newest conversation timestamp")
...
class LearnRequest(BaseModel):
    ...
    doc_type: str = Field("auto", description="Document type (auto, pdf, markdown, text, html)")
...
class ProfileResponse(BaseModel):
    ...
    relationship_age_days: int = Field(..., ge=0, description="Days since first interaction")
...
class PreferenceUpdate(BaseModel):
    ...
    response_length: Optional[str] = Field(None, description="Response length preference")
    ...
    def validate_communication_style(cls, v):
        ...
        if v is not None:
            valid_styles = ["casual", "professional", "friendly", "technical", "playful"]
...
class ErrorResponse(BaseModel):
    ...
    def validate_error_code(cls, v):
        ...
        if not v.isupper() or " " in v:
            raise ValueError("Error code must be uppercase with underscores (e.g., INVALID_REQUEST)")
        return v
