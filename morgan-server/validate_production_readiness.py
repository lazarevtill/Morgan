...
class ProductionReadinessValidator:
    ...
    async def validate_qdrant(self) -> bool:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(f"{self.qdrant_url}/collections", timeout=5) as resp:
                    if resp.status == 200:
    ...
    async def validate_ollama(self) -> bool:
        ...
        try:
            ...
            print_warning(f"To start Ollama: Visit https://ollama.ai/download and install, then run 'ollama serve'")
            self.add_result(ValidationResult(
                "Ollama Connectivity",
                False,
    ...
    async def validate_server_health(self) -> bool:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(f"{self.server_url}/api/status", timeout=5) as resp:
                    if resp.status == 200:
                        data = await resp.json()
    ...
    async def run_all_validations(self) -> bool:
        ...
        print_info("  2. Ollama running: ollama serve (after installing from https://ollama.ai)")
        ...
        if passed == total:
            ...
            print(f"{Colors.RED}Please address the issues above before deploying to production.{Colors.RESET}\n")
