    ...
    **Validates: Requirements 10.1**
    ...
    def test_property_request_logging_completeness(
        ...
        with LogCapture("morgan.middleware.logging") as log_capture:
            ...
            assert request_log.get("method") == method, f"Expected method {method}, got {request_log.get('method')}"
            assert request_log.get("path") == path, f"Expected path {path}, got {request_log.get('path')}"
            
            if user_id:
                assert request_log.get("user_id") == user_id, "Request log missing or incorrect user_id"
            
            if conversation_id:
                assert request_log.get("conversation_id") == conversation_id, "Request log missing or incorrect conversation_id"
            ...
            assert "response_time_ms" in response_log, "Response log missing response_time_ms"
            assert "status_code" in response_log, "Response log missing status_code"
            assert response_log.get("method") == method, f"Expected method {method} in response log"
            assert response_log.get("path") == path, f"Expected path {path} in response log"
            
            # Verify response time is a positive number
            assert isinstance(response_log["response_time_ms"], (int, float)), "response_time_ms should be numeric"
            assert response_log["response_time_ms"] >= 0, "response_time_ms should be non-negative"
            
            # Verify status code is valid
            assert isinstance(response_log["status_code"], int), "status_code should be integer"
            assert 100 <= response_log["status_code"] < 600, "status_code should be valid HTTP status"
            
            # Verify request IDs match
            assert request_log["request_id"] == response_log["request_id"], "Request IDs should match"
    ...
    def test_property_response_status_logged(self, status_code):
        ...
        with LogCapture("morgan.middleware.logging") as log_capture:
            ...
            assert "status_code" in response_log, "Response log missing status_code"
            assert response_log["status_code"] == status_code, f"Expected status {status_code}, got {response_log['status_code']}"
    ...
    **Validates: Requirements 10.2**
    ...
    def test_property_error_logging_detail(
        ...
        with LogCapture("morgan.middleware.error") as log_capture:
            ...
            assert len(logs) >= 1, f"Expected at least 1 error log entry, got {len(logs)}"
            ...
            assert error_log["level"] == "ERROR", f"Expected ERROR level, got {error_log['level']}"
            
            assert "method" in error_log, "Error log missing method"
            assert error_log["method"] == "GET", f"Expected GET method, got {error_log['method']}"
            
            assert "path" in error_log, "Error log missing path"
            assert error_log["path"] == "/error", f"Expected /error path, got {error_log['path']}"
            
            assert "error_code" in error_log, "Error log missing error_code"
            assert "error_type" in error_log, "Error log missing error_type"
            assert error_log["error_type"] == "ValueError", f"Expected ValueError, got {error_log['error_type']}"
            
            assert "error_message" in error_log, "Error log missing error_message"
            assert error_message in error_log["error_message"], "Error message not in log"
            
            assert "status_code" in error_log, "Error log missing status_code"
            assert error_log["status_code"] == 400, f"Expected 400 status, got {error_log['status_code']}"
            ...
            if user_id:
                assert error_log.get("user_id") == user_id, "Error log missing or incorrect user_id"
            
            if conversation_id:
                assert error_log.get("conversation_id") == conversation_id, "Error log missing or incorrect conversation_id"
    ...
    def test_property_different_error_types_logged(self, error_type):
        ...
        with LogCapture("morgan.middleware.error") as log_capture:
            ...
            # Verify error type
            assert error_log.get("error_type") == exception_class.__name__, \
                f"Expected {exception_class.__name__}, got {error_log.get('error_type')}"
            
            # Verify error code
            assert error_log.get("error_code") == expected_code, \
                f"Expected {expected_code}, got {error_log.get('error_code')}"
            
            # Verify status code
            assert error_log.get("status_code") == expected_status, \
    ...
    def test_property_error_context_includes_path(self, path):
        ...
        with LogCapture("morgan.middleware.error") as log_capture:
            ...
            assert "path" in error_log, "Error log missing path"
            assert error_log["path"] == path, f"Expected path {path}, got {error_log['path']}"
