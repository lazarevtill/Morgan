...
async def test_concurrent_request_handling(num_sessions, requests_per_session):
    ...
    **Validates: Requirements 6.1, 6.2**
    ...
    try:
        ...
        assert (
            len(set(session_ids)) == num_sessions
            ...
            for req_idx in range(requests_per_session):
                ...
                async def process_request(sess, req_id):
                    async with manager.track_request(
                        ...
                        assert (
                            tracked_session.metadata["session_index"]
                            == sess.metadata["session_index"]
                        ), (
                        ...
                        assert (
                            tracked_session.metadata["data"] == sess.metadata["data"]
                        ), (
        ...
        assert (
            len(results) == expected_total_requests
        ...
        for session in sessions:
            ...
            assert (
                retrieved.metadata["session_index"] == session.metadata["session_index"]
            ), f"Session metadata was corrupted during concurrent processing"

            # Verify no active requests remain
            assert (
                not retrieved.has_active_requests()
    ...
    **Validates: Requirements 6.5**
    ...
    try:
        ...
        for session in sessions_to_delete:
            ...
            assert result is True, f"Failed to delete session {session.session_id}"
        ...
        for session in sessions_to_keep:
            ...
            assert (
                retrieved.metadata["session_index"]
                == original["metadata"]["session_index"]
            ...
            assert (
                retrieved.conversation_id == original["conversation_id"]
        ...
        for session in sessions_to_keep:
            async with manager.track_request(session.session_id) as tracked_session:
                ...
                assert (
                    tracked_session.metadata["data"]
                    == original_data[session.session_id]["metadata"]["data"]
                )
...
@pytest.mark.asyncio
async def test_expired_session_cleanup_isolation(
    num_active_sessions, num_expired_sessions
):
    """
    ...
    try:
        ...
        for i in range(num_active_sessions):
            session = await manager.create_session(
                user_id=f"active-user-{i}", metadata={"type": "active", "index": i}
            )
        ...
        for i in range(num_expired_sessions):
            session = await manager.create_session(
                user_id=f"expired-user-{i}", metadata={"type": "expired", "index": i}
            )
        ...
        for session in active_sessions:
            ...
            assert (
                retrieved.metadata["type"] == "active"
            ), f"Active session metadata corrupted"
            assert (
                retrieved.metadata["index"] == session.metadata["index"]
