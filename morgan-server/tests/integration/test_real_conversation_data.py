...
class TestRealConversationImport:
    ...
    def test_emotional_intelligence_with_real_conversations(self):
        ...
        assert detections[0].primary_tone in [EmotionalTone.JOYFUL, EmotionalTone.EXCITED, EmotionalTone.CONTENT]
        # Last message should be positive (grateful for support)
        assert detections[-1].primary_tone in [EmotionalTone.JOYFUL, EmotionalTone.GRATEFUL, EmotionalTone.CONTENT]
    ...
    def test_emotional_trend_analysis_with_real_data(self):
        ...
        assert trend["dominant_tone"] in ["joyful", "grateful", "content", "excited", "neutral"]
    ...
    def test_memory_search_with_real_queries(self):
        ...
        for query, expected_keyword in test_queries:
            ...
            found = any(expected_keyword.lower() in message.content.lower() 
                       for _, message, _ in results)
            assert found, f"Expected to find '{expected_keyword}' in search results for '{query}'"
    ...
    def test_user_profile_with_real_data(self):
        ...
        assert any("work" in topic.lower() or "presentation" in topic.lower() 
...
class TestEndToEndChatFlow:
    ...
    def test_complete_conversation_flow(self):
        ...
        assert emotion.primary_tone in [EmotionalTone.EXCITED, EmotionalTone.JOYFUL, EmotionalTone.CONTENT]
    ...
    def test_multi_turn_conversation_with_context(self):
        ...
        assert len(results) > 0
        assert any("fastapi" in message.content.lower() 
                   for _, message, _ in results)
