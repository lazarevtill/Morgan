...
class TestServerConfig:
    ...
    def test_custom_values(self):
        ...
        config = ServerConfig(
            host="127.0.0.1", port=9000, llm_model="llama3", log_level="DEBUG"
        )
...
class TestLoadConfigFromFile:
    ...
    def test_load_yaml_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump({"host": "192.168.1.1", "port": 9090, "llm_model": "llama3"}, f)
    ...
    def test_load_json_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            json.dump({"host": "192.168.1.2", "port": 8888, "llm_model": "mistral"}, f)
            temp_path = Path(f.name)
...
class TestLoadConfig:
    ...
    def test_load_with_overrides(self):
        ...
        config = load_config(host="10.0.0.1", port=5000, llm_model="custom-model")
    ...
    def test_load_with_yaml_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(
                {"host": "172.16.0.1", "port": 3000, "llm_model": "yaml-model"}, f
            )
            temp_path = Path(f.name)
    ...
    def test_load_with_json_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            json.dump(
                {"host": "172.16.0.2", "port": 4000, "llm_model": "json-model"}, f
            )
            temp_path = Path(f.name)
    ...
    def test_precedence_override_over_file(self):
        ...
        try:
            config = load_config(config_file=temp_path, port=9999)  # Override
...
class TestValidateRequiredConfig:
    ...
    def test_invalid_cache_dir(self):
        ...
        try:
            ...
            with pytest.raises(
                ConfigurationError, match="Cannot create cache directory"
            ):
                validate_required_config(config)
        finally:
            temp_file.unlink()
...
class TestGetConfig:
    ...
    def test_get_config_with_overrides(self):
        ...
        config = get_config(validate=False, host="192.168.1.100", port=7000)
...
class TestConfigurationFormatSupport:
    ...
    **Validates: Requirements 3.2**
    ...
    @settings(max_examples=100)
    def test_property_format_handles_literals(self, embedding_device, log_format):
        """
    ...
    **Validates: Requirements 1.4, 3.4, 3.5**
    ...
    def test_property_invalid_port_rejected(self, port):
        ...
        assert any(
            ...
            for keyword in ["port", "validation", "greater", "less"]
        )
    ...
    def test_property_invalid_llm_provider_rejected(self, llm_provider):
        ...
        error_msg = str(exc_info.value).lower()
        assert any(
            keyword in error_msg for keyword in ["llm_provider", "literal", "input"]
        )
    ...
    def test_property_invalid_log_level_rejected(self, log_level):
        ...
        error_msg = str(exc_info.value).lower()
        assert any(
            keyword in error_msg for keyword in ["log_level", "literal", "input"]
        )
    ...
    def test_property_invalid_llm_endpoint_rejected(self, llm_endpoint):
        ...
        error_msg = str(exc_info.value).lower()
        assert any(
            keyword in error_msg for keyword in ["endpoint", "http", "empty", "value"]
        )
    ...
    def test_property_invalid_vector_db_url_rejected(self, vector_db_url):
        ...
        assert any(
            keyword in error_msg
            for keyword in ["url", "http", "empty", "value", "database"]
        )
    ...
    def test_property_invalid_embedding_device_rejected(self, embedding_device):
        ...
        assert any(
            keyword in error_msg
            for keyword in ["device", "embedding", "cpu", "cuda", "mps"]
        )

    @given(cache_size_mb=st.integers(max_value=0))  # Zero or negative cache size
    @settings(max_examples=100)
    def test_property_invalid_cache_size_rejected(self, cache_size_mb):
        ...
        assert any(
            keyword in error_msg
            for keyword in ["cache_size_mb", "greater", "validation"]
        )

    @given(workers=st.integers(max_value=0))  # Zero or negative workers
    @settings(max_examples=100)
    def test_property_invalid_workers_rejected(self, workers):
        ...
        error_msg = str(exc_info.value).lower()
        assert any(
            keyword in error_msg for keyword in ["workers", "greater", "validation"]
        )

    @given(timeout=st.integers(max_value=0))  # Zero or negative timeout
    @settings(max_examples=100)
    def test_property_invalid_timeout_rejected(self, timeout):
        ...
        error_msg = str(exc_info.value).lower()
        assert any(
            keyword in error_msg for keyword in ["timeout", "greater", "validation"]
        )

    @given(cache_dir=st.just(""))  # Empty cache directory
    @settings(max_examples=100)
    def test_property_empty_cache_dir_rejected(self, cache_dir):
        ...
        error_msg = str(exc_info.value).lower()
        assert any(
            keyword in error_msg for keyword in ["cache", "directory", "empty", "value"]
        )
    ...
    def test_property_invalid_embedding_provider_rejected(self, embedding_provider):
        ...
        assert any(
            keyword in error_msg
            for keyword in ["embedding_provider", "literal", "input"]
        )
    ...
    def test_property_invalid_embedding_endpoint_rejected(self, embedding_endpoint):
        ...
        assert any(
            keyword in error_msg
            for keyword in ["endpoint", "http", "empty", "value", "required"]
        )

    @given(provider=st.sampled_from(["ollama", "openai-compatible"]))
    @settings(max_examples=100)
    def test_property_remote_provider_requires_endpoint(self, provider):
        ...
        with pytest.raises(Exception) as exc_info:
            ServerConfig(
                embedding_provider=provider, embedding_endpoint=None  # Missing endpoint
            )
