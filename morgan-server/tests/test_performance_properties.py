...
def test_performance_under_load(num_concurrent_requests: int, request_complexity: str):
    ...
    **Validates: Requirements 6.4**
    ...
    def make_request(request_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                in [200, 503],  # 503 is OK for tests without full components
            }
    ...
    assert len(errors) == 0, f"Some requests failed under load:\n" + "\n".join(
        errors[:5]
    )
...
def test_sustained_load_performance():
    ...
    **Validates: Requirements 6.4**
    ...
    for round_num in range(num_rounds):
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=requests_per_round
        ) as executor:
            futures = [
                executor.submit(make_request, i) for i in range(requests_per_round)
            ]
            response_times = [
                future.result() for future in concurrent.futures.as_completed(futures)
            ]
    ...
    degradation_factor = (
        last_round_p95 / first_round_p95 if first_round_p95 > 0 else 1.0
    )
