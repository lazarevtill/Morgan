...
class TestSession:
    ...
    def test_session_creation(self):
        ...
        session = Session(
            session_id="test-123", user_id="user-456", conversation_id="conv-789"
        )
...
class TestSessionManager:
    ...
    async def test_create_session(self, session_manager):
        ...
        session = await session_manager.create_session(
            user_id="user-123", conversation_id="conv-456", metadata={"key": "value"}
        )
    ...
    async def test_session_isolation(self, session_manager):
        ...
        session1 = await session_manager.create_session(
            user_id="user-1", metadata={"data": "session1"}
        )
        session2 = await session_manager.create_session(
            user_id="user-2", metadata={"data": "session2"}
        )
    ...
    async def test_session_manager_stop_cleans_all_sessions(self):
        ...
        manager = SessionManager(
            session_timeout_minutes=60, cleanup_interval_seconds=300
        )
        await manager.start()
