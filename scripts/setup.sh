...
setup_netbird() {
    ...
    if [[ -z "${NETBIRD_SETUP_KEY:-}" ]]; then
        ...
        if [[ -z "$NETBIRD_SETUP_KEY" ]]; then
            ...
            log_info "To connect later, run: sudo netbird up --management-url https://vpn.example.com --setup-key <your-key>"
            return
        fi
    fi
    ...
    if sudo netbird up --management-url https://vpn.example.com --setup-key "$NETBIRD_SETUP_KEY" --daemon-addr ""; then
        ...
        log_error "Please connect manually: sudo netbird up --management-url https://vpn.example.com --setup-key <your-key>"
        exit 1
    fi
}
