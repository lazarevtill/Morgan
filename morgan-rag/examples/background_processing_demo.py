...
def demo_task_scheduler():
    ...
    for task in scheduler.list_tasks():
        print(f"  - {task.task_id}: {task.task_type} on {task.collection_name} ({task.frequency.value})")
...
def demo_background_service():
    ...
    queries = ["demo query 1", "demo query 2", "demo query 1"]  # Duplicate for popularity
    for query in queries:
