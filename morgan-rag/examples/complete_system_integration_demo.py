...
class SystemIntegrationDemo:
    ...
    async def initialize_system(self):
        ...
        try:
            ...
            if success:
                ...
                print(
                    f"   - Companion features: {self.config.enable_companion_features}"
                )
                print(
                    f"   - Emotional intelligence: {self.config.enable_emotional_intelligence}"
                )
                print(
                    f"   - Hierarchical search: {self.config.enable_hierarchical_search}"
                )
                print(
                    f"   - Batch optimization: {self.config.enable_batch_optimization}"
                )
                print(
                    f"   - Intelligent caching: {self.config.enable_intelligent_caching}"
                )
                print(
                    f"   - Performance monitoring: {self.config.enable_performance_monitoring}"
                )
            else:
        ...
        try:
            ...
            for user in self.demo_users[:2]:  # Process for first 2 users
                ...
                if result.success:
                    ...
                    print(
                        f"   ⚡ Processing rate: {result.performance_metrics.get('processing_rate', 0):.1f} docs/min"
                    )
                    print(
                        f"   🤖 Companion enhanced: {result.companion_metrics.get('companion_enhanced', False)}"
                    )
                    print(
                        f"   💭 Emotional context: {result.companion_metrics.get('emotional_context_applied', False)}"
                    )

                    # Check performance targets
                    processing_rate = result.performance_metrics.get(
                        "processing_rate", 0
                    )
                    if processing_rate >= self.config.target_processing_rate:
                        print(
                            f"   🎯 Processing rate target MET ({processing_rate:.1f} >= {self.config.target_processing_rate})"
                        )
                    else:
                        print(
                            f"   ⚠️  Processing rate below target ({processing_rate:.1f} < {self.config.target_processing_rate})"
                        )
    ...
    async def demo_search_workflow(self):
        ...
        try:
            for i, scenario in enumerate(test_scenarios, 1):
                ...
                print(
                    f"   💭 Emotion: {scenario['emotional_context']['primary_emotion']} "
                ...
                if result.success:
                    ...
                    print(
                        f"   ⚡ Search latency: {result.performance_metrics.get('search_latency', 0):.3f}s"
                    )
                    print(
                        f"   🎯 Candidate reduction: {result.performance_metrics.get('candidate_reduction', 0):.1%}"
                    )
                    print(
                        f"   🔄 Strategies used: {', '.join(result.performance_metrics.get('strategies_used', []))}"
                    )
                    print(
                        f"   🤖 Companion enhanced: {result.companion_metrics.get('companion_enhanced', False)}"
                    )
                    print(
                        f"   💭 Emotional enhanced: {result.companion_metrics.get('emotional_enhanced', False)}"
                    )
                    ...
                    if search_latency <= self.config.target_search_latency:
                        print(
                            f"   🎯 Search latency target MET ({search_latency:.3f}s <= {self.config.target_search_latency}s)"
                        )
                    else:
                        print(
                            f"   ⚠️  Search latency above target ({search_latency:.3f}s > {self.config.target_search_latency}s)"
                        )

                    candidate_reduction = result.performance_metrics.get(
                        "candidate_reduction", 0
                    )
                    if candidate_reduction >= self.config.target_candidate_reduction:
                        print(
                            f"   🎯 Candidate reduction target MET ({candidate_reduction:.1%} >= {self.config.target_candidate_reduction:.1%})"
                        )
                    else:
                        print(
                            f"   ⚠️  Candidate reduction below target ({candidate_reduction:.1%} < {self.config.target_candidate_reduction:.1%})"
                        )
            ...
            for scenario in conversation_scenarios:
                ...
                for i, conv in enumerate(scenario["conversations"], 1):
                    ...
                    if not conversation_id:
                        conversation_id = result.companion_metrics.get(
                            "conversation_id"
                        )
                    ...
                    if result.success:
                        print(
                            f"      ✅ Conversation processed in {conversation_time:.2f}s"
                        )
                        print(
                            f"      🧠 Memories extracted: {result.performance_metrics.get('memories_extracted', 0)}"
                        )
                        print(
                            f"      🎯 Response confidence: {result.performance_metrics.get('response_confidence', 0):.2f}"
                        )
                        print(
                            f"      📚 Sources used: {result.performance_metrics.get('sources_used', 0)}"
                        )
                        print(
                            f"      💭 Emotion detected: {result.companion_metrics.get('detected_emotion', 'none')}"
                        )
                        print(
                            f"      🤖 Relationship updated: {result.companion_metrics.get('relationship_updated', False)}"
                        )

                        if result.companion_metrics.get("detected_emotion"):
                            emotion_intensity = result.companion_metrics.get(
                                "emotion_intensity", 0
                            )
                            print(
                                f"      📊 Emotion intensity: {emotion_intensity:.1f}"
                            )
                    else:
                        print(
                            f"      ❌ Conversation failed: {'; '.join(result.errors)}"
                        )
    ...
    async def demo_system_health_monitoring(self):
        ...
        try:
            ...
            print(
                f"📅 Health Check Time: {health_status.timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            ...
            if health_status.performance_summary:
                ...
                if "system_resources" in perf:
                    ...
                    print(
                        f"   💾 Memory usage: {resources.get('current_memory_percent', 0):.1f}%"
                    )
                    print(
                        f"   🖥️  CPU usage: {resources.get('current_cpu_percent', 0):.1f}%"
                    )
                ...
                if "application_performance" in perf:
                    ...
                    if "search" in app_perf:
                        ...
                        print(
                            f"   🔍 Search P95: {search_perf.get('p95_duration', 0):.3f}s"
                        )
                    if "processing" in app_perf:
                        proc_perf = app_perf["processing"]
                        print(
                            f"   📚 Processing P95: {proc_perf.get('p95_duration', 0):.2f}s"
                        )
    ...
    async def demo_performance_validation(self):
        ...
        try:
            ...
            print(
                f"📊 Overall Performance: {'✅ PASSED' if validation_results['overall_success'] else '❌ FAILED'}"
            )
            ...
            print(f"\n📈 Performance Summary:")
            print(
                f"   🎯 Target Processing Rate: {self.config.target_processing_rate} docs/min"
            )
            ...
            print(
                f"   🎯 Target Candidate Reduction: {self.config.target_candidate_reduction:.0%}"
            )
    ...
    async def demo_error_handling_and_recovery(self):
        ...
        try:
            ...
            if not result.success:
                print(
                    f"   ✅ Gracefully handled empty documents: {result.errors[0] if result.errors else 'No specific error'}"
                )
            else:
            ...
            if not search_result.success:
                print(
                    f"   ✅ Gracefully handled empty query: {search_result.errors[0] if search_result.errors else 'No specific error'}"
                )
            else:
            ...
            if not conv_result.success:
                print(
                    f"   ✅ Gracefully handled invalid user: {conv_result.errors[0] if conv_result.errors else 'No specific error'}"
                )
            else:
    ...
    async def run_complete_demo(self):
        ...
        try:
            ...
            print("=" * 60)
            print(
                "✅ Document Processing Workflow - Demonstrated batch optimization and companion awareness"
            )
            print(
                "✅ Advanced Search Workflow - Showed hierarchical search with emotional intelligence"
            )
            print(
                "✅ Conversation Workflow - Illustrated relationship building and memory processing"
            )
            ...
            print(f"\n🚀 Key Achievements:")
            print(
                f"   • End-to-end workflows from document ingestion to companion responses"
            )
            ...
            print(
                f"   ✅ Integrated all components into cohesive companion-aware system"
            )
            print(
                f"   ✅ Implemented end-to-end workflows from document ingestion to companion responses"
            )
