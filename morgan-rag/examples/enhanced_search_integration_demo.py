...
def create_sample_emotional_context(emotion: str = "frustration", intensity: float = 0.8) -> Dict[str, Any]:
        ...
        try:
            ...
            if results:
                for j, result in enumerate(results, 1):
                    print(f"  {j}. Score: {result.score:.3f} | Type: {result.result_type}")
            ...
            search_results = multi_stage_engine.search(
                query=scenario['query'],
                max_results=3,
                strategies=["semantic", "memory"] if scenario['emotional_context'] else ["semantic"],
            ...
            if search_results.results:
                for j, result in enumerate(search_results.results, 1):
                    print(f"  {j}. Score: {result.score:.3f} | Strategy: {result.strategy}")
                    print(f"     RRF Score: {result.rrf_score:.3f}" if result.rrf_score else "     No RRF Score")
                    
                    # Show hierarchical scores if available
                    if hasattr(result, 'hierarchical_scores') and result.hierarchical_scores:
...
def demo_hierarchical_search_features():
    ...
    try:
        ...
        print(f"Query: '{query}'")
        print(f"Emotional Context: {emotional_context['primary_emotion']} ({emotional_context['intensity']})")
        ...
        if hierarchical_results:
            ...
            for i, result in enumerate(hierarchical_results, 1):
                ...
                if original_score != result.score:
                    ...
                    print(f"     Original Score: {original_score:.3f} → Enhanced: {result.score:.3f} (+{boost:.3f})")
                
                if enhancement_factors:
                    print(f"     Enhancement Factors: {', '.join(enhancement_factors)}")
                
                print(f"     Emotional Enhanced: {payload.get('emotional_enhanced', False)}")
                print(f"     Companion Enhanced: {payload.get('companion_enhanced', False)}")
                print()
        else:
            ...
            try:
                ...
                if results:
                    ...
                    for i, result in enumerate(results, 1):
                        ...
                        enhancement_factors = metadata.get('enhancement_factors', [])
                        if enhancement_factors:
                            print(f"     Personalization: {', '.join(enhancement_factors)}")
