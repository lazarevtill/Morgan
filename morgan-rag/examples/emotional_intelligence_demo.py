...
def demo_empathetic_responses():
    ...
    for i, (emotion_type, intensity, context) in enumerate(scenarios, 1):
        ...
        emotional_state = EmotionalState(
            primary_emotion=emotion_type, intensity=intensity, confidence=0.8
        )
...
def demo_user_profile_learning():
    ...
    print(
        f"  Preferred response length: {final_profile.communication_preferences.preferred_response_length.value}"
    )
    print(
        f"  Topics of interest: {final_profile.communication_preferences.topics_of_interest}"
    )
