...
def demo_remote_batch_optimization():
    ...
    individual_throughput = (
        len(individual_embeddings) / individual_time if individual_time > 0 else 0
    )

    print(
        f"  Results: {len(individual_embeddings)} documents in {individual_time:.2f}s"
    )
    ...
    try:
        ...
        batch_speedup = (
            individual_throughput / batch_throughput if batch_throughput > 0 else 0
        )
    ...
    try:
        ...
        optimized_speedup = (
            individual_throughput / optimized_throughput
        ...
        if optimized_speedup >= 5.0:  # Adjusted target for remote API
            print(
                f"  🎯 OPTIMIZATION TARGET ACHIEVED: {optimized_speedup:.1f}x >= 5x improvement!"
            )
        else:
            print(
                f"  📈 Performance improvement: {optimized_speedup:.1f}x (target: 5x+ for remote APIs)"
            )
...
def demo_remote_connection_pooling():
    ...
    no_pool_throughput = (
        len(no_pool_embeddings) / no_pool_time if no_pool_time > 0 else 0
    )
    ...
    try:
        ...
        pool_embeddings = embedding_service.encode_batch(
            test_texts, instruction="document", show_progress=False
        )

        pool_time = time.time() - start_time
        pool_throughput = len(pool_embeddings) / pool_time if pool_time > 0 else 0
        pool_speedup = (
            no_pool_throughput / pool_throughput if pool_throughput > 0 else 0
        )
...
def demo_remote_async_processing():
    ...
    print(
        f"Testing async processing with {len(companion_queries)} companion queries..."
    )
    ...
    for query in companion_queries:
        ...
        if result["success"]:
            print(
                f"  ✓ Processed: '{query[:40]}...' ({result['processing_time']:.2f}s)"
            )
        else:
    ...
    task_ids = []
    for query in companion_queries:
        task_id = async_processor.submit_companion_task(async_embed_query, query)
    ...
    successful_results = [
        r for r in async_results if r and r.success and r.result["success"]
    ]
...
def demo_integrated_remote_optimization():
    ...
    try:
        ...
        results = smart_search.find_relevant_info(
            query=user_query, max_results=3, use_enhanced_search=True
        )
        ...
        if processing_time < 2.0:  # Target for remote API integration
            ...
            print(
                f"  📈 Performance: {processing_time:.2f}s (target: <2.0s for remote integration)"
            )
