...
def demonstrate_memory_extraction():
    ...
    for i, (turn, emotional_state) in enumerate(zip(conversation_turns, emotional_states)):
        print(f"\n--- Turn {i+1} ---")
        print(f"Question: {turn.question[:100]}...")
        print(f"Emotion: {emotional_state.primary_emotion.value} (intensity: {emotional_state.intensity:.2f})")
    ...
    for content in test_contents:
        ...
        score = memory_processor.score_importance(content, context, emotional_weight=1.5)
    ...
    print(f"Average Importance Score: {sum(m.importance_score for m in all_memories) / len(all_memories):.3f}")
