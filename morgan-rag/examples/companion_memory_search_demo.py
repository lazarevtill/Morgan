...
def demo_companion_memory_search():
    ...
    try:
        ...
        for i, result in enumerate(emotional_results, 1):
            ...
            print(f"     Emotional: {result.emotional_context.get('has_emotional_content', False)}")
        ...
        for i, result in enumerate(relationship_results, 1):
            print(f"  {i}. Score: {result.score:.3f} | Significance: {result.relationship_significance:.2f}")
        ...
        for i, result in enumerate(enhanced_results, 1):
            ...
            if is_enhanced:
                print(f"     Memory Type: {result.metadata.get('memory_type', 'unknown')}")
                print(f"     Relationship: {result.metadata.get('relationship_significance', 0.0):.2f}")
    ...
    try:
        ...
        for scenario in scenarios:
            ...
            for i, result in enumerate(results, 1):
                print(f"  {i}. Score: {result.score:.3f}")
                print(f"     Personalization: {', '.join(result.personalization_factors)}")
                print(f"     Engagement: {result.user_engagement_score:.2f}")
                print()
