...
def demo_rrf_fusion():
    ...
    semantic_results = [
        SearchResult("Docker containerization platform", "docker-intro.md", 0.95, "knowledge", {}),
        SearchResult("Install Docker on Ubuntu", "install-guide.md", 0.88, "knowledge", {}),
        SearchResult("Kubernetes orchestration", "k8s-guide.md", 0.82, "knowledge", {}),
    ]
    
    keyword_results = [
        SearchResult("Install Docker on Ubuntu", "install-guide.md", 0.90, "knowledge", {}),  # Duplicate
        SearchResult("Docker Compose tutorial", "compose-guide.md", 0.85, "knowledge", {}),
        SearchResult("Container networking guide", "networking.md", 0.78, "knowledge", {}),
    ]
    
    category_results = [
        SearchResult("Docker security best practices", "security.md", 0.87, "knowledge", {}),
        SearchResult("Docker containerization platform", "docker-intro.md", 0.83, "knowledge", {}),  # Duplicate
    ]
    ...
    for i, result in enumerate(fused_results):
        ...
        print(f"   Found in {metadata['strategy_count']} strategies: {metadata['strategies_found_in']}")
    ...
    print("Manual calculation for 'Install Docker on Ubuntu' (appears in semantic #2 and keyword #1):")
    ...
    manual_rrf = (semantic_weight * (1.0 / (k + semantic_rank))) + (keyword_weight * (1.0 / (k + keyword_rank)))
    strategy_boost = 1.0 + (0.1 * (2 - 1))  # 2 strategies found it
    boosted_rrf = manual_rrf * strategy_boost
    
    print(f"  Strategy weights: Semantic={semantic_weight:.3f}, Keyword={keyword_weight:.3f}")
    print(f"  Semantic contribution: {semantic_weight:.3f} * 1/({k} + {semantic_rank}) = {semantic_weight * (1.0/(k + semantic_rank)):.6f}")
    print(f"  Keyword contribution: {keyword_weight:.3f} * 1/({k} + {keyword_rank}) = {keyword_weight * (1.0/(k + keyword_rank)):.6f}")
    ...
    install_result = next((r for r in fused_results if "Install Docker" in r.content), None)
    if install_result:
        print(f"  Actual raw RRF score: {install_result.metadata['rrf_raw_score']:.6f}")
        print(f"  Match: {'✓' if abs(install_result.metadata['rrf_raw_score'] - boosted_rrf) < 0.001 else '✗'}")
        ...
        print(f"  Final score: 0.7 * {boosted_rrf:.6f} + 0.3 * {avg_orig_score:.3f} = {final_score:.6f}")
        print(f"  Actual final score: {install_result.rrf_score:.6f}")
        print(f"  Final match: {'✓' if abs(install_result.rrf_score - final_score) < 0.001 else '✗'}")
