...
class MultimodalDocumentManager:
    ...
    def add_document(
        self, content: str, images: List[Any] = None, metadata: Dict[str, Any] = None
    ) -> MultimodalDocument:
        """
        ...
        document = self.multimodal_processor.process_multimodal_document(
            content=content, images=images, metadata=metadata
        )
        ...
        logger.info(
            f"Added document. Collection now has {len(self.documents)} documents"
        )
        return document

    def search(
        self, query: str, include_images: bool = True, max_results: int = 10
...
def demonstrate_multimodal_processing():
    ...
    try:
        ...
        try:
            ...
            print(
                f"     Combined embedding dim: {len(multimodal_doc.combined_embedding)}"
            )
        ...
        for query in search_queries:
            print(f"\n   Query: '{query}'")
            results = manager.search(query=query, include_images=True, max_results=3)
        ...
        for i, result in enumerate(image_results):
            ...
            print(
                f"       Images: {len(result.images)}, Score: {result.relevance_score:.3f}"
            )
