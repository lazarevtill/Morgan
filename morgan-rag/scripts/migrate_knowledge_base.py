...
Examples:
  ...
  python migrate_knowledge_base.py cleanup --keep-days 7      # Clean old backups
    ...
    migrate_parser = subparsers.add_parser(
        "migrate",
        help="Migrate collection to hierarchical format"
    )
...
def cmd_analyze(args):
    ...
    if args.collection:
        ...
        print(f"Estimated Time: {analysis['estimated_migration_time_minutes']:.1f} minutes")
        ...
        for analysis in collections:
            if analysis.get("exists", False):
                ...
                print(f"{analysis['collection_name']:<30} {analysis['total_points']:<10} {format_str:<20} {analysis['migration_needed']:<20} {analysis['estimated_migration_time_minutes']:<15.1f}")
...
def cmd_migrate(args):
    ...
    print(f"{'Dry run' if args.dry_run else 'Executing'} migration for '{args.source_collection}'...")
...
def cmd_validate(args):
    ...
    print(f"Validating migration: {args.source_collection} -> {args.target_collection}...")
    ...
    if result.total_points_checked > 0:
        print(f"Validation Rate: {(result.points_valid / result.total_points_checked * 100):.1f}%")
...
def cmd_list_backups(args):
    ...
    for backup in backups:
        print(f"{backup['file_name']:<40} {backup['collection_name']:<20} {backup['total_points']:<10} {backup['file_size_mb']:<12.1f} {backup['backup_timestamp'][:19]}")
