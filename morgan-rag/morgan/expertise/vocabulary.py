...
class DomainVocabulary:
    ...
    def get_learning_velocity(self, weeks: int = 4) -> float:
        ...
        new_terms = [
            term for term in self.terms.values()
            if term.first_seen >= cutoff_date
        ]
        return len(new_terms) / weeks
    
    def get_term_suggestions(self, partial_term: str, limit: int = 5) -> List[VocabularyTerm]:
        ...
        suggestions.sort(
            key=lambda t: (t.usage_count, t.confidence_score), 
            reverse=True
        )
...
class VocabularyLearner:
    ...
    def learn_from_interaction(
        ...
        try:
            ...
            logger.info(f"Learned {len(learned_terms)} new terms for {user_id} in {domain}")
    ...
    def get_domain_vocabulary(self, user_id: str, domain: str) -> DomainVocabulary:
        ...
        """
        # Check cache first
        if user_id in self.vocabulary_cache and domain in self.vocabulary_cache[user_id]:
            ...
            try:
                ...
                for term_data in data.get("terms", []):
                    term = VocabularyTerm(
                        ...
                        learning_source=LearningSource(term_data.get("learning_source", "user_usage")),
    ...
    def get_vocabulary_stats(self, user_id: str, domain: str) -> Dict[str, Any]:
        ...
        type_distribution = Counter(term.term_type.value for term in vocabulary.terms.values())
        
        # Confidence distribution
        high_confidence = len([t for t in vocabulary.terms.values() if t.confidence_score > 0.7])
        medium_confidence = len([t for t in vocabulary.terms.values() if 0.3 < t.confidence_score <= 0.7])
        low_confidence = len([t for t in vocabulary.terms.values() if t.confidence_score <= 0.3])
        ...
        return {
            ...
            "user_defined_terms": len([t for t in vocabulary.terms.values() if t.user_defined]),
    ...
    def save_domain_vocabulary(self, vocabulary: DomainVocabulary):
        ...
        # Save to storage
        vocab_path = self.storage_path / f"{vocabulary.user_id}_{vocabulary.domain_name}_vocabulary.json"
        ...
        try:
            ...
            logger.error(f"Error saving vocabulary for {vocabulary.user_id}/{vocabulary.domain_name}: {e}")
    ...
    def _extract_terms(self, text: str, domain: str) -> List[Tuple[str, TermType]]:
        ...
        for i, word in enumerate(words):
            ...
            if word[0].isupper() and len(word) > 2:
                ...
                if any(tech_word in context for tech_word in ["using", "with", "implement", "configure"]):
