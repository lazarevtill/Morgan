...
class DomainContextEngine:
    ...
    def __init__(self):
        ...
        self.storage_path = Path(self.settings.data_dir) / "context"
    ...
    def build_context(
        ...
        try:
            ...
            new_elements = self._extract_context_elements(
                message_text, domain, user_id
            )
            ...
            try:
                ...
                for element_data in data.get("elements", []):
                    element = ContextElement(
                        ...
                        prerequisite_elements=element_data.get("prerequisite_elements", []),
                        ...
                        last_accessed=datetime.fromisoformat(element_data["last_accessed"]),
    ...
    def get_contextual_suggestions(
        ...
        for element in context.elements.values():
            if (current_topic.lower() in element.content.lower() or
                any(keyword.lower() in current_topic.lower() 
        ...
        suggestions.sort(
            key=lambda s: (
                {"critical": 4, "high": 3, "medium": 2, "low": 1}.get(s.get("relevance", "low"), 1),
        ...
        logger.info("Generated %d contextual suggestions for %s", 
                   len(unique_suggestions), current_topic)
        return unique_suggestions
    
    def identify_context_gaps(
        self, 
        user_id: str, 
        domain: str
    ) -> List[Dict[str, Any]]:
        ...
        for element in context.elements.values():
            ...
            if missing_prereqs:
                gaps.append({
                    ...
                    "impact": "high" if element.relevance == ContextRelevance.CRITICAL else "medium"
                })
        
        # Find isolated elements (no relationships)
        for element in context.elements.values():
            if (not element.related_elements and 
                not element.prerequisite_elements and 
                not element.dependent_elements):
                gaps.append({
        ...
        logger.info("Identified %d context gaps for %s in %s", 
                   len(gaps), user_id, domain)
        return gaps
    ...
    def save_domain_context(self, context: DomainContext):
        ...
        try:
            ...
            logger.error("Error saving context for %s/%s: %s", 
                        context.user_id, context.domain_name, e)
            ...
            for existing_id, existing_element in context.elements.items():
                ...
                if total_words > 0:
                    ...
                    if similarity > 0.3:
                        ...
                        if new_element.element_id not in existing_element.related_elements:
                            existing_element.related_elements.append(new_element.element_id)
                
                # Check for prerequisite relationships
                if any(word in new_element.content.lower() 
                    ...
                    if new_element.element_id not in existing_element.dependent_elements:
                        existing_element.dependent_elements.append(new_element.element_id)
    ...
    def _update_workflow_patterns(self, context: DomainContext, text: str):
        ...
        if len(workflow_indicators) >= 2:
            ...
            for sentence in sentences:
                ...
                if any(indicator in sentence.lower() for indicator in self.WORKFLOW_INDICATORS):
