...
class DomainDepthProfile:
    ...
    def get_topics_by_level(self, level: KnowledgeLevel) -> List[str]:
        ...
        return [
            topic for topic, assessment in self.topic_assessments.items()
    ...
    def get_learning_recommendations(self) -> List[Dict[str, Any]]:
        ...
        for topic, assessment in self.topic_assessments.items():
            if assessment.knowledge_level in [KnowledgeLevel.SURFACE, KnowledgeLevel.FUNCTIONAL]:
                recommendations.append({
                    "type": "deepen_knowledge",
                    "topic": topic,
                    "current_level": assessment.knowledge_level.value,
                    "target_level": KnowledgeLevel.WORKING.value,
                    "priority": "high" if assessment.confidence_score > 0.6 else "medium"
                })
    ...
    def _update_overall_metrics(self):
        ...
        working_plus_topics = len([
            ...
            if a.knowledge_level in [KnowledgeLevel.WORKING, KnowledgeLevel.DEEP, KnowledgeLevel.EXPERT]
        ])
        self.breadth_score = min(working_plus_topics / 10.0, 1.0)  # Normalize to 10 topics
        ...
        if len(assessments) > 1:
            ...
            variance = sum((x - mean_level) ** 2 for x in level_values_list) / len(level_values_list)
...
class KnowledgeDepthAssessor:
    ...
    def assess_knowledge_depth(
        ...
        """
        logger.info("Assessing knowledge depth for user %s in domain %s", user_id, domain)
        ...
        if profile_path.exists():
            try:
                ...
                profile = DomainDepthProfile(
                    ...
                    preferred_depth=KnowledgeLevel(data.get("preferred_depth", "working")),
                ...
                for assessment_data in data.get("topic_assessments", []):
                    assessment = KnowledgeAssessment(
                        ...
                        knowledge_level=KnowledgeLevel(assessment_data["knowledge_level"]),
                        confidence_score=assessment_data.get("confidence_score", 0.0),
                        depth_score=assessment_data.get("depth_score", 0.0),
                        assessment_method=AssessmentMethod(assessment_data["assessment_method"]),
                        evidence=assessment_data.get("evidence", []),
                        indicators=assessment_data.get("indicators", {}),
                        previous_level=KnowledgeLevel(assessment_data["previous_level"]) if assessment_data.get("previous_level") else None,
                ...
                logger.error("Error loading depth profile for %s/%s: %s", user_id, domain, e)
    ...
    def get_appropriate_content_level(
        ...
        if assessment:
            ...
            if current_level == KnowledgeLevel.SURFACE:
                ...
                return KnowledgeLevel.DEEP if assessment.confidence_score > 0.7 else KnowledgeLevel.WORKING
            else:
    ...
    def identify_learning_opportunities(
        ...
        logger.info("Identified %d learning opportunities for %s", 
                   len(opportunities), user_id)
    ...
    def save_depth_profile(self, profile: DomainDepthProfile):
        ...
        # Save to storage
        profile_path = self.storage_path / f"{profile.user_id}_{profile.domain_name}_depth.json"
        ...
        try:
            ...
            for assessment in profile.topic_assessments.values():
                assessments_data.append({
                    ...
                    "previous_level": assessment.previous_level.value if assessment.previous_level else None,
            ...
            logger.error("Error saving depth profile for %s/%s: %s", 
                        profile.user_id, profile.domain_name, e)
    ...
    def _extract_topics(self, text: str, domain: str) -> List[str]:
        ...
        for topic in topics:
            ...
            if (len(topic_clean) > 2 and 
                topic_clean not in seen and
                not topic_clean.isdigit()):
    ...
    def _assess_topic_knowledge(
        ...
        if not any(complexity_scores.values()):
            ...
            max_level = max(complexity_scores.keys(), key=lambda k: complexity_scores[k])
    ...
    def _update_assessment(
        self, 
        existing: KnowledgeAssessment, 
        new: KnowledgeAssessment
    ):
        ...
        existing.confidence_score = (
            (1 - weight) * existing.confidence_score + 
            weight * new.confidence_score
        )
        
        # Update depth score
        existing.depth_score = (
            (1 - weight) * existing.depth_score + 
            weight * new.depth_score
        )
        ...
        for key, value in new.indicators.items():
            if key in existing.indicators:
                existing.indicators[key] = (
                    (1 - weight) * existing.indicators[key] + 
                    weight * value
                )
            else:
