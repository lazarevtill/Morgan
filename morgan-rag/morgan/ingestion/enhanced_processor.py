...
class EnhancedDocumentProcessor:
    ...
    def __init__(self):
        ...
        self.max_file_size = (
            self.settings.morgan_max_file_size * 1024 * 1024
        )  # Convert MB to bytes
    ...
    def process_source(
        ...
        logger.info(
            f"Processing source: {source_path} (type={document_type}, strategy={chunk_strategy})"
        )
        ...
        try:
            ...
            if self._is_url(source_path):
                ...
                return self._process_file(
                    Path(source_path), document_type, chunk_strategy
                )
            elif Path(source_path).is_dir():
                return self._process_directory(
                    Path(source_path), document_type, chunk_strategy, show_progress
                )
            else:
    ...
    def chunk_document(
        ...
        logger.debug(
            f"Chunking {len(content)} chars using {strategy} strategy (max_size={max_size})"
        )

        if strategy == "semantic":
            return self._chunk_semantic(
                content, document_type, source, max_size, overlap
            )
        elif strategy == "fixed":
            return self._chunk_fixed(content, source, max_size, overlap)
        elif strategy == "adaptive":
            return self._chunk_adaptive(
                content, document_type, source, max_size, overlap
            )
        else:
    ...
    def extract_metadata(
        self, content: str, source_path: str, document_type: str = "auto"
    ) -> Dict[str, Any]:
        """
    ...
    def _process_file(
        self, file_path: Path, document_type: str, chunk_strategy: str
    ) -> ProcessingResult:
    ...
    def _process_url(
        self, url: str, document_type: str, chunk_strategy: str
    ) -> ProcessingResult:
        ...
        try:
            # Fetch content
            headers = {"User-Agent": "Morgan RAG Document Processor 1.0"}
    ...
    def _extract_pdf_content(self, file_path: Path) -> str:
        ...
        if not HAS_PDFPLUMBER and not HAS_PYPDF2:
            logger.error("PDF processing not available - install pdfplumber or PyPDF2")
            return ""
    ...
    def _extract_office_content(self, file_path: Path) -> str:
        ...
        try:
            if extension in [".docx", ".doc"]:
                if not HAS_DOCX:
                    logger.error(
                        "Word document processing not available - install python-docx"
                    )
                    return ""
                ...
                if not HAS_OPENPYXL:
                    logger.error(
                        "Excel document processing not available - install openpyxl"
                    )
                    return ""
    ...
    def _extract_web_content(self, html_content: str, url: str = "") -> str:
        ...
        if not HAS_BS4:
            logger.warning(
                "HTML processing not available - install beautifulsoup4. Returning raw HTML."
            )
    ...
    def _chunk_semantic(
        self, content: str, document_type: str, source: str, max_size: int, overlap: int
    ) -> List[DocumentChunk]:
        """
    ...
    def _chunk_code_semantic(
        self, content: str, source: str, max_size: int, overlap: int
    ) -> List[DocumentChunk]:
        """
    ...
    def _chunk_python_code(
        ...
        try:
                ...
                if isinstance(
                    ...
                    if len(chunk_content) <= max_size:
                        ...
                        boundaries = ChunkBoundaries(
                            ...
                            context_before=(
                                lines[max(0, start_line - 2) : start_line]
                                if start_line > 0
                                else []
                            ),
                            context_after=(
                                lines[end_line : end_line + 2]
            ...
            logger.warning(
                f"Python syntax error in {source}, using generic code chunking: {e}"
            )
    ...
    def _chunk_adaptive(
        ...
        for chunk in semantic_chunks:
            if len(chunk.content) < max_size * 0.3:  # Very small chunk
                ...
                if (
                    adjusted_chunks
                    and len(adjusted_chunks[-1].content) + len(chunk.content)
                    <= max_size
                ):
                    ...
                    prev_chunk.metadata["merged_chunks"] = (
                        prev_chunk.metadata.get("merged_chunks", 1) + 1
                    )
                else:
    ...
    def _split_large_code_block(
        self, content: str, source: str, start_line: int, max_size: int, overlap: int
    ) -> List[DocumentChunk]:
        ...
        for i, line in enumerate(lines):
            ...
            if current_size + line_size <= max_size:
                ...
                if overlap > 0 and current_chunk_lines:
                    ...
                    current_size = sum(
                        len(line_text) + 1 for line_text in current_chunk_lines
                    )
                else:
    ...
    def _add_overlap_to_chunks(
        self, chunks: List[DocumentChunk], overlap: int
    ) -> List[DocumentChunk]:
        """
        ...
        for i in range(1, len(chunks)):
            ...
            overlap_text = (
                previous_chunk.content[-overlap:]
                if len(previous_chunk.content) > overlap
    ...
    def _extract_code_metadata(self, content: str) -> Dict[str, Any]:
        ...
        function_count = len(
            re.findall(r"^\s*(def|function|func)\s+\w+", content, re.MULTILINE)
        )
        class_count = len(
            re.findall(r"^\s*(class|struct|interface)\s+\w+", content, re.MULTILINE)
        )
        ...
        metadata.update(
            {
                ...
                "import_count": len(
                    re.findall(
                        r"^\s*(import|from|#include|using)", content, re.MULTILINE
                    )
                ),
            }
        )
    ...
    def _extract_markdown_metadata(self, content: str) -> Dict[str, Any]:
        ...
        for level_marks, title in headers:
            ...
            header_counts[f"h{level}_count"] = (
                header_counts.get(f"h{level}_count", 0) + 1
            )
    ...
    def _extract_web_metadata(self, content: str) -> Dict[str, Any]:
        ...
        if not HAS_BS4:
            logger.warning(
                "HTML metadata extraction not available - install beautifulsoup4"
            )
            return metadata
