...
from morgan.utils.error_handling import StorageError, ValidationError
...
class VectorDBClient:
    ...
    @handle_storage_errors(
        "upsert_points", "vector_db_client", RetryConfig(max_attempts=3, base_delay=2.0)
    )
                ...
                try:
                    ...
                    def upsert_operation(
                        ...
                        if self.use_connection_pooling:
                            try:
                                with self.pool_manager.get_connection(
                                    "qdrant"
                                ) as pooled_client:
                    ...
                    logger.warning(
                        f"Optimized batch upsert failed, falling back to standard: {e}"
                    )
    ...
    @handle_storage_errors(
        "search", "vector_db_client", RetryConfig(max_attempts=2, base_delay=1.0)
    )
    ...
    def search_with_filter(
        self, collection_name: str, filter_conditions: Dict[str, Any], limit: int = 100
    ) -> List[SearchResult]:
        """
        ...
        try:
            ...
            results = self.client.scroll(
                collection_name=collection_name, scroll_filter=filter_obj, limit=limit
            )
        ...
        try:
            ...
            for schema_def in companion_schemas:
                if not self.collection_exists(schema_def["name"]):
                    ...
                    success_count += 1
                    logger.debug(f"Collection already exists: {schema_def['name']}")
    ...
    def create_hierarchical_collection(
        ...
        try:
            ...
            logger.info(
                f"Created hierarchical collection '{name}' with {coarse_size}/{medium_size}/{fine_size}D vectors"
            )
            return True
    ...
    def upsert_companion_profile(
        self, user_id: str, profile_embedding: List[float], profile_data: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            # Add metadata
            profile_data.update(
                {"updated_at": datetime.now().isoformat(), "profile_version": "1.0"}
            )
            ...
            for i in range(0, len(memories), self.batch_size):
                ...
                for memory in batch:
                    try:
                        ...
                        errors.append(
                            f"Failed to prepare memory {memory.get('id', 'unknown')}: {e}"
                        )
                        continue
    ...
    def upsert_emotional_states_batch(
        self, emotional_states: List[Dict[str, Any]]
    ) -> BatchOperationResult:
        """
            ...
            for i in range(0, len(validated_states), self.batch_size):
                ...
                for state in batch:
                    try:
                        ...
                        errors.append(
                            f"Failed to prepare emotional state {state.get('id', 'unknown')}: {e}"
                        )
                        continue
                ...
                if batch_success:
                    ...
                    errors.append(
                        f"Failed to upsert emotional batch {i//self.batch_size + 1}"
                    )
                ...
                if isinstance(value, dict) and "gte" in value:
                    must_conditions.append(
                        models.FieldCondition(
                            key=key, range=models.Range(gte=value["gte"])
                        )
                    )
                else:
                    must_conditions.append(
    ...
    def search_hierarchical(
        ...
        try:
            ...
            if fine_vector:
                ...
                if candidate_ids:
                    final_filter = models.Filter(
                        must=[models.HasIdCondition(has_id=candidate_ids[: limit * 2])]
                    )
            ...
            logger.debug(
                f"Hierarchical search completed: {len(enhanced_results)} enhanced results"
            )
        ...
        try:
            ...
            filter_obj = models.Filter(
                must=[
                    ...
                    models.FieldCondition(
                        key="timestamp", range=models.DatetimeRange(gte=threshold_date)
                    ),
                ]
            )
            ...
            for result in results[0]:
                search_results.append(
                    SearchResult(
                        id=str(result.id), score=1.0, payload=result.payload or {}
                    )
                )

            logger.debug(
                f"Found {len(search_results)} emotional records for user {user_id}"
            )
            return search_results
            ...
            for result in search_results:
                ...
                if emotional_context:
                    ...
                    if emotional_boost > 0:
                        enhancement_factors.append(
                            f"emotional_boost:{emotional_boost:.3f}"
                        )
                ...
                if user_id:
                    ...
                    if companion_boost > 0:
                        enhancement_factors.append(
                            f"companion_boost:{companion_boost:.3f}"
                        )
    ...
    def _calculate_emotional_relevance_boost(
        self, result: SearchResult, emotional_context: Dict[str, Any]
    ) -> float:
        """
        ...
        try:
            ...
            indicator_matches = sum(
                1 for indicator in emotional_indicators if indicator.lower() in content
            )
            ...
            if primary_emotion in negative_emotions:
                supportive_matches = sum(
                    1 for keyword in supportive_keywords if keyword in content
                )
    ...
    def _calculate_companion_personalization_boost(
        self, result: SearchResult, user_id: str
    ) -> float:
        """
    ...
    def _calculate_content_quality_boost(self, result: SearchResult) -> float:
        ...
        try:
            ...
            if any(
                indicator in content for indicator in ["#", "##", "```", "- ", "1. "]
            ):
                boost += 0.03
            ...
            if (
                len(content) > 200 and len(content) < 5000
            ):  # Sweet spot for completeness
                boost += 0.02
    ...
    def search_with_emotional_context(
        ...
        try:
            ...
            if score_threshold:
                enhanced_results = [
                    r for r in enhanced_results if r.score >= score_threshold
                ]
    ...
    def upsert_hierarchical_points(
        ...
        try:
            ...
            for point in points:
                ...
                missing_vectors = [
                    v for v in required_vectors if v not in point["vector"]
                ]
                if missing_vectors:
                    raise ValidationError(
            ...
            if use_batch_optimization and len(points) > 50:
                try:
                    ...
                    def hierarchical_upsert_operation(
                        batch_points: List[Dict[str, Any]],
                    ) -> List[Any]:
                        qdrant_points = []
                        for point in batch_points:
                    ...
                    logger.warning(
                        f"Optimized hierarchical batch upsert failed, falling back to standard: {e}"
                    )
            ...
            logger.debug(
                f"Upserted {len(points)} hierarchical points to '{collection_name}'"
            )
            return True
    ...
    def dual_write_points(
        ...
        """
        results = {"legacy": False, "hierarchical": False}
        ...
        try:
            ...
            try:
                ...
                if results["legacy"]:
                    logger.debug(
                        f"Successfully wrote {len(points)} points to legacy collection '{legacy_collection}'"
                    )
            except Exception as e:
                logger.error(
                    f"Failed to write to legacy collection '{legacy_collection}': {e}"
                )
            ...
            try:
                ...
                if results["hierarchical"]:
                    logger.debug(
                        f"Successfully wrote {len(hierarchical_points)} points to hierarchical collection '{hierarchical_collection}'"
                    )
            except Exception as e:
                logger.error(
                    f"Failed to write to hierarchical collection '{hierarchical_collection}': {e}"
                )
            ...
            if results["legacy"] and results["hierarchical"]:
                logger.info(
                    f"Dual-write successful: {len(points)} points to both collections"
                )
            elif results["legacy"]:
                logger.warning(
                    f"Partial dual-write: only legacy collection '{legacy_collection}' succeeded"
                )
            elif results["hierarchical"]:
                logger.warning(
                    f"Partial dual-write: only hierarchical collection '{hierarchical_collection}' succeeded"
                )
            else:
                    ...
                    if hasattr(collection_info.config, "params") and hasattr(
                        ...
                        if isinstance(vectors_config, dict):
                            ...
                            if all(v in existing_vectors for v in required_vectors):
                                logger.debug(
                                    f"Hierarchical collection '{name}' already exists with proper configuration"
                                )
                                return True
                            else:
                                logger.warning(
                            ...
                            logger.warning(
                                f"Collection '{name}' exists but doesn't have named vectors configuration"
                            )
                            return False
                    else:
                        logger.warning(
                            f"Collection '{name}' exists but configuration is not accessible"
                        )
                        return False

                except Exception as e:
                    logger.warning(
                        f"Could not validate existing collection '{name}': {e}"
                    )
    ...
    def get_hierarchical_collection_info(self, collection_name: str) -> Dict[str, Any]:
        ...
        try:
                ...
                if isinstance(vectors_config, dict):
                    ...
                    for vector_name, vector_config in vectors_config.items():
                        result["named_vectors"][vector_name] = {
                            ...
                            "distance": (
                                vector_config.distance.name
                                if hasattr(vector_config.distance, "name")
                    ...
                    required_vectors = ["coarse", "medium", "fine"]
                    has_all_vectors = all(
                        v in result["named_vectors"] for v in required_vectors
                    )
                    ...
                    if not has_all_vectors:
                        missing = [
                            ...
                            if v not in result["named_vectors"]
                        ]
            ...
            logger.error(
                f"Failed to get hierarchical collection info for '{collection_name}': {e}"
            )
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    logger.warning(
                        f"Upsert attempt {attempt + 1} failed, retrying: {e}"
                    )
