...
from morgan.utils.error_handling import (
    ...
    get_degradation_manager, get_recovery_manager, DegradationLevel
)
...
class CompanionFeatureManager:
    ...
    def record_feature_success(self, feature_type: CompanionFeatureType, 
        ...
        logger.debug("Recorded companion feature success",
                    feature_type=feature_type.value,
                    operation=operation,
                    duration=duration,
                    confidence_score=confidence_score)
    
    def record_feature_error(self, feature_type: CompanionFeatureType, 
        ...
        logger.error("Recorded companion feature error",
                    feature_type=feature_type.value,
                    operation=operation,
                    error=str(error))
    ...
    def _apply_feature_degradation(self, feature_type: CompanionFeatureType):
        ...
        if feature_type in [CompanionFeatureType.EMOTIONAL_ANALYSIS, 
                           CompanionFeatureType.EMPATHY_GENERATION]:
        ...
        elif feature_type in [CompanionFeatureType.RELATIONSHIP_TRACKING,
                             CompanionFeatureType.PERSONALIZATION]:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if enable_sentiment_fallback:
                    try:
                        logger.warning("Emotional analysis failed, attempting sentiment fallback")
                        ...
                        logger.error(f"Sentiment analysis fallback failed: {sentiment_error}")
...
def handle_relationship_tracking_errors(
    enable_basic_tracking: bool = True,
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not degradation_manager.is_feature_enabled("companion"):
                logger.debug("Companion features disabled, skipping relationship tracking")
                return None
            ...
            try:
                ...
                if enable_basic_tracking:
                    try:
                        logger.warning("Relationship tracking failed, using basic interaction counting")
                        ...
                        logger.error(f"Basic interaction tracking failed: {basic_error}")
...
def handle_memory_processing_errors(
    enable_simple_extraction: bool = True,
    skip_on_failure: bool = True
):
    """
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if enable_simple_extraction:
                    try:
                        logger.warning("Memory processing failed, using simple keyword extraction")
                        
                        conversation_turn = kwargs.get("conversation_turn", args[0] if args else None)
                        if conversation_turn:
                            simple_memories = _simple_memory_extraction(conversation_turn)
...
def handle_empathy_generation_errors(
    ...
    enable_neutral_response: bool = True
):
    """
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if enable_template_fallback:
                    try:
                        logger.warning("Empathy generation failed, using template-based response")
                        
                        user_emotion = kwargs.get("user_emotion")
                        context = kwargs.get("context", "")
                        
                        template_response = _template_empathetic_response(user_emotion, context)
                        logger.info("Template-based empathy response successful")
                        return template_response
                        
                    except Exception as template_error:
                        logger.error(f"Template-based empathy response failed: {template_error}")
...
def _simple_sentiment_analysis(text: str):
    ...
    positive_words = ["happy", "good", "great", "excellent", "wonderful", "amazing", "love"]
...
def _template_empathetic_response(user_emotion, context: str):
    ...
    emotion_key = getattr(user_emotion, 'primary_emotion', 'neutral') if user_emotion else 'neutral'
...
def register_companion_recovery_procedures():
    ...
    def recover_emotional_processing(error: EmotionalProcessingError, context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if total_ops > 0:
                ...
                if error_rate < 0.5:  # If error rate is below 50%, consider recoverable
                    feature_manager.feature_health[CompanionFeatureType.EMOTIONAL_ANALYSIS] = True
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Emotion detection successful: {result.primary_emotion} ({result.confidence})")
    except Exception as e:
    ...
    try:
        ...
        print(f"Emotion detection with fallback: {result.primary_emotion} ({result.confidence})")
    except Exception as e:
        print(f"Emotion detection failed: {e}")
    
    # Test feature manager
    feature_manager = get_companion_feature_manager()
    print(f"Feature health: {feature_manager.get_feature_health(CompanionFeatureType.EMOTIONAL_ANALYSIS)}")
