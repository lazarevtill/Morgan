...
from morgan.utils.error_handling import (
    MorganError, ErrorCategory, ErrorSeverity, RetryConfig, with_retry,
    get_degradation_manager, get_recovery_manager, CircuitBreaker
)
...
class JinaEmbeddingError(MorganError):
    ...
    def __init__(self, message: str, model_type: Optional[JinaModelType] = None, **kwargs):
        super().__init__(
            message,
...
class JinaServiceManager:
    ...
    def _initialize_circuit_breakers(self):
        ...
        for service_type, config in configs.items():
            self.circuit_breakers[service_type.value] = CircuitBreaker(
                name=f"jina_{service_type.value}",
                config=config
            )
    ...
    def record_service_success(self, service_type: JinaServiceType, 
        ...
        logger.debug("Recorded Jina service success",
                    service_type=service_type.value,
                    operation=operation,
                    duration=duration)
    
    def record_service_error(self, service_type: JinaServiceType, 
        ...
        error_rate = metrics["error_count"] / total_operations if total_operations > 0 else 1.0
        
        self.service_health[service_type] = error_rate < 0.1  # Healthy if error rate < 10%
        
        logger.error("Recorded Jina service error",
                    service_type=service_type.value,
                    operation=operation,
                    error=str(error),
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    service_manager.record_service_success(service_type, operation, duration)
            ...
            if enable_fallback and model_type:
                ...
                for fallback_model in fallback_models:
                    try:
                        ...
                        service_manager.record_service_success(service_type, f"{operation}_fallback", duration)
            ...
            try:
                ...
                if "model" in str(e).lower() and "load" in str(e).lower():
                    if enable_local_fallback:
                        logger.warning("Jina embedding model failed to load, attempting local fallback")
                        ...
                        try:
                            ...
                            logger.error(f"Local embedding fallback failed: {fallback_error}")
...
def handle_jina_reranking_errors(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            except JinaRerankingError as e:
                if enable_embedding_fallback:
                    logger.warning("Jina reranking failed, falling back to embedding-based ranking")
                    
                    try:
                        # Fallback to embedding-based ranking
                        query = kwargs.get("query", args[0] if args else "")
                        results = kwargs.get("results", args[1] if len(args) > 1 else [])
                        ...
                        logger.error(f"Embedding-based ranking fallback failed: {fallback_error}")
...
def handle_jina_web_scraping_errors(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            except JinaWebScrapingError as e:
                if enable_html_fallback:
                    url = kwargs.get("url", args[0] if args else None)
                    
                    if url:
                        logger.warning(f"Jina web scraping failed for {url}, attempting basic HTML parsing")
                        ...
                        try:
                            ...
                            logger.error(f"Basic HTML parsing fallback failed: {fallback_error}")
...
def handle_jina_multimodal_errors(
    ...
    def decorator(func: Callable) -> Callable:
        @with_jina_retry(
            service_type=JinaServiceType.MULTIMODAL,
            ...
            try:
                ...
                if enable_text_only_fallback and text_content:
                    ...
                    try:
                        ...
                        return {
                            "text_embeddings": text_embeddings[0] if text_embeddings else [],
                            "image_embeddings": [],
                            "combined_embedding": text_embeddings[0] if text_embeddings else [],
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                current_failures = handle_background_task_errors.failure_counts.get(task_type, 0)
                handle_background_task_errors.failure_counts[task_type] = current_failures + 1
                ...
                if (enable_graceful_degradation and 
                    handle_background_task_errors.failure_counts[task_type] >= max_consecutive_failures):
                    ...
                    from morgan.utils.error_handling import ErrorContext, ErrorCategory, ErrorSeverity
                ...
                raise MorganError(
                    ...
                    component="background_processor",
                    operation=f"background_{task_type}",
                    severity=ErrorSeverity.HIGH if current_failures >= max_consecutive_failures else ErrorSeverity.MEDIUM,
                    metadata={
                        "task_type": task_type,
                        "consecutive_failures": current_failures + 1
                    },
...
def register_jina_recovery_procedures():
    ...
    def recover_jina_embedding_service(error: JinaEmbeddingError, context: Dict[str, Any]) -> bool:
    ...
    recovery_manager.register_procedure(RecoveryProcedure(
...
if __name__ == "__main__":
    ...
    service_manager = get_jina_service_manager()
    print(f"Service health: {service_manager.get_service_health(JinaServiceType.EMBEDDING)}")
