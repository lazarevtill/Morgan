...
class GracefulDegradationManager:
    ...
    def force_degradation_level(self, level: DegradationLevel, reason: str = "manual"):
        ...
        with self._lock:
            ...
            logger.info(
                f"Forced degradation: {old_level.value} -> {level.value} (reason: {reason})"
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_events": (
                self.degradation_history[-10:] if self.degradation_history else []
            ),
        }
...
class ErrorRecoveryManager:
    ...
    def attempt_recovery(
        self, error: MorganError, context: Optional[Dict[str, Any]] = None
    ) -> bool:
        """
    ...
    def _find_applicable_procedures(
        self, error: MorganError
        ...
        for procedure in self.procedures.values():
            if any(
                ...
                for error_type in procedure.applicable_errors
            ):
    ...
    def _execute_recovery_procedure(
        ...
        try:
            ...
            logger.error(
                f"Recovery procedure {procedure.name} timed out after {procedure.timeout}s"
            )
            return False
        except Exception as e:
    ...
    def _register_default_procedures(self):
        ...
        def recover_embedding_service(
            error: EmbeddingError, context: Dict[str, Any]
        ) -> bool:
        ...
        def recover_companion_features(
            error: CompanionError, context: Dict[str, Any]
        ) -> bool:
            ...
            try:
                ...
                degradation_manager.force_degradation_level(
                    DegradationLevel.MINIMAL, "companion_error_recovery"
                )
                return True
            except Exception:
                return False
        ...
        def recover_memory_processing(
            error: MemoryProcessingError, context: Dict[str, Any]
        ) -> bool:
            """Recover memory processing by skipping current operation."""
            try:
                # Log the memory processing failure and continue
                logger.warning(
                    f"Skipping memory processing due to error: {error.error_id}"
                )
                return True
            except Exception:
                return False
    ...
    def get_recovery_stats(self) -> Dict[str, Any]:
        ...
        successful_attempts = sum(
            1 for record in self.recovery_history if record["success"]
        )
...
class SystemHealthMonitor:
    ...
    def register_health_check(
        self, component: str, check_function: Callable[[], bool], timeout: float = 10.0
    ):
