...
class JinaCodeEmbeddings:
    ...
    def encode(
        ...
        try:
            ...
            if languages is None:
                languages = [
                    self._detect_language(snippet) for snippet in code_snippets
                ]

            processing_time = time.time() - start_time

            logger.debug(
                f"Encoded {len(code_snippets)} code snippets in {processing_time:.3f}s"
            )
    ...
    def search_code_semantically(
        self, query: str, code_collection: List[str], top_k: int = 10
        ...
        for idx in top_indices:
            ...
            function_name = (
                structure.functions[0]["name"] if structure.functions else None
            )
        ...
        if language == "python":
            for i, line in enumerate(lines):
                if line.strip().startswith("def "):
                    ...
                    functions.append(
                        {"name": func_name, "line": i + 1, "signature": line.strip()}
                    )
        ...
        if language == "python":
            for i, line in enumerate(lines):
                if line.strip().startswith("class "):
                    ...
                    classes.append(
                        {"name": class_name, "line": i + 1, "signature": line.strip()}
                    )
