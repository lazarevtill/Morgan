...
class ModelSelector:
    ...
    def select_embedding_model(self, content_type: str) -> str:
        ...
        """
        model = self.EMBEDDING_MODELS.get(content_type, self.EMBEDDING_MODELS["default"])
        logger.debug(f"Selected embedding model '{model}' for content type '{content_type}'")
        return model
    ...
    def is_valid_language(self, language: str) -> bool:
        ...
        """
        return language in self.RERANKER_MODELS or language != "en"  # Non-English uses multilingual