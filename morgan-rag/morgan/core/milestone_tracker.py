...
from ..emotional.models import RelationshipMilestone, CompanionProfile, ConversationContext, EmotionalState
...
class MilestoneTracker:
    ...
    def generate_celebration_message(self, milestone: RelationshipMilestone) -> str:
        ...
        celebrations = {
            MilestoneType.FIRST_CONVERSATION: 
                "Welcome! I'm excited to start this journey of learning and discovery with you.",
            
            MilestoneType.BREAKTHROUGH_MOMENT: 
                "What a wonderful breakthrough! I'm so glad I could help you reach this understanding.",
            
            MilestoneType.GOAL_ACHIEVED: 
                "Congratulations on achieving your goal! Your dedication and hard work have paid off.",
            
            MilestoneType.LEARNING_MILESTONE: 
                "It's amazing to see how much you've learned! Your curiosity and persistence inspire me.",
            
            MilestoneType.EMOTIONAL_SUPPORT: 
                "I'm honored that you trust me to support you through this. You're stronger than you know.",
            
            MilestoneType.TRUST_BUILDING: 
                "Thank you for sharing something so personal with me. Our growing trust means a lot.",
            
            MilestoneType.REGULAR_USER: 
                "I've really enjoyed our conversations! You've become such an important part of my day.",
            
            MilestoneType.DEEP_CONVERSATION: 
                "This has been such a meaningful conversation. I love how deeply we can explore ideas together."
        }
    ...
    def get_milestone_statistics(self, user_profile: CompanionProfile) -> Dict[str, Any]:
        ...
        for milestone in user_profile.relationship_milestones:
            ...
            milestone_counts[milestone_type] = milestone_counts.get(milestone_type, 0) + 1
    ...
    def _should_trigger_milestone(
        ...
        existing_types = [m.milestone_type.value for m in user_profile.relationship_milestones]
        ...
        if milestone_type == MilestoneType.FIRST_CONVERSATION:
            ...
            return (
                emotional_state.primary_emotion.value == "joy" and
                emotional_state.intensity > 0.7 and
                len(conversation_context.message_text) > 100 and
                milestone_type.value not in existing_types[-3:]  # Not in last 3 milestones
            )
        
        elif milestone_type == MilestoneType.TRUST_BUILDING:
            trust_keywords = ['personal', 'share', 'trust', 'private', 'secret', 'feel', 'emotion']
            message_lower = conversation_context.message_text.lower()
            return (
                any(word in message_lower for word in trust_keywords) and
                len(conversation_context.message_text) > 50 and
                milestone_type.value not in existing_types[-2:]  # Not in last 2 milestones
            )
        
        elif milestone_type == MilestoneType.REGULAR_USER:
            return (
                user_profile.interaction_count >= 10 and
                milestone_type.value not in existing_types
            )
        
        elif milestone_type == MilestoneType.DEEP_CONVERSATION:
            return (
                len(conversation_context.message_text) > 200 and
                user_profile.interaction_count >= 5 and
                milestone_type.value not in existing_types[-5:]  # Not in last 5 milestones
            )
        
        elif milestone_type == MilestoneType.LEARNING_MILESTONE:
            learning_keywords = ['learn', 'understand', 'got it', 'makes sense', 'clear now', 'thank you']
            message_lower = conversation_context.message_text.lower()
            return (
                any(word in message_lower for word in learning_keywords) and
                user_profile.interaction_count >= 3 and
                milestone_type.value not in existing_types[-3:]
            )
    ...
    def _create_milestone(
        ...
        logger.info(f"Created milestone: {milestone_type.value} for user {user_profile.user_id}")
        return milestone
    ...
    def _generate_milestone_description(
        ...
        descriptions = {
            MilestoneType.FIRST_CONVERSATION: 
                f"First conversation with {user_profile.preferred_name}",
            
            MilestoneType.BREAKTHROUGH_MOMENT: 
                f"Breakthrough moment in conversation about: {conversation_context.message_text[:50]}...",
            
            MilestoneType.TRUST_BUILDING: 
                f"{user_profile.preferred_name} shared something personal, building trust",
            
            MilestoneType.GOAL_ACHIEVED: 
                f"{user_profile.preferred_name} achieved an important goal",
            
            MilestoneType.LEARNING_MILESTONE: 
                f"{user_profile.preferred_name} had a significant learning moment",
            
            MilestoneType.EMOTIONAL_SUPPORT: 
                f"Provided emotional support to {user_profile.preferred_name}",
            
            MilestoneType.REGULAR_USER: 
                f"{user_profile.preferred_name} became a regular user (10+ conversations)",
            
            MilestoneType.DEEP_CONVERSATION: 
                f"Deep, meaningful conversation with {user_profile.preferred_name}"
        }
        
        return descriptions.get(
            milestone_type,
            f"Milestone reached with {user_profile.preferred_name}"
        )
