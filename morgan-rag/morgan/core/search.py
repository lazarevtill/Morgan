...
from morgan.utils.error_handling import (
    SearchError, EmbeddingError, StorageError, ErrorCategory, ErrorSeverity
)
from morgan.utils.error_decorators import (
    handle_search_errors, handle_embedding_errors, monitor_performance
)
...
class SmartSearch:
    ...
    def __init__(self):
        ...
        # Search settings
        self.default_max_results = getattr(self.settings, 'morgan_default_search_results', 10)
        ...
        try:
            ...
            if use_enhanced_search and (emotional_context or user_id):
                try:
                    ...
                    from morgan.search.multi_stage_search import SearchStrategy as MultiStageStrategy
                    
                    strategy_mapping = {
                        SearchStrategy.KNOWLEDGE: [MultiStageStrategy.SEMANTIC],
                        SearchStrategy.MEMORY: [MultiStageStrategy.MEMORY],
                        SearchStrategy.HYBRID: [MultiStageStrategy.SEMANTIC, MultiStageStrategy.MEMORY, MultiStageStrategy.CONTEXTUAL],
                        SearchStrategy.SEMANTIC: [MultiStageStrategy.SEMANTIC]
                    }
                    
                    strategies = strategy_mapping.get(strategy, [MultiStageStrategy.SEMANTIC, MultiStageStrategy.MEMORY])
                    ...
                    logger.debug(f"Enhanced multi-stage search found {len(results)} results")
                    return results
        ...
        try:
            ...
            if use_async_processing:
                ...
                if result and result.success:
                    ...
                    logger.warning(f"Async conversation search failed or timed out, falling back to sync")
            ...
            try:
                ...
                if len(results) < max_results:
                    try:
                        ...
                        companion_results = companion_engine.search_with_emotional_context(
                            query=query,
                            user_id=user_id,
                ...
                logger.warning(f"Enhanced conversation search failed, falling back to basic search: {e}")
    ...
    def find_similar_questions(
        ...
        try:
            ...
            for result in results:
                ...
                if any(word in content_lower for word in ['how', 'what', 'why', 'when', 'where', 'which', '?']):
        ...
        try:
            ...
            for result in results:
                ...
                for sentence in sentences:
                    ...
                    if partial_query.lower() in sentence.lower() and len(sentence) > len(partial_query):
    ...
    def _search_conversations_with_emotion(
        ...
        try:
            ...
            if emotional_context and hasattr(self.vector_db, 'search_with_emotional_context'):
            ...
            if user_pattern and emotional_context:
                ...
                for result in results:
                    try:
                        ...
                        from morgan.optimization.emotional_optimizer import EmotionalState
                        ...
                        empathy_boost = optimization_params.get("empathy_level", 0.5) * 0.1
                        result.score = min(1.0, result.score + empathy_boost)
                        
                        # Add optimization metadata
                        result.metadata.update({
                            "empathy_level": optimization_params.get("empathy_level"),
                            "emotional_tone": optimization_params.get("emotional_tone"),
                            "personalization_factors": optimization_params.get("personalization_factors", [])
                        })
                        
                    except Exception as e:
                        logger.warning(f"Failed to apply emotional optimization to result: {e}")
    ...
    def _enhance_results(self, results: List[SearchResult], query: str) -> List[SearchResult]:
        """
        ...
        try:
            ...
            for result in results:
                ...
                enhanced_source = self._enhance_source_description(result.source, result.result_type)
