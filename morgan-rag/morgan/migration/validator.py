...
class MigrationValidator:
        ...
        try:
            ...
            if not self.vector_db.collection_exists(source_collection):
                validation_errors.append(
                    f"Source collection '{source_collection}' does not exist"
                )
            ...
            if sample_size > 0:
                ...
                source_sample = self.vector_db.scroll_points(
                    collection_name=source_collection, limit=sample_size
                )
                ...
                for source_point in source_sample:
                    ...
                    if target_point is None:
                        validation_errors.append(
                            f"Point {point_id} missing in target collection"
                        )
                        points_invalid += 1
                        continue

                    # Validate point structure and content
                    point_validation = self._validate_point_pair(
            ...
            performance_metrics = {
                ...
                "migration_efficiency": (
                    target_count / source_count if source_count > 0 else 0
                ),
                "validation_coverage": (
                    sample_size / source_count if source_count > 0 else 0
                ),
    ...
    def _validate_point_pair(
        self, source_point: Dict[str, Any], target_point: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if source_point.get("id") != target_point.get("id"):
                errors.append(
                    f"ID mismatch: {source_point.get('id')} != {target_point.get('id')}"
                )
            ...
            if not isinstance(source_vector, list):
                errors.append(
                    f"Source vector should be list, got {type(source_vector)}"
                )

            if not isinstance(target_vector, dict):
                errors.append(
                    f"Target vector should be dict, got {type(target_vector)}"
                )
            else:
            ...
            return {"is_valid": False, "errors": [f"Point validation error: {str(e)}"]}
    ...
    def validate_backup(self, backup_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            missing_fields = [
                field for field in required_fields if field not in backup_data
            ]
            ...
            return {
                ...
                "sample_validation": {
                    ...
                    "validation_rate": (
                        valid_points / sample_size if sample_size > 0 else 0
                    ),
                },
            }

        except Exception as e:
            return {"is_valid": False, "error": f"Backup validation failed: {str(e)}"}
    ...
    def compare_collections(
        self, collection1: str, collection2: str, sample_size: int = 50
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "comparison": {
                    ...
                    "similarity_ratio": (
                        len(common_ids) / max(len(ids1), len(ids2))
                        if max(len(ids1), len(ids2)) > 0
                        else 0
                    ),
                },
            }
