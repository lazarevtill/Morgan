...
class RollbackManager:
                ...
                try:
                    ...
                    with open(backup_file, "r", encoding="utf-8") as f:
                        ...
                        try:
                            ...
                            collection_name = backup_data.get(
                                "collection_name", "unknown"
                            )
                            backup_timestamp = backup_data.get(
    ...
    def validate_backup_for_rollback(self, backup_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "is_valid": validation_rate
                >= 0.8,  # At least 80% of sample should be valid
                "collection_name": collection_name,
                "total_points": total_points,
    ...
    def execute_rollback(
        ...
        try:
            ...
            if not validation["is_valid"]:
                raise Exception(
                    f"Backup validation failed: {validation.get('error', 'Unknown error')}"
                )
            ...
            logger.info(
                f"Starting rollback to collection '{collection_name}' from {backup_path}"
            )
            ...
            if isinstance(sample_vector, dict):
                ...
                if not success:
                    raise Exception(
                        f"Failed to create hierarchical collection '{collection_name}'"
                    )
                ...
                vector_size = (
                    len(sample_vector) if isinstance(sample_vector, list) else 1536
                )

                self.vector_db.create_collection(
                    name=collection_name, vector_size=vector_size, distance="cosine"
                )
            ...
            logger.info(
                f"Rollback completed: {points_restored} points restored in {execution_time:.1f}s"
            )
            ...
            logger.error(
                f"Rollback failed after {execution_time:.1f}s: {error_message}"
            )
    ...
    def _restore_hierarchical_points(
        ...
        for i in range(0, len(points), batch_size):
            ...
            for point in batch:
                hierarchical_point = {
                    ...
                    "vector": point[
                        "vector"
                    ],  # Should already be dict with named vectors
                    "payload": {
                        **point["payload"],
            ...
            success = self.vector_db.upsert_hierarchical_points(
                collection_name, hierarchical_points
            )
            if not success:
    ...
    def cleanup_old_backups(self, keep_days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"files_deleted": 0, "space_freed_mb": 0, "error": str(e)}
