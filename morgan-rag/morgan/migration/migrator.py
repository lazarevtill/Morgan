...
class KnowledgeBaseMigrator:
    ...
    def create_migration_plan(
        ...
        backup_path = str(
            self.backup_dir / f"{source_collection}_backup_{timestamp}.json"
        )
    ...
    def execute_migration(self, plan: MigrationPlan) -> MigrationResult:
        ...
        try:
            logger.info(
                f"Starting migration: {plan.source_collection} -> {plan.target_collection}"
            )
            ...
            if not plan.dry_run:
                backup_created = self._create_backup(
                    plan.source_collection, plan.backup_path
                )
            ...
            if not plan.dry_run:
                ...
                if not success:
                    raise Exception(
                        f"Failed to create target collection '{plan.target_collection}'"
                    )
            ...
            while offset < plan.total_points:
                ...
                batch_result = self._migrate_batch(
                    batch_points, plan.target_collection, dry_run=plan.dry_run
                )
                ...
                logger.info(
                    f"Migration progress: {progress:.1f}% ({offset}/{plan.total_points})"
                )
            ...
            logger.error(
                f"Migration failed after {execution_time:.1f}s: {error_message}"
            )
    ...
    def _create_backup(self, collection_name: str, backup_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Backup created: {len(all_points)} points saved to {backup_path}"
            )
            return True
        ...
        try:
            ...
            for point in batch_points:
                try:
                    ...
                    if not content:
                        logger.warning(
                            f"Point {point.get('id')} has no content, skipping"
                        )
                        failed += 1
                        continue
                    ...
                    if not dry_run:
                        hierarchical_embedding = (
                            self.hierarchical_service.create_hierarchical_embeddings(
                                content=content,
                        ...
                        hierarchical_point = {
                            "id": point.get("id"),
                            "vector": {
                                "coarse": hierarchical_embedding.get_embedding(
                                    "coarse"
                                ),
                                "medium": hierarchical_embedding.get_embedding(
                                    "medium"
                                ),
            ...
            if hierarchical_points and not dry_run:
                success = self.vector_db.upsert_hierarchical_points(
                    target_collection, hierarchical_points
                )
    ...
    def estimate_migration_resources(self, collection_name: str) -> Dict[str, Any]:
        ...
        estimated_memory_mb = (total_points * 2048 * 3) / (
            1024 * 1024
        )  # 3 embeddings per point
