...
class GitHashTracker:
    ...
    def calculate_git_hash(self, source_path: str) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.warning(
                    f"Failed to get Git hash for {source_path}: {result.stderr}"
                )
                self._increment_metric("hash_calculations")
                return self._calculate_file_hash(source_path)
    ...
    def store_git_hash(
        ...
        try:
            ...
            logger.debug(
                f"Stored Git hash for collection {collection_name}: {git_hash}"
            )
            return True
    ...
    def _store_collection_info(
        ...
        try:
            ...
            collections[collection_name] = {
                ...
                "git_hash": git_hash,
                "document_count": document_count,
                "created_at": collections.get(collection_name, {}).get(
                    "created_at", now
                ),
                "last_accessed": now,
                "size_bytes": size_bytes,
            ...
            return {
                ...
                "collection_stats": {
                    ...
                    "average_documents_per_collection": (
                        total_documents / total_collections
        ...
        try:
            ...
            if hit_rate < 0.7:
                ...
                recommendations.append(
                    "Review document change patterns to optimize caching strategy"
                )

            if cache_perf["hash_calculations"] > total_requests * 2:
                recommendations.append(
                    "High hash calculation overhead detected - consider optimizing Git operations"
                )

            if collection_stats["total_collections"] > 100:
                recommendations.append(
                    "Large number of collections - consider cleanup of unused collections"
                )
