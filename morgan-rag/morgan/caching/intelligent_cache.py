...
from .cache_models import CacheMetrics, CacheStatus, CollectionCacheInfo
...
class IntelligentCacheManager:
    ...
    def check_cache_validity(
        self, source_path: str, collection_name: str
    ) -> CacheStatus:
        """
        ...
        try:
            ...
            # Update metrics
            self._update_metrics(cache_hit=cache_status.is_valid, hash_calculated=True)

            processing_time = time.time() - start_time
            logger.debug(
                "Cache validity check for %s: valid=%s, time=%.3fs",
                collection_name,
                cache_status.is_valid,
                processing_time,
            )
            ...
            logger.error("Error checking cache validity for %s: %s", collection_name, e)
    ...
    def get_cached_collection(self, collection_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Error getting cached collection %s: %s", collection_name, e)
            return None
    ...
    def store_collection(
        ...
        try:
            ...
            if git_hash is None:
                ...
                if not git_hash:
                    logger.error("Failed to calculate Git hash for %s", source_path)
                    return False
    ...
    def invalidate_cache(self, collection_name: str) -> bool:
        ...
        try:
            ...
            logger.error("Error invalidating cache for %s: %s", collection_name, e)
            return False
    ...
    def get_cache_speedup(
        ...
        try:
            ...
            speedup = base_speedup + (max_speedup - base_speedup) * max(
                doc_factor, size_factor
            )
    ...
    def get_incremental_changes(
        ...
        try:
            ...
            logger.error(
                "Error getting incremental changes for %s: %s", collection_name, e
            )
            return []
            ...
            for collection_path in self.collections_dir.glob("*.json"):
                try:
                    ...
                    if last_accessed < cutoff_date:
                        ...
                        cleaned_count += 1
                        logger.debug("Cleaned up expired cache for %s", collection_name)
    ...
    def optimize_cache_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_size_cleaned = sum(
                c["size_mb"]
                for c in large_unused_collections
        ...
        try:
            ...
            stats["collections"] = {
                ...
                "avg_documents_per_collection": (
                    round(total_documents / len(collections), 1) if collections else 0
                ),
            }
    ...
    def _generate_performance_insights(
        self, collections: List[CollectionCacheInfo]
    ) -> Dict[str, Any]:
        """
        ...
        recent_access_count = sum(
            1 for c in collections if (now - c.last_accessed).days <= 7
        )
    ...
    def monitor_cache_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if collections:
                now = datetime.now()
                stale_count = sum(
                    1 for c in collections if (now - c.last_accessed).days > 30
                )
            ...
            if score >= 80:
                ...
                health_status["recommendations"].extend(
                    ["Immediate cache optimization needed", "Review caching strategy"]
                )
