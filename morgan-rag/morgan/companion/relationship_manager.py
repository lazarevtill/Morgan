...
from ..emotional.models import (
    CompanionProfile, UserPreferences, RelationshipMilestone, 
    EmotionalState, ConversationContext, PersonalizedGreeting,
    ConversationTopic, ConversationStyle, MilestoneType,
    CommunicationStyle, ResponseLength, InteractionData
)
...
class CompanionRelationshipManager:
    ...
    def build_user_profile(
        ...
        if existing_profile:
            ...
            first_interaction = (
                ...
                if interactions else datetime.utcnow()
            )
            last_interaction = (
    ...
    def adapt_conversation_style(
        ...
        """
        logger.debug(
            f"Adapting conversation style for user {user_profile.user_id}"
        )
        ...
        empathy_emphasis = min(
            empathy_base + user_profile.trust_level * 0.3, 1.0
        )
    ...
    def generate_personalized_greeting(
        ...
        if user_profile.preferred_name and user_profile.preferred_name != user_profile.user_id:
    ...
    def _update_profile_from_interactions(
        ...
        if profile.interaction_count > 0:
            ...
            if satisfaction_scores and sum(satisfaction_scores) / len(satisfaction_scores) > 0.7:
                trust_boost *= 1.5
    ...
    def _generate_milestone_details(
        ...
        descriptions = {
            MilestoneType.FIRST_CONVERSATION: (
                "Your first conversation with Morgan", 0.8
            ),
            MilestoneType.BREAKTHROUGH_MOMENT: (
                "A breakthrough moment in our conversation", 0.9
            ),
            MilestoneType.GOAL_ACHIEVED: (
                "Achieving an important goal together", 0.9
            ),
            MilestoneType.LEARNING_MILESTONE: (
                "A significant learning achievement", 0.7
            ),
            MilestoneType.EMOTIONAL_SUPPORT: (
                "Providing meaningful emotional support", 0.8
            ),
            MilestoneType.TRUST_BUILDING: (
                "Building deeper trust in our relationship", 0.8
            )
        }
    ...
    def _update_profile_from_milestone(
        ...
        profile.engagement_score = min(
            profile.engagement_score + engagement_boost, 1.0
        )