...
class CompanionStorage:
    ...
    def store_companion_profile(self, profile: CompanionProfile) -> bool:
        ...
        try:
            ...
            return self.vector_client.upsert_points(
                collection_name=self.COMPANIONS_COLLECTION, points=[point_data]
            )

        except Exception as e:
            logger.error(
                f"Failed to store companion profile for {profile.user_id}: {e}"
            )
            return False
        ...
        try:
            ...
            point_data = {
                ...
                "payload": {
                    ...
                    "secondary_emotions": [
                        e.value for e in emotional_state.secondary_emotions
                    ],
            ...
            return self.vector_client.upsert_points(
                collection_name=self.EMOTIONS_COLLECTION, points=[point_data]
            )
    ...
    def store_relationship_milestone(
        self, user_id: str, milestone: RelationshipMilestone
    ) -> bool:
        """
        ...
        try:
            ...
            milestone_embedding = self.embedding_service.embed_text(
            ...
            return self.vector_client.upsert_points(
                collection_name=self.MILESTONES_COLLECTION, points=[point_data]
            )
            ...
            for result in results:
                ...
                if timestamp >= cutoff_date:
                    emotional_states.append(
                        self._build_emotional_state_from_payload(payload)
                    )
    ...
    def search_similar_emotional_states(
        self, emotional_state: EmotionalState, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            return [
                {
                    "emotional_state": self._build_emotional_state_from_payload(
                        result.payload
                    ),
    ...
    def _build_companion_profile_from_payload(
        ...
        communication_preferences = UserPreferences(
            ...
            communication_style=CommunicationStyle(
                payload.get("communication_style", "friendly")
            ),
            preferred_response_length=ResponseLength(
                payload.get("preferred_response_length", "detailed")
            ),
    ...
    def _build_emotional_state_from_payload(
        self, payload: Dict[str, Any]
    ) -> EmotionalState:
        ...
        return EmotionalState(
            ...
            secondary_emotions=[
                EmotionType(e) for e in payload.get("secondary_emotions", [])
            ],
            emotional_indicators=payload.get("emotional_indicators", []),
            timestamp=datetime.fromisoformat(payload["timestamp"]),
        )

    def _build_milestone_from_payload(
        self, payload: Dict[str, Any]
    ) -> RelationshipMilestone:
    ...
    def delete_user_data(self, user_id: str) -> bool:
        ...
        try:
            ...
            success &= self.vector_client.delete_points(
                collection_name=self.COMPANIONS_COLLECTION, point_ids=[user_id]
            )
            ...
            if emotion_results:
                ...
                success &= self.vector_client.delete_points(
                    collection_name=self.EMOTIONS_COLLECTION, point_ids=emotion_ids
                )
            ...
            if milestone_results:
                ...
                success &= self.vector_client.delete_points(
                    collection_name=self.MILESTONES_COLLECTION, point_ids=milestone_ids
                )
