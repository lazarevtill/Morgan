...
class BackgroundProcessingService:
    ...
    def _worker_loop(self):
        ...
        while self.running:
            try:
                ...
                self.logger.error(
                    f"Error in background worker loop: {e}", exc_info=True
                )
    ...
    def _should_warm_cache(self) -> bool:
        ...
        if not hasattr(self, "_last_cache_warm"):
            self._last_cache_warm = datetime.now() - timedelta(
                hours=2
            )  # Force first run
    ...
    def _perform_cache_warming(self):
        ...
        try:
            ...
            self.logger.info(
                f"Cache warming completed: {total_precomputed} queries precomputed"
            )
