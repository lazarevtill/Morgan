...
from morgan.background.domain.entities import TaskStatus
...
class BackgroundTaskExecutor(TaskExecutor):
    ...
    def execute_task_now(
        self, task_type: str, collection_name: str, force: bool = False
    ) -> TaskExecution:
        """
    ...
    def _execute_task(self, task: ScheduledTask, force: bool = False) -> TaskExecution:
        ...
        try:
            ...
            self.logger.info(
                f"Starting task {task.task_id} ({task.task_type} on {task.collection_name})"
            )
            ...
            if task.task_type == "reindex":
                ...
                result = self.reranking_task.rerank_popular_queries(
            ...
            if hasattr(result, "success") and result.success:
                ...
                execution.error_message = getattr(
                    result, "error_message", "Unknown error"
                )
                self.logger.error(
                    f"Task {task.task_id} failed: {execution.error_message}"
                )
            ...
            self.logger.error(
                f"Task {task.task_id} failed with exception: {e}", exc_info=True
            )
    ...
    def get_execution_stats(self) -> Dict[str, Any]:
        ...
        improvements = [
            m.improvement for m in self.quality_history if m.improvement > 0
        ]
