...
class PrecomputedSearchCache:
    ...
    def track_query(
        ...
        if query_hash in self.query_analytics:
            ...
            analytics.average_response_time = (total_time + response_time) / analytics.access_count
        else:
    ...
    def precompute_query_results(
        ...
        try:
            self.logger.info(f"Precomputing results for query: '{query}' in {collection_name}")
            ...
            if self.reranking_service:
                reranked_results, rerank_scores = self._apply_reranking(query, search_results)
    ...
    def warm_cache(
        ...
        for analytics in popular_queries:
            try:
                ...
                self.logger.warning(f"Failed to precompute '{analytics.query_text}': {e}")
                continue
        
        self.logger.info(f"Cache warming completed: {precomputed_count} queries precomputed")
    ...
    def invalidate_cache(
        ...
        for cache_key, cached_result in self.precomputed_cache.items():
            ...
            if query_pattern and query_pattern.lower() not in cached_result.query_text.lower():
                should_remove = False
    ...
    def _assess_result_quality(
        ...
        avg_rerank_score = sum(rerank_scores) / len(rerank_scores) if rerank_scores else avg_search_score
