...
class UserPreferenceProfile:
    ...
    preference_sources: Dict[
        str, Dict[str, PreferenceSource]
    ]  # category -> {key: source}
    ...
    def get_preference(
        self, category: PreferenceCategory, key: str, default: Any = None
    ) -> Any:
        """Get a specific preference value."""
        cat_str = category.value
        return self.preferences.get(cat_str, {}).get(key, default)

    def get_confidence(self, category: PreferenceCategory, key: str) -> float:
...
class PreferenceExtractor:
    ...
    COMMUNICATION_PATTERNS = {
        ...
        "casual_language": [r"\b(hey|hi|yeah|yep|cool|awesome)\b", r"[!]{2,}|[?]{2,}"],
    ...
    def extract_preferences(
        ...
        """
        logger.info(
            f"Extracting preferences for user {user_id} from {len(interactions)} interactions"
        )
...
{{
    ...
    def _extract_communication_preferences(
        ...
        formal_score = self._calculate_pattern_score(
            messages, self.COMMUNICATION_PATTERNS["formal_language"]
        )
        casual_score = self._calculate_pattern_score(
            messages, self.COMMUNICATION_PATTERNS["casual_language"]
        )
        ...
        if formal_score > casual_score and formal_score > 0.3:
            updates.append(
                PreferenceUpdate(
                    ...
                    source=PreferenceSource.CONVERSATION_ANALYSIS,
                    evidence=[
                        f"Formal language patterns detected in {len(messages)} messages"
                    ],
                )
            )
            ...
            updates.append(
                PreferenceUpdate(
                    ...
                    source=PreferenceSource.CONVERSATION_ANALYSIS,
                    evidence=[
                        f"Casual language patterns detected in {len(messages)} messages"
                    ],
                )
            )

        # Analyze technical preference
        technical_score = self._calculate_pattern_score(
            messages, self.COMMUNICATION_PATTERNS["technical_preference"]
        )
        simple_score = self._calculate_pattern_score(
            messages, self.COMMUNICATION_PATTERNS["simple_preference"]
        )
        ...
        if technical_score > simple_score and technical_score > 0.2:
            updates.append(
                PreferenceUpdate(
                    ...
                    source=PreferenceSource.CONVERSATION_ANALYSIS,
                    evidence=["Technical language preference detected"],
                )
            )
            ...
            updates.append(
                PreferenceUpdate(
                    ...
                    source=PreferenceSource.CONVERSATION_ANALYSIS,
                    evidence=["Simple language preference detected"],
                )
            )
    ...
    def _extract_content_preferences(
        ...
        brief_score = self._calculate_pattern_score(
            messages, self.CONTENT_PATTERNS["brief_responses"]
        )
        detailed_score = self._calculate_pattern_score(
            messages, self.CONTENT_PATTERNS["detailed_responses"]
        )
        ...
        if brief_score > detailed_score and brief_score > 0.2:
            updates.append(
                PreferenceUpdate(
                    ...
                    source=PreferenceSource.CONVERSATION_ANALYSIS,
                    evidence=["Brief response preference detected"],
                )
            )
            ...
            updates.append(
                PreferenceUpdate(
                    ...
                    source=PreferenceSource.CONVERSATION_ANALYSIS,
                    evidence=["Detailed response preference detected"],
                )
            )

        # Analyze example preference
        example_score = self._calculate_pattern_score(
            messages, self.CONTENT_PATTERNS["examples_preferred"]
        )
        if example_score > 0.2:
            updates.append(
                PreferenceUpdate(
                    ...
                    source=PreferenceSource.CONVERSATION_ANALYSIS,
                    evidence=["Examples preference detected in conversations"],
                )
            )
    ...
    def _extract_topic_preferences(
        ...
        if messages:
            learning_score = self._calculate_pattern_score(
                messages, self.TOPIC_PATTERNS["learning_topics"]
            )
            work_score = self._calculate_pattern_score(
                messages, self.TOPIC_PATTERNS["work_topics"]
            )
            personal_score = self._calculate_pattern_score(
                messages, self.TOPIC_PATTERNS["personal_topics"]
            )
            ...
            if learning_score > 0.2:
                updates.append(
                    PreferenceUpdate(
                        ...
                        source=PreferenceSource.CONVERSATION_ANALYSIS,
                        evidence=["Learning-focused conversations detected"],
                    )
                )
            ...
            if work_score > 0.2:
                updates.append(
                    PreferenceUpdate(
                        ...
                        source=PreferenceSource.CONVERSATION_ANALYSIS,
                        evidence=["Work-focused conversations detected"],
                    )
                )
            ...
            if personal_score > 0.2:
                updates.append(
                    PreferenceUpdate(
                        ...
                        source=PreferenceSource.CONVERSATION_ANALYSIS,
                        evidence=["Personal conversations detected"],
                    )
                )
                ...
                if confidence > 0.1:
                    updates.append(
                        PreferenceUpdate(
                            ...
                            source=PreferenceSource.IMPLICIT_BEHAVIOR,
                            evidence=[
                                f"Learning indicator '{indicator}' observed {count} times"
                            ],
                        )
                    )
        ...
        if feedback_frequency > 0.1:  # User provides feedback regularly
            updates.append(
                PreferenceUpdate(
                    ...
                    source=PreferenceSource.IMPLICIT_BEHAVIOR,
                    evidence=[
                        f"User provides feedback {feedback_frequency:.1%} of the time"
                    ],
                )
            )
    ...
    def _extract_emotional_preferences(
        ...
        if emotional_states:
            ...
            emotion_counter = Counter(
                state.primary_emotion for state in emotional_states
            )

            # Analyze emotional intensity preferences
            avg_intensity = sum(state.intensity for state in emotional_states) / len(
                emotional_states
            )
                ...
                if confidence > 0.2:
                    updates.append(
                        PreferenceUpdate(
                            ...
                            source=PreferenceSource.IMPLICIT_BEHAVIOR,
                            evidence=[
                                f"Emotion '{emotion.value}' observed {count} times"
                            ],
                        )
                    )
    ...
    def _calculate_pattern_score(
        self, messages: List[str], patterns: List[str]
    ) -> float:
...
class PreferenceStorage:
    ...
    def __init__(self):
        ...
        self.storage_path = Path(self.settings.morgan_data_dir) / "preferences"
    ...
    def get_user_preferences(self, user_id: str) -> UserPreferenceProfile:
        ...
        profile = UserPreferenceProfile(
            user_id=user_id, preferences={}, confidence_scores={}, preference_sources={}
        )
