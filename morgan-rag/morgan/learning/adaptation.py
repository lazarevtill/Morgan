...
from ..emotional.models import (
    CompanionProfile, ConversationContext, CommunicationStyle,
    ResponseLength, EmotionType
)
from .preferences import UserPreferenceProfile, PreferenceCategory
...
class ResponseStyleAdapter:
    ...
    def _adapt_formality_level(
        ...
        if formality_pref:
            ...
            if formality_pref == "formal" and current_style != CommunicationStyle.FORMAL:
            ...
            elif formality_pref == "casual" and current_style != CommunicationStyle.CASUAL:
                adaptations.append(Adaptation(
                    adaptation_id=str(uuid.uuid4()),
                    adaptation_type=AdaptationType.RESPONSE_STYLE,
                    parameter="formality_level",
    ...
    def _adapt_response_length(
        ...
        if length_pref:
            ...
            current_length = user_profile.communication_preferences.preferred_response_length
            ...
            elif length_pref == "detailed" and current_length != ResponseLength.COMPREHENSIVE:
                adaptations.append(Adaptation(
                    adaptation_id=str(uuid.uuid4()),
                    adaptation_type=AdaptationType.RESPONSE_STYLE,
                    parameter="response_length",
...
class ContentSelectionAdapter:
    ...
    def _adapt_search_weighting(
        ...
        topic_prefs = preference_profile.preferences.get(PreferenceCategory.TOPICS.value, {})
        
        if topic_prefs:
            # Create search weight adaptations for preferred topics
            for pref_key, pref_value in topic_prefs.items():
                if pref_key.startswith("interest_") and isinstance(pref_value, (int, float)):
        ...
        if preference_profile.get_preference(PreferenceCategory.TOPICS, "learning_focus"):
            confidence = preference_profile.get_confidence(
...
class BehavioralAdaptationEngine:
    ...
    def adapt_behavior(
        ...
        if all_adaptations:
            overall_confidence = sum(a.confidence for a in all_adaptations) / len(all_adaptations)
        else:
    ...
    def _generate_adaptation_reasoning(self, adaptations: List[Adaptation]) -> str:
        ...
        adaptation_types = set(a.adaptation_type for a in adaptations)
        type_counts = {t: sum(1 for a in adaptations if a.adaptation_type == t) 
        ...
        return f"Applied {len(adaptations)} total adaptations: " + ", ".join(reasoning_parts)