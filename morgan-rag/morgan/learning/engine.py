...
from ..emotional.models import (
    InteractionData, CompanionProfile, ConversationContext
)
...
from .adaptation import BehavioralAdaptationEngine, AdaptationResult
from .feedback import FeedbackProcessor, UserFeedback, LearningUpdate
...
class LearningEngine:
    ...
    def learn_from_feedback(
        ...
        try:
            ...
            if learning_update.preference_updates:
                self._apply_preference_updates(user_id, learning_update.preference_updates)
            
            # Update adaptation strategies if needed
            if learning_update.adaptation_changes:
                self.adaptation_engine.update_strategies(
                    user_id=user_id,
                    changes=learning_update.adaptation_changes
                )
            ...
            logger.info(f"Applied {len(learning_update.preference_updates)} preference updates")
    ...
    def extract_preferences(
        ...
        try:
            ...
            logger.info(f"Updated {len(preference_updates)} preferences for user {user_id}")
    ...
    def _calculate_learning_score(self, session: LearningSession) -> float:
        ...
        pattern_score = min(session.patterns_identified / session.interactions_processed, 1.0)
        preference_score = min(session.preferences_updated / session.interactions_processed, 1.0)
        adaptation_score = min(session.adaptations_applied / session.interactions_processed, 1.0)
        feedback_score = min(session.feedback_processed / session.interactions_processed, 1.0)
