...
class AsyncProcessor:
    ...
    async def _execute_task(self, async_task: AsyncTask) -> TaskResult:
        ...
        try:
            ...
            logger.warning(f"Task {async_task.task_id} timed out after {execution_time:.1f}s")
            ...
            if async_task.retry_count < async_task.max_retries:
                ...
                try:
                    ...
                    logger.error(f"Failed to re-queue task {async_task.task_id} for retry")
        ...
        while not self.shutdown_event.wait(self.config.heartbeat_interval):
            try:
                ...
                if stats['queue_size'] > self.config.max_queue_size * 0.8:
                    logger.warning(f"Task queue is {stats['queue_size']}/{self.config.max_queue_size} (80%+ full)")
