...
class ConnectionPool:
    ...
    async def start(self):
        ...
        logger.info(
            f"ConnectionPool '{self.name}' started with {len(self.idle_connections)} connections"
        )
        ...
        with self.pool_lock:
            ...
            while self.idle_connections:
                ...
                if (
                    ...
                    and pooled_conn.age < self.config.max_connection_age
                    and pooled_conn.idle_time < self.config.idle_timeout
                ):
                    ...
                    logger.debug(
                        f"Reusing connection {pooled_conn.connection_id} from pool '{self.name}'"
                    )
                    return pooled_conn
                else:
            ...
            if (
                ...
                logger.debug(
                    f"Created new connection {pooled_conn.connection_id} for pool '{self.name}'"
                )
                return pooled_conn
    ...
    async def _release_connection(self, pooled_conn: PooledConnection):
        ...
        with self.pool_lock:
            ...
            if (
                ...
                logger.debug(
                    f"Returned connection {pooled_conn.connection_id} to pool '{self.name}'"
                )
            else:
                # Connection is unhealthy or too old, close it
                await self._close_connection(pooled_conn)
                logger.debug(
                    f"Closed unhealthy connection {pooled_conn.connection_id} from pool '{self.name}'"
                )
    ...
    async def _create_connection(self) -> PooledConnection:
        ...
        try:
            ...
            logger.debug(
                f"Created connection {pooled_conn.connection_id} in {creation_time:.3f}s"
            )
            return pooled_conn
    ...
    async def _perform_health_check(self):
        ...
        with self.pool_lock:
            ...
            if unhealthy_connections:
                logger.info(
                    f"Removed {len(unhealthy_connections)} unhealthy connections from pool '{self.name}'"
                )
    ...
    async def _check_connection_health(self, pooled_conn: PooledConnection) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Connection health check failed for {pooled_conn.connection_id}: {e}"
            )
            return False
    ...
    def _update_stats(self):
        ...
        if self.stats.total_connections > 0:
            self.stats.pool_efficiency = (
                self.stats.active_connections / self.stats.total_connections
            )
        else:
