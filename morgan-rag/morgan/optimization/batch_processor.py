...
class BatchProcessor:
    ...
    @handle_embedding_errors(
        "process_vector_operations_batch",
        "batch_processor",
        RetryConfig(max_attempts=2, base_delay=1.0),
    )
    ...
    def process_vector_operations_batch(
        ...
        try:
            ...
            batch_size = self._get_optimal_batch_size(
                f"vector_{operation_type}", len(operations)
            )
            ...
            for batch_idx, batch in enumerate(batches):
                try:
                    ...
                    processed_count += (
                        len(batch_results) if batch_results else len(batch)
                    )
                    batch_sizes_used.append(len(batch))

                    logger.debug(
                        f"Completed vector batch {batch_idx + 1}/{len(batches)}"
                    )
    ...
    def process_streaming_batch(
        ...
        """
        batch_size = self._get_optimal_batch_size(
            operation_name, estimated_total or 1000
        )
        current_batch = []
        total_processed = 0
        batch_count = 0
            ...
            for item in data_iterator:
                ...
                if len(current_batch) >= batch_size:
                    ...
                    try:
                        ...
                        processed_count = (
                            len(results) if results else len(current_batch)
                        )
            ...
            if throughput_key in self.metrics:
                ...
                if throughput_data:
                    metrics[op_type] = PerformanceMetrics(
                        ...
                        avg_batch_size=(
                            sum(batch_size_data) / len(batch_size_data)
                            if batch_size_data
                            else 0.0
                        ),
