...
class CompanionMemorySearchEngine:
    ...
    def search_with_emotional_context(
        ...
        try:
            logger.debug(
                f"Searching memories with emotional context for user {user_id}: '{query}'"
            )
            ...
            memory_results = [
                ...
                if r.result_type in ["memory", "enhanced_memory", "companion_memory"]
            ]
            ...
            if include_emotional_moments:
                ...
                final_results = [
                    ...
                    if r.memory_type != "emotional_moment"
                ]

            logger.debug(
                f"Found {len(final_results)} enhanced companion-aware memory results"
            )
    ...
    def get_relationship_memories(
        ...
        try:
            ...
            relationship_query = (
                "trust relationship milestone breakthrough support understanding"
            )
            ...
            relationship_results = [
                r
                for r in results
                if r.relationship_significance > 0.5
                or "trust" in r.personalization_factors
            ]
    ...
    def search_similar_conversations(
        ...
        try:
            ...
            conversation_results = [
                r for r in similar_results if r.memory_type == "conversation_turn"
            ]
    ...
    def _enhance_memory_result(
        ...
        try:
            ...
            if not emotional_context and result.result_type == "memory":
                ...
                emotional_context = self._extract_emotional_context_from_content(
                    result.content
                )
    ...
    def _calculate_relationship_significance(
        ...
        try:
            ...
            keyword_matches = sum(
                1 for keyword in relationship_keywords if keyword in content_lower
            )
    ...
    def _calculate_engagement_score(
        ...
        try:
            ...
            if "Q:" in result.content:
                question_part = (
                    result.content.split("Q:")[1].split("A:")[0]
                    if "A:" in result.content
                    else result.content
                )
    ...
    def _extract_personalization_factors(
        ...
        try:
            ...
            if any(
                word in content_lower for word in ["learn", "understand", "explain"]
            ):
    ...
    def _calculate_enhanced_score(
        ...
        try:
            ...
            if query_emotion.intensity > 0.5:
                query_emotions = [
                    query_emotion.primary_emotion.value
                ] + query_emotion.secondary_emotions
    ...
    def _apply_companion_ranking(
        self, results: List[CompanionMemorySearchResult], query_emotion: EmotionalState
    ) -> List[CompanionMemorySearchResult]:
    ...
    def _execute_enhanced_memory_search(
        ...
        try:
            ...
            personalized_query = self._build_personalized_query_enhanced(
                user_profile, query
            )
            ...
            if direct_results:
                all_results.extend(
                    direct_results[0]
                )  # First element is the results list
            if personalized_results:
                all_results.extend(
                    personalized_results[0]
                )  # First element is the results list
    ...
    def _enhance_memory_result_with_personalization(
        ...
        try:
            ...
            return self._enhance_memory_result(
                result, query_emotion, user_id, min_relationship_significance
            )
    ...
    def _apply_advanced_companion_ranking(
        ...
        try:
            ...
            for result in results:
                ...
                temporal_boost = self._calculate_temporal_relevance_boost(
                    result.timestamp
                )
        ...
        try:
            ...
            for result in results:
                ...
                if self._matches_user_preferences(enhanced_result, user_profile):
                    ...
                    enhanced_result.personalization_factors.append(
    ...
    def _build_personalized_query_enhanced(
        self, user_profile: Dict[str, Any], original_query: str
    ) -> str:
    ...
    def _search_by_emotional_context(
        self, query_emotion: EmotionalState, max_results: int, user_id: str
    ) -> List[SearchResult]:
        ...
        try:
            ...
            primary_emotion = (
                query_emotion.primary_emotion.value
                if hasattr(query_emotion.primary_emotion, "value")
                else str(query_emotion.primary_emotion)
            )
    ...
    def _calculate_personalization_boost(
        ...
        try:
            ...
            if comm_style == "technical" and any(
                word in content
                for word in ["code", "function", "api", "implementation"]
            ):
                boost += 0.06

            # Emotional preference boost
            emotional_prefs = user_profile.get("emotional_preferences", {})
            if emotional_prefs.get("supportive_tone") and any(
                word in content for word in ["help", "support", "guide"]
            ):
                boost += 0.04
    ...
    def _extract_user_preference_factors(
        self, result: SearchResult, user_profile: Dict[str, Any]
    ) -> List[str]:
    ...
    def _calculate_emotional_resonance_boost(
        self, query_emotion: EmotionalState, memory_emotional_context: Dict[str, Any]
    ) -> float:
        ...
        try:
            ...
            if query_primary in memory_emotions:
                ...
                for emotion in query_emotion.secondary_emotions
            ):
    ...
    def _calculate_user_personalization_boost(
        self, result: CompanionMemorySearchResult, user_profile: Dict[str, Any]
    ) -> float:
        ...
        try:
            ...
            if (
                interaction_patterns.get("prefers_detailed_explanations")
                and len(result.content) > 300
            ):
                boost += 0.05

            if (
                interaction_patterns.get("likes_code_examples")
    ...
    def _add_conversation_context(
        self, result: CompanionMemorySearchResult
    ...
    def _matches_user_preferences(
        ...
        try:
            ...
            comm_style = user_profile.get("communication_style", "")
            if comm_style == "technical" and any(
                word in content for word in ["code", "function", "api"]
            ):
                return True
