...
class IntensityMeasurer:
    ...
    def measure_intensity(
        ...
        result = {
            ...
            "adjustment_factors": {
                ...
                "text_modifiers": self._get_applied_modifiers(emotional_state.emotional_indicators),
        ...
        if calibrate:
            self._update_calibration_data(emotional_state.primary_emotion, final_intensity)
    ...
    def analyze_intensity_patterns(
        ...
        # Measure intensities
        intensities = [self.measure_intensity(e)["final_intensity"] for e in recent_emotions]
    ...
    def compare_intensity_periods(
        ...
        current_volatility = statistics.stdev(current_intensities) if len(current_intensities) > 1 else 0.0
        previous_volatility = statistics.stdev(previous_intensities) if len(previous_intensities) > 1 else 0.0
        ...
        return {
            ...
            "changes": {
                ...
                "intensity_change_percent": (intensity_change / previous_mean) * 100 if previous_mean > 0 else 0,
    ...
    def calibrate_intensity_scale(
        ...
        for emotion_type in EmotionType:
            emotion_intensities = [
                e.intensity for e in emotional_states
    ...
    def _apply_contextual_adjustments(
        ...
        length_adjustment = self.CONTEXTUAL_ADJUSTMENTS["conversation_length"][length_category]
    ...
    def _apply_calibration(
        ...
        if emotion_key in self.calibration_data and isinstance(self.calibration_data[emotion_key], (int, float)):
    ...
    def _get_contextual_factors(self, context: Optional[ConversationContext]) -> Dict[str, Any]:
    ...
    def _calculate_intensity_distribution(self, intensities: List[float]) -> Dict[str, float]:
    ...
    def _update_calibration_data(self, emotion_type: EmotionType, intensity: float):
        ...
        if len(self.calibration_data[emotion_key]) > 100:
            self.calibration_data[emotion_key] = self.calibration_data[emotion_key][-100:]
