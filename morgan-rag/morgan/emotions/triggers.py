...
class EmotionalTrigger:
    ...
    def get_trigger_strength(self) -> float:
        ...
        return (self.sensitivity_score * 0.4) + (self.confidence * 0.4) + (frequency_factor * 0.2)
...
class EmotionalTriggerDetector:
    ...
    def detect_triggers(
        ...
        if len(memories) < 5:
            logger.debug(f"Insufficient data for trigger detection: {len(memories)} memories")
            return []
        ...
        all_triggers = (
            keyword_triggers + topic_triggers + temporal_triggers +
            contextual_triggers + semantic_triggers + personal_triggers
        )
        ...
        logger.info(f"Detected {len(detected_triggers)} emotional triggers for user {user_id}")
    ...
    def get_user_triggers(
        ...
        if trigger_types:
            user_triggers = [
                t for t in user_triggers
                if t.trigger_type in trigger_types
            ]
        ...
        if min_strength > 0.0:
            user_triggers = [
                t for t in user_triggers
                if t.get_trigger_strength() >= min_strength
            ]
        ...
        for trigger in user_triggers:
            ...
            if activation_score > 0.5:
                activated_triggers.append({
                    ...
                    'predicted_response': trigger.emotional_responses[0] if trigger.emotional_responses else EmotionType.NEUTRAL,
                    'sensitivity_level': trigger.sensitivity_score,
                    'recommendations': self._generate_trigger_recommendations(trigger)
                })
    ...
    def _initialize_trigger_patterns(self):
        ...
        self._trigger_patterns = {
            'negative_self_talk': re.compile(r'\b(i am|i\'m)\s+(stupid|worthless|useless|terrible|awful|horrible)\b', re.IGNORECASE),
            'failure_words': re.compile(r'\b(fail|failed|failure|mistake|wrong|error|mess up)\b', re.IGNORECASE),
            'stress_indicators': re.compile(r'\b(stressed|overwhelmed|anxious|worried|panic|pressure)\b', re.IGNORECASE),
            'loss_grief': re.compile(r'\b(lost|death|died|gone|miss|grief|mourning)\b', re.IGNORECASE),
            'rejection_abandonment': re.compile(r'\b(rejected|abandoned|alone|lonely|ignored|left out)\b', re.IGNORECASE),
            'criticism_judgment': re.compile(r'\b(criticized|judged|blamed|attacked|insulted)\b', re.IGNORECASE),
            'health_concerns': re.compile(r'\b(sick|illness|disease|pain|hurt|hospital|doctor)\b', re.IGNORECASE),
            'financial_stress': re.compile(r'\b(money|debt|broke|expensive|afford|financial|budget)\b', re.IGNORECASE)
        }
    ...
    def _detect_keyword_triggers(
        ...
        for indicator, associated_memories in indicator_emotions.items():
            if len(associated_memories) >= 3:  # Minimum occurrences
                emotions = [m.emotional_state.primary_emotion for m in associated_memories]
                ...
                if consistency >= 0.7:  # High consistency threshold
                    ...
                    intensities = [m.emotional_state.intensity for m in associated_memories]
                    avg_intensity = sum(intensities) / len(intensities)
                    
                    trigger_id = hashlib.md5(f"{user_id}_keyword_{indicator}".encode()).hexdigest()[:16]
                    ...
                    trigger.associated_memories = [m.memory_id for m in associated_memories]
    ...
    def _detect_topic_triggers(
        ...
        for memory in memories:
            ...
            topics = []
            if any(word in message for word in ['work', 'job', 'career', 'boss', 'colleague']):
                topics.append('work')
            if any(word in message for word in ['family', 'parent', 'mother', 'father', 'sibling']):
                topics.append('family')
            if any(word in message for word in ['relationship', 'partner', 'boyfriend', 'girlfriend', 'spouse']):
                topics.append('relationships')
            if any(word in message for word in ['health', 'medical', 'doctor', 'hospital', 'sick']):
                topics.append('health')
            if any(word in message for word in ['money', 'financial', 'debt', 'budget', 'expensive']):
        ...
        for topic, associated_memories in topic_emotions.items():
            if len(associated_memories) >= 4:  # Minimum occurrences for topics
                ...
                if negative_ratio >= 0.6:  # Topic consistently triggers negative emotions
                    ...
                    trigger_id = hashlib.md5(f"{user_id}_topic_{topic}".encode()).hexdigest()[:16]
            ...
            for pattern_key, associated_memories in pattern_data.items():
                if len(associated_memories) >= 3:
                    ...
                    if negative_ratio >= 0.7:
                        ...
                        trigger_id = hashlib.md5(f"{user_id}_temporal_{pattern_type}_{pattern_key}".encode()).hexdigest()[:16]
        ...
        for memory in memories:
            ...
            if context.user_feedback:
                if context.user_feedback <= 2:
                    context_groups['feedback_context']['negative_feedback'].append(memory)
                elif context.user_feedback >= 4:
                    context_groups['feedback_context']['positive_feedback'].append(memory)
            ...
            for context_key, associated_memories in context_data.items():
                if len(associated_memories) >= 4:
                    ...
                    if consistency >= 0.6:
                        ...
                        trigger_id = hashlib.md5(f"{user_id}_contextual_{context_type}_{context_key}".encode()).hexdigest()[:16]
    ...
    def _detect_semantic_triggers(
        ...
        for pattern_name, pattern_regex in self._trigger_patterns.items():
            ...
            if len(matching_memories) >= 3:
                emotions = [m.emotional_state.primary_emotion for m in matching_memories]
                ...
                if negative_ratio >= 0.6:
                    ...
                    intensities = [m.emotional_state.intensity for m in matching_memories]
                    avg_intensity = sum(intensities) / len(intensities)
                    
                    trigger_id = hashlib.md5(f"{user_id}_semantic_{pattern_name}".encode()).hexdigest()[:16]
                    ...
                    trigger = EmotionalTrigger(
                        ...
                        context_factors={
                            ...
                            'negative_ratio': negative_ratio,
                            'pattern_description': self.TRIGGER_TYPES.get('semantic', '')
                        },
                        detection_count=len(matching_memories)
                    )
                    
                    trigger.associated_memories = [m.memory_id for m in matching_memories]
                    trigger.response_intensities = intensities
    ...
    def _detect_personal_triggers(
        ...
        personal_patterns = {
            'self_reference': re.compile(r'\b(i am|i\'m|my|me|myself)\b', re.IGNORECASE),
            'family_reference': re.compile(r'\b(family|parent|mother|father|mom|dad|sibling|brother|sister)\b', re.IGNORECASE),
            'past_reference': re.compile(r'\b(remember|past|before|used to|back then|childhood)\b', re.IGNORECASE),
            'future_concern': re.compile(r'\b(future|tomorrow|next|will|going to|plan|hope|worry about)\b', re.IGNORECASE)
        }
        ...
        for pattern_name, pattern_regex in personal_patterns.items():
            ...
            if len(matching_memories) >= 4:
                ...
                if consistency >= 0.5:  # Lower threshold for personal triggers
                    ...
                    trigger_id = hashlib.md5(f"{user_id}_personal_{pattern_name}".encode()).hexdigest()[:16]
    ...
    def _generate_trigger_recommendations(self, trigger: EmotionalTrigger) -> List[str]:
        ...
        if trigger.trigger_type == 'keyword':
            ...
            recommendations.append("Provide emotional support when discussing this topic")
