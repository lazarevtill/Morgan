...
class EmotionalContext:
    ...
    def get_emotional_trajectory(self) -> str:
        ...
        # Check volatility
        variance = sum(
            (v - sum(valences) / len(valences)) ** 2 for v in valences
        ) / len(valences)
    ...
    def get_contextual_recommendations(self) -> Dict[str, Any]:
        ...
        if self.current_emotion.primary_emotion in [
            EmotionType.SADNESS,
            EmotionType.FEAR,
        ]:
...
class EmotionalContextBuilder:
    ...
    def build_context(
        ...
        logger.debug(
            f"Built emotional context for user {user_id} "
    ...
    def update_context_with_feedback(
        ...
        context.context_confidence = max(
            0.0, min(1.0, context.context_confidence + (feedback_factor * 0.1))
        )
        ...
        logger.debug(
            f"Updated context with feedback: {user_feedback}/5, "
    ...
    def analyze_emotional_patterns(
        ...
        memories = self.memory_storage.retrieve_memories(
            user_id=user_id, max_age_days=timeframe_days, limit=100
        )
    ...
    def _gather_recent_emotions(
        ...
        recent_memories = self.memory_storage.retrieve_memories(
            user_id=user_id, max_age_days=7, limit=limit
        )
    ...
    def _retrieve_relevant_memories(
        ...
        memory_limits = {"shallow": 3, "medium": 5, "deep": 10}
    ...
    def _analyze_contextual_factors(
        ...
        if recent_emotions:
            ...
            factors["emotional_consistency"] = consistent_emotions / len(
                recent_emotions
            )
        ...
        if conversation_context.session_duration:
            factors["session_duration_minutes"] = (
                conversation_context.session_duration.total_seconds() / 60
            )
    ...
    def _analyze_temporal_patterns(
        ...
        if hourly_emotions:
            hour_counts = {
                hour: len(emotions) for hour, emotions in hourly_emotions.items()
            }
        ...
        if daily_emotions:
            day_counts = {
                day: len(emotions) for day, emotions in daily_emotions.items()
            }
