...
from morgan.emotional.models import (
    EmotionalState, EmotionType, MoodPattern, ConversationContext
)
...
class MoodAnalyzer:
    ...
    def analyze_mood_patterns(
        ...
        recent_emotions = [
            emotion for emotion in emotional_states
            if emotion.timestamp >= cutoff_date
        ]
        
        if len(recent_emotions) < config["min_samples"]:
            return self._create_insufficient_data_pattern(user_id, timeframe, len(recent_emotions))
    ...
    def compare_mood_periods(
        ...
        """
        current_pattern = self.analyze_mood_patterns(user_id, current_emotions, timeframe)
        previous_pattern = self.analyze_mood_patterns(user_id, previous_emotions, timeframe)
        
        # Calculate changes
        intensity_change = current_pattern.average_intensity - previous_pattern.average_intensity
        stability_change = current_pattern.mood_stability - previous_pattern.mood_stability
        
        # Analyze emotion shifts
        emotion_shifts = self._analyze_emotion_shifts(
            previous_pattern.dominant_emotions,
            current_pattern.dominant_emotions
        )
        ...
        return {
            ...
            "comparison_confidence": min(
                current_pattern.pattern_confidence,
                previous_pattern.pattern_confidence
            )
        }
    ...
    def detect_mood_anomalies(
        ...
        baseline_emotions = [e for e in sorted_emotions if e.timestamp >= baseline_cutoff]
    ...
    def _analyze_dominant_emotions(
        ...
        for emotion_type in emotion_counts:
            ...
            intensity_score = weighted_scores[emotion_type] / emotion_counts[emotion_type]
    ...
    def _analyze_emotional_trends(
        ...
        return {
            ...
            "valence_change": recent_avg - older_avg if len(valence_scores) >= 3 else 0.0,
            "most_recent_emotions": [e.primary_emotion.value for e in sorted_emotions[-3:]]
        }
    ...
    def _create_insufficient_data_pattern(
        ...
        return MoodPattern(
            ...
            mood_stability=0.5,
            emotional_trends={"trend": "insufficient_data", "sample_count": sample_count},
            pattern_confidence=0.0
        )
    
    def _analyze_emotion_shifts(
        self,
        previous_emotions: List[EmotionType],
        current_emotions: List[EmotionType]
    ) -> Dict[str, Any]:
    ...
    def _determine_overall_trend(
        ...
        if stability_change < -0.1:
            negative_score += 1
        if any(e in emotion_shifts.get("new_emotions", []) 
    ...
    def _calculate_baseline_statistics(
        ...
        return {
            ...
            "intensity_stdev": statistics.stdev(intensities) if len(intensities) > 1 else 0.1,
    ...
    def _check_emotion_anomaly(
        ...
        if intensity_z_score > 2.0 or is_unusual_emotion:
            return {
                ...
                "anomaly_score": max(intensity_z_score / 3.0, 0.8 if is_unusual_emotion else 0.0),
                "reason": "unusual_intensity" if intensity_z_score > 2.0 else "unusual_emotion"
            }
    ...
    def _detect_cyclical_patterns(self, emotions: List[EmotionalState]) -> Dict[str, Any]:
