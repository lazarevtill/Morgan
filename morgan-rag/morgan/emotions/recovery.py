...
class EmotionalRecovery:
    ...
    def calculate_recovery_effectiveness(self) -> float:
        ...
        initial_valence = self._get_emotional_valence(
            self.initial_state.primary_emotion
        )
        recovery_valence = self._get_emotional_valence(
            self.recovery_state.primary_emotion
        )

        valence_improvement = recovery_valence - initial_valence
        intensity_improvement = (
            self.initial_state.intensity - self.recovery_state.intensity
        )
        ...
        # Combine scores
        effectiveness = (
            (valence_score * 0.4) + (intensity_score * 0.4) + (speed_score * 0.2)
        )
...
class EmotionalRecoveryTracker:
    ...
    def detect_recoveries(
        ...
        memories = self.memory_storage.retrieve_memories(
            user_id=user_id, max_age_days=analysis_days, min_importance=0.2, limit=200
        )

        if len(memories) < 10:
            logger.debug(
                f"Insufficient data for recovery detection: {len(memories)} memories"
            )
            return []
        ...
        logger.info(
            f"Detected {len(detected_recoveries)} emotional recoveries for user {user_id}"
        )
    ...
    def get_user_recoveries(
        ...
        if recovery_types:
            user_recoveries = [
                r for r in user_recoveries if r.recovery_type in recovery_types
            ]
    ...
    def analyze_recovery_patterns(
        self, user_id: str, timeframe_days: int = 60
    ) -> Dict[str, Any]:
        """
        ...
        recovery_times = [
            r.recovery_duration.total_seconds() / 3600 for r in recoveries
        ]  # in hours
        effectiveness_scores = [
            r.calculate_recovery_effectiveness() for r in recoveries
        ]
        ...
        recovery_time_variance = (
            statistics.variance(recovery_times) if len(recovery_times) > 1 else 0
        )
        ...
        return {
            ...
            "recovery_time_consistency": max(
                0.0, 1.0 - (recovery_time_variance / max(1, avg_recovery_time))
            ),
    ...
    def assess_current_recovery_potential(
        self, user_id: str, current_emotional_state: EmotionalState
    ) -> Dict[str, Any]:
        """
        ...
        for recovery in user_recoveries:
            if (
                recovery.initial_state.primary_emotion
                == current_emotional_state.primary_emotion
            ):
                intensity_diff = abs(
                    recovery.initial_state.intensity - current_emotional_state.intensity
                )
    ...
    def _analyze_recovery_sequence(
        self, memories: List[EmotionalMemory], start_index: int, min_improvement: float
        ...
        for i in range(1, max_look_ahead + 1):
            ...
            if (
                valence_improvement >= min_improvement
                or intensity_improvement >= min_improvement
            ):
                recovery_duration = current_state.timestamp - initial_state.timestamp
                ...
                recovery_id = (
                    f"{initial_memory.user_id}_{initial_state.timestamp.isoformat()}"
                )
    ...
    def _classify_recovery_type(self, memory_sequence: List[EmotionalMemory]) -> str:
        ...
        all_content = " ".join(
            [m.conversation_context.message_text.lower() for m in memory_sequence]
        )

        if any(
            word in all_content
            for word in ["help", "support", "talk", "listen", "understand"]
        ):
            return "supported"
        elif any(
            word in all_content
            for word in ["distract", "activity", "do something", "busy"]
        ):
            return "distraction"
        elif any(
            word in all_content
            for word in ["think", "realize", "understand", "perspective"]
        ):
            return "cognitive"
        elif any(
            word in all_content for word in ["friend", "family", "social", "together"]
        ):
            return "social"
    ...
    def _extract_support_factors(
        self, memory_sequence: List[EmotionalMemory]
    ) -> List[str]:
        ...
        positive_feedback_count = sum(
            ...
            if m.conversation_context.user_feedback
            and m.conversation_context.user_feedback >= 4
        )
    ...
    def _calculate_effectiveness_trend(
        self, recoveries: List[EmotionalRecovery]
    ) -> str:
    ...
    def _calculate_resilience_score(self, recoveries: List[EmotionalRecovery]) -> float:
        ...
        if len(effectiveness_scores) > 1:
            consistency = 1.0 - (
                statistics.stdev(effectiveness_scores) / max(0.1, avg_effectiveness)
            )
        ...
        severe_recoveries = [r for r in recoveries if r.initial_state.intensity > 0.7]
    ...
    def _generate_recovery_recommendations(
        self, similar_recoveries: List[EmotionalRecovery]
    ) -> List[str]:
        ...
        if support_effectiveness:
            ...
            recommendations.append(
                f"Consider using {best_support.replace('_', ' ')} - it has been effective before"
            )
        ...
        if type_effectiveness:
            best_type = max(
                type_effectiveness, key=lambda t: statistics.mean(type_effectiveness[t])
            )
            recommendations.append(
                f"Try {best_type.replace('_', ' ')} recovery approach"
            )
        ...
        if avg_time < 6:
            ...
            recommendations.append(
                "Recovery usually takes several hours - give it time"
            )
        else:
            recommendations.append(
                "Recovery may take a day or more - focus on self-care"
            )

        return (
            recommendations
            if recommendations
            else ["Continue with current support approach"]
        )
