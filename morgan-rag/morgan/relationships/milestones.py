...
from ..emotional.models import (
    ...
    RelationshipMilestone, MilestoneType, InteractionData
)
...
class MilestoneDetector:
    ...
    def detect_milestones(
        ...
        context = MilestoneContext(
            ...
            recent_milestones=user_profile.relationship_milestones[-5:]  # Last 5 milestones
        )
    ...
    def analyze_milestone_trends(
        ...
        recent_milestones = [
            m for m in user_profile.relationship_milestones
        ...
        return {
            ...
            'average_significance': sum(m.emotional_significance for m in recent_milestones) / len(recent_milestones),
    ...
    def suggest_milestone_opportunities(
        ...
        if ('trust_building' not in existing_types and 
            user_profile.trust_level > 0.3 and 
        ...
        if ('learning_milestone' not in existing_types and 
        ...
        if ('deep_conversation' not in existing_types and 
            user_profile.trust_level > 0.4 and 
            user_profile.interaction_count >= 8):
            opportunities.append({
                'milestone_type': 'deep_conversation',
                'category': 'communication',
                'opportunity': 'Engage in a profound, meaningful conversation',
                'readiness_score': (user_profile.trust_level + user_profile.engagement_score) / 2,
        ...
        logger.info(f"Identified {len(opportunities)} milestone opportunities for user {user_profile.user_id}")
    ...
    def validate_milestone(
        ...
        if context.interaction_data.emotional_state.primary_emotion.value in ['joy', 'surprise']:
            validation_score += 0.3
            validation_reasons.append("positive emotional context")
        elif context.interaction_data.emotional_state.primary_emotion.value in ['sadness', 'fear']:
        ...
        recent_same_type = [
            m for m in context.recent_milestones
        ...
        reason = "; ".join(validation_reasons) if validation_reasons else "standard validation"
    ...
    def _initialize_milestone_patterns(self) -> List[MilestonePattern]:
        ...
        return [
            ...
            MilestonePattern(
                ...
                detection_rules={
                    'min_trust_level': 0.3,
                    'personal_keywords': ['feel', 'think', 'personal', 'share', 'trust', 'believe'],
            ...
            MilestonePattern(
                ...
                detection_rules={
                    'learning_keywords': ['learn', 'understand', 'got it', 'makes sense', 'clear', 'thank you'],
                    'min_engagement': 0.4,
                    'min_interactions': 2
                },
            ...
            MilestonePattern(
                ...
                detection_rules={
                    'emotional_keywords': ['sad', 'worried', 'anxious', 'stressed', 'difficult', 'hard'],
            ...
            MilestonePattern(
                ...
                detection_rules={
                    'achievement_keywords': ['achieved', 'accomplished', 'completed', 'success', 'done', 'finished'],
            ...
            MilestonePattern(
                ...
                category=MilestoneCategory.LEARNING,
                detection_rules={
                    'breakthrough_keywords': ['breakthrough', 'revelation', 'realize', 'suddenly', 'aha', 'insight'],
                ...
                required_conditions=['breakthrough_indicators', 'high_emotional_impact']
            )
        ]
    ...
    def _should_check_milestone(
        ...
        if recent_same_type:
            interactions_since = context.user_profile.interaction_count - len(context.recent_milestones)
    ...
    def _evaluate_milestone_pattern(
        ...
        message_text = context.interaction_data.conversation_context.message_text.lower()
        emotional_state = context.interaction_data.emotional_state
        
        # Check keyword matches
        for keyword_type in ['personal_keywords', 'learning_keywords', 'emotional_keywords', 
                           'achievement_keywords', 'breakthrough_keywords']:
        ...
        if 'min_trust_level' in rules:
            if context.user_profile.trust_level >= rules['min_trust_level']:
                match_score += 0.2
                match_reasons.append(f"trust level: {context.user_profile.trust_level:.2f}")
            else:
                return None  # Hard requirement
        ...
        if 'min_engagement' in rules:
            if context.user_profile.engagement_score >= rules['min_engagement']:
                match_score += 0.2
                match_reasons.append(f"engagement: {context.user_profile.engagement_score:.2f}")
        ...
        if 'emotional_intensity' in rules:
            if emotional_state.intensity >= rules['emotional_intensity']:
                match_score += 0.2
                match_reasons.append(f"emotional intensity: {emotional_state.intensity:.2f}")
        
        if 'high_emotion_intensity' in rules:
            if emotional_state.intensity >= rules['high_emotion_intensity']:
                match_score += 0.3
                match_reasons.append(f"high emotional intensity: {emotional_state.intensity:.2f}")
        ...
        milestone = RelationshipMilestone(
            ...
            description=self._generate_milestone_description(pattern, context, match_reasons),
    ...
    def _generate_milestone_description(
        ...
        base_description = descriptions.get(
            pattern.milestone_type,
            f"Meaningful milestone reached with {user_name}"
        )
        
        # Add context if available
        if len(context.interaction_data.conversation_context.message_text) > 50:
            snippet = context.interaction_data.conversation_context.message_text[:50] + "..."
            base_description += f" (Context: {snippet})"
        
        return base_description
    
    def _get_pattern_for_milestone(self, milestone_type: MilestoneType) -> Optional[MilestonePattern]:
    ...
    def _predict_next_milestone(
        ...
        if user_profile.interaction_count <= 3:
            ...
            if 'trust_building' not in existing_types and user_profile.trust_level > 0.3:
