...
class EmotionalMirror:
    ...
    def mirror_emotion(
        ...
        try:
            ...
            adjusted_intensity = min(1.0, emotional_state.intensity * mirroring_intensity)
    ...
    def create_reflection_prompt(
        ...
        if emotional_state.intensity > 0.7:
            ...
            selected_prompt = base_prompts[1] if len(base_prompts) > 1 else base_prompts[0]
        else:
            # Low intensity - use gentle prompt
            selected_prompt = base_prompts[-1] if len(base_prompts) > 2 else base_prompts[0]
    ...
    def generate_emotional_reflection(
        ...
        reflection = {
            ...
            "reflection_prompt": self.create_reflection_prompt(emotional_state, context),
            "emotional_patterns": self._identify_emotional_patterns(emotional_state, context),
            "growth_opportunities": self._identify_growth_opportunities(emotional_state),
            "coping_suggestions": self._suggest_coping_strategies(emotional_state),
            "reflection_confidence": self._calculate_reflection_confidence(emotional_state)
        }
        ...
        if reflection_depth in ["medium", "deep"]:
            reflection.update({
                ...
                "relationship_impact": self._analyze_relationship_impact(emotional_state, context)
            })
        
        # Add deepest analysis for deep reflection
        if reflection_depth == "deep":
            reflection.update({
                "underlying_values": self._identify_underlying_values(emotional_state, context),
                "personal_growth_insights": self._generate_growth_insights(emotional_state, context),
                "future_emotional_preparation": self._suggest_future_preparation(emotional_state)
            })
    ...
    def create_empathetic_echo(
        ...
        try:
            ...
            emotional_phrases = self._extract_emotional_phrases(user_words, emotional_state)
            ...
            if emotional_phrases:
                ...
                return self.mirror_emotion(emotional_state, ConversationContext(
                    user_id="", conversation_id="", message_text=user_words, timestamp=datetime.utcnow()
                ))
    ...
    def _identify_growth_opportunities(self, emotional_state: EmotionalState) -> List[str]:
        ...
        return growth_maps.get(emotional_state.primary_emotion, [
    ...
    def _suggest_coping_strategies(self, emotional_state: EmotionalState) -> List[str]:
        ...
        return coping_strategies.get(emotional_state.primary_emotion, [
            "Take time to understand what you're feeling",
            "Be patient and gentle with yourself",
            "Consider what support you might need"
        ])
    
    def _calculate_reflection_confidence(self, emotional_state: EmotionalState) -> float:
    ...
    def _identify_emotional_needs(self, emotional_state: EmotionalState) -> List[str]:
        ...
        return needs_map.get(emotional_state.primary_emotion, ["understanding", "support", "acceptance"])
    ...
    def _analyze_relationship_impact(
        ...
        impact_analysis = {
            ...
            "connection_opportunities": self._identify_connection_opportunities(emotional_state)
        }
    ...
    def _assess_communication_impact(self, emotional_state: EmotionalState) -> str:
        ...
        return communication_impacts.get(emotional_state.primary_emotion, "communication style may be influenced by current emotional state")
    ...
    def _assess_relationship_needs(self, emotional_state: EmotionalState) -> List[str]:
        ...
        return relationship_needs.get(emotional_state.primary_emotion, ["understanding", "acceptance"])
    
    def _identify_connection_opportunities(self, emotional_state: EmotionalState) -> List[str]:
        ...
        opportunities = {
            ...
            EmotionType.ANGER: ["discussing values and boundaries", "problem-solving together"],
            EmotionType.FEAR: ["providing reassurance", "building trust"],
            EmotionType.SURPRISE: ["exploring new experiences together", "adapting together"],
            EmotionType.DISGUST: ["discussing values and ethics", "supporting boundaries"],
            EmotionType.NEUTRAL: ["exploring interests together", "building understanding"]
        }
        
        return opportunities.get(emotional_state.primary_emotion, ["building understanding", "showing empathy"])
    ...
    def _generate_growth_insights(
        ...
        insights.extend(emotion_insights.get(emotional_state.primary_emotion, [
    ...
    def _suggest_future_preparation(self, emotional_state: EmotionalState) -> List[str]:
        ...
        return preparation_strategies.get(emotional_state.primary_emotion, [
            "Develop emotional awareness and coping skills",
            "Build a toolkit for managing various emotional states"
        ])
    
    def _extract_emotional_phrases(self, user_words: str, emotional_state: EmotionalState) -> List[str]:
        """Extract key emotional phrases from user's words."""
        phrases = []
        ...
        emotion_words = {
            EmotionType.JOY: ["happy", "excited", "thrilled", "delighted", "joyful", "elated"],
            EmotionType.SADNESS: ["sad", "depressed", "down", "upset", "heartbroken", "miserable"],
            EmotionType.ANGER: ["angry", "mad", "furious", "frustrated", "irritated", "annoyed"],
            EmotionType.FEAR: ["scared", "afraid", "worried", "anxious", "nervous", "terrified"],
            EmotionType.SURPRISE: ["surprised", "shocked", "amazed", "astonished", "unexpected"],
            EmotionType.DISGUST: ["disgusted", "revolted", "sick", "appalled", "horrified"],
    ...
    def _get_echo_validation(self, emotional_state: EmotionalState) -> str:
        ...
        return validations.get(emotional_state.primary_emotion, "your feelings are completely valid")
