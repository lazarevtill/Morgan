...
from morgan.emotional.models import (
    EmotionalState, EmotionType, ConversationContext, EmpatheticResponse,
    CompanionProfile, RelationshipMilestone
)
...
class EmpatheticResponseGenerator:
    ...
    def create_relationship_aware_response(
        ...
        if companion_profile.relationship_duration.days > 30:
            relationship_elements.append(f"relationship_duration:{companion_profile.relationship_duration.days}days")
        
        # Add interaction history awareness
        if companion_profile.interaction_count > 10:
            relationship_elements.append(f"interaction_history:{companion_profile.interaction_count}conversations")
        
        # Add milestone awareness
        if include_milestones and companion_profile.relationship_milestones:
            recent_milestones = [
                m for m in companion_profile.relationship_milestones
    ...
    def _get_empathy_template(self, emotional_state: EmotionalState) -> str:
        ...
        if not templates:
            return f"I can sense what you're experiencing, and I want you to know I'm here to support you."
    ...
    def _determine_emotional_tone(self, emotional_state: EmotionalState) -> str:
        ...
        return tone_map.get(emotional_state.primary_emotion, "empathetic and supportive")
    ...
    def _identify_personalization_elements(
        ...
        elements.append(f"emotion_acknowledgment:{emotional_state.primary_emotion.value}")
    ...
    def _build_llm_context(
        ...
        if companion_profile:
            context_parts.append(f"Relationship: {companion_profile.interaction_count} interactions over {companion_profile.relationship_duration.days} days")
            
            if companion_profile.preferred_name != "friend":
                context_parts.append(f"User prefers to be called: {companion_profile.preferred_name}")
        
        if context.previous_messages:
            context_parts.append(f"Recent conversation context available")
