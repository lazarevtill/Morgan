...
def deploy(ctx, branch, force, parallel):
    ...
    results = asyncio.run(manager.deploy_all(
        git_branch=branch,
        force=force,
        parallel=parallel
    ))
...
@click.option('--rolling/--parallel', default=True, help='Rolling update (zero-downtime)')
@click.pass_context
def update(ctx, branch, rolling):
    ...
    if rolling:
        ...
        click.echo("Update order: Background → Reranking → Embeddings → LLM#2 → LLM#1 → Manager")
    else:
        click.echo("Starting parallel update (faster, but downtime expected)...")

    results = asyncio.run(manager.update_all(
        git_branch=branch,
        rolling=rolling
    ))
...
@click.option('--hosts', multiple=True, help='Specific hosts (default: all with service)')
@click.pass_context
def restart(ctx, service, hosts):
    ...
    results = asyncio.run(manager.restart_service(
...
def sync_config(ctx, config_file, source):
    ...
    results = asyncio.run(manager.sync_config(
