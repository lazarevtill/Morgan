...
class CompanionMetrics:
    ...
    def __init__(self, metrics_collector: MetricsCollector):
        ...
        self._satisfaction_history: Dict[str, List[Tuple[float, datetime]]] = (
            defaultdict(list)
        )
    ...
    def record_emotional_interaction(
        ...
        self.metrics_collector.record_empathy_score(
            response_type="emotional_response", empathy_score=response_empathy_score
        )
        ...
        if user_satisfaction is not None:
            ...
            self._satisfaction_history[user_id].append(
        ...
        logger.debug(
            "Recorded emotional interaction",
            user_id=user_id,
    ...
    def record_relationship_milestone(
        self,
        user_id: str,
        ...
        if user_id in self._user_relationships:
            ...
            if milestone_type not in relationship.milestones_reached:
                ...
                relationship.trust_level = min(
                    1.0, relationship.trust_level + trust_boost
                )
        ...
        logger.info(
            "Recorded relationship milestone",
            user_id=user_id,
            milestone_type=milestone_type.value,
            significance_score=significance_score,
        )

    def record_emotion_detection_feedback(
        self,
        user_id: str,
        detected_emotion: EmotionType,
        ...
        logger.debug(
            "Recorded emotion detection feedback",
            user_id=user_id,
    ...
    def record_personalization_effectiveness(
        self, user_id: str, personalization_score: float, interaction_type: str
    ):
        ...
        if user_id in self._user_relationships:
            ...
            relationship.engagement_score = (current_engagement * 0.8) + (
                personalization_score * 0.2
            )

        logger.debug(
            "Recorded personalization effectiveness",
            user_id=user_id,
            score=personalization_score,
            interaction_type=interaction_type,
        )

    def _update_user_relationship(
        self, user_id: str, interaction: EmotionalInteraction
    ):
        ...
        if relationship.total_interactions == 1:
            ...
            relationship.relationship_duration_days = max(
                relationship.relationship_duration_days, duration
            )
        ...
        if interaction.user_satisfaction is not None:
            ...
            relationship.average_satisfaction = (
                current_avg * (total_interactions - 1) + interaction.user_satisfaction
            ) / total_interactions
    ...
    def get_companion_quality_metrics(
        ...
        recent_interactions = [
            interaction
            for interaction in self._emotional_interactions
        ...
        empathy_accuracy = (
            sum(recent_empathy_scores) / len(recent_empathy_scores)
            if recent_empathy_scores
            else 0.0
        )
        ...
        emotional_support_interactions = [
            interaction
            for interaction in recent_interactions
            if interaction.response_empathy_score > 0.7
            and interaction.user_satisfaction is not None
        ]
        emotional_support_success = sum(
            1
            for interaction in emotional_support_interactions
        ...
        emotional_support_success_rate = (
            ...
            if emotional_support_interactions
            else 0.0
        )

        # Relationship retention (users with interactions in last 7 days vs total)
        relationship_retention_rate = (
            active_users_7d / total_users if total_users > 0 else 0.0
        )
        ...
        users_with_milestones = len(
            [
                relationship
                for relationship in self._user_relationships.values()
                if relationship.milestones_reached
            ]
        )
        milestone_completion_rate = (
            users_with_milestones / total_users if total_users > 0 else 0.0
        )
    ...
    def get_satisfaction_trends(
        ...
        return [
            (score, timestamp)
            for score, timestamp in self._satisfaction_history[user_id]
            if timestamp >= cutoff_time
        ]

    def get_milestone_statistics(
        self, time_window: timedelta = timedelta(days=30)
    ) -> Dict[MilestoneType, int]:
    ...
    def generate_companion_insights(self) -> Dict[str, Any]:
        ...
        insights = {
            ...
            "user_engagement": {
                ...
                "engagement_trend": (
                    "growing"
                    if quality_metrics.active_users_7d
                    > quality_metrics.active_users_24h * 3
                    else "stable"
                ),
            },
            "emotional_intelligence": {
                ...
                "improvement_areas": self._identify_emotion_improvement_areas(
                    emotion_accuracy
                ),
            },
            ...
            "recommendations": self._generate_companion_recommendations(
                quality_metrics, emotion_accuracy
            ),
        }

        return insights

    def _identify_emotion_improvement_areas(
        self, emotion_accuracy: Dict[str, float]
    ) -> List[str]:
        ...
        for emotion_key, accuracy in emotion_accuracy.items():
            if accuracy < 0.7:  # Below 70% accuracy threshold
                improvement_areas.append(
                    f"Improve detection accuracy for {emotion_key}"
                )
    ...
    def _generate_companion_recommendations(
        self,
        quality_metrics: CompanionQualityMetrics,
        emotion_accuracy: Dict[str, float],
    ) -> List[str]:
