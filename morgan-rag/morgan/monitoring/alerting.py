...
class AlertManager:
    ...
    async def check_companion_metrics_alert(
        self, companion_metrics: CompanionQualityMetrics
    ):
        ...
        try:
            ...
            for channel in alert.channels:
                try:
                    ...
                    logger.error(
                        "Failed to deliver alert through channel",
            ...
            logger.error(
                "Error processing alert", alert_id=alert.alert_id, error=str(e)
            )
    ...
    async def _deliver_console_alert(self, alert: Alert):
        ...
        print(
            f"{color}[ALERT {alert.severity.value.upper()}]{reset_color} {alert.message}"
        )
    ...
    async def _deliver_email_alert(self, alert: Alert):
        ...
        try:
            ...
            msg["Subject"] = (
                f"Morgan RAG Alert: {alert.rule_name} ({alert.severity.value.upper()})"
            )
...
"""
            ...
            logger.error(
                "Failed to send email alert", alert_id=alert.alert_id, error=str(e)
            )
    ...
    def get_alert_statistics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        resolution_rate = (
            len(resolved_alerts) / len(recent_alerts) if recent_alerts else 0
        )
    ...
    def cleanup_old_alerts(self):
        ...
        logger.debug(
            "Cleaned up old alerts", retention_hours=self.config.alert_retention_hours
        )
