...
class MonitoringDashboard:
    ...
    async def _collect_system_health_data(self) -> Dict[str, Any]:
        ...
        if resource_history:
            avg_cpu = sum(s.cpu_percent for s in resource_history) / len(
                resource_history
            )
            avg_memory = sum(s.memory_percent for s in resource_history) / len(
                resource_history
            )
        ...
        return {
            ...
            "system_resources": {
                "current_cpu_percent": (
                    current_stats.cpu_percent if current_stats else 0
                ),
                "current_memory_percent": (
                    current_stats.memory_percent if current_stats else 0
                ),
                "average_cpu_percent_1h": avg_cpu,
                "average_memory_percent_1h": avg_memory,
                "memory_used_bytes": (
                    current_stats.memory_used_bytes if current_stats else 0
                ),
                "memory_available_bytes": (
                    current_stats.memory_available_bytes if current_stats else 0
                ),
            },
    ...
    def _collect_performance_data(self) -> Dict[str, Any]:
        ...
        return {
            "processing": {
                "average_duration": (
                    processing_summary.average if processing_summary else 0
                ),
                "p95_duration": (
                    processing_summary.percentile_95 if processing_summary else 0
                ),
                "p99_duration": (
                    processing_summary.percentile_99 if processing_summary else 0
                ),
                "error_rate": (
                    processing_summary.error_rate if processing_summary else 0
                ),
                "total_operations": (
                    processing_summary.total_count if processing_summary else 0
                ),
            ...
            "embedding": {
                "average_duration": (
                    embedding_summary.average if embedding_summary else 0
                ),
                "p95_duration": (
                    embedding_summary.percentile_95 if embedding_summary else 0
                ),
                "p99_duration": (
                    embedding_summary.percentile_99 if embedding_summary else 0
                ),
                "total_operations": (
                    embedding_summary.total_count if embedding_summary else 0
                ),
            },
            "active_operations": {
                "count": len(active_operations),
                "longest_running": (
                    max(active_operations.values()) if active_operations else 0
                ),
    ...
    def _collect_companion_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "emotional_intelligence": {
                "overall_accuracy": companion_insights["emotional_intelligence"][
                    "empathy_effectiveness"
                ],
                "emotion_detection_accuracy": emotion_accuracy,
                "improvement_areas": companion_insights["emotional_intelligence"][
                    "improvement_areas"
                ],
            },
            "relationship_building": {
                "milestone_stats": milestone_stats,
                "trust_building_success": companion_insights["relationship_building"][
                    "trust_building_success"
                ],
                "personalization_effectiveness": companion_insights[
                    "relationship_building"
    ...
    def _get_cache_performance(self) -> Dict[str, Any]:
        ...
        cache_metrics = self.metrics_collector.get_recent_metrics(
            "cache_hit_rate", limit=10
        )
    ...
    def _cleanup_old_data(self):
        ...
        self._dashboard_data_history = [
            data
            for data in self._dashboard_data_history
    ...
    def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "companion_summary": {
                "user_satisfaction": companion["quality_metrics"][
    ...
    async def generate_health_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "executive_summary": {
                ...
                "key_metrics": {
                    "uptime_hours": dashboard_data.system_health["uptime_seconds"]
                    / 3600,
                    "user_satisfaction": dashboard_data.companion_metrics[
                        "quality_metrics"
                    ]["average_satisfaction"],
                    "system_performance": (
                        "good"
                        if dashboard_data.performance_metrics["processing"][
                            "p95_duration"
                        ]
                        < 5.0
                        else "degraded"
            ...
            "recommendations": {
                "immediate_actions": [
                    r for r in dashboard_data.recommendations if "CRITICAL" in r
                ],
                "optimization_opportunities": [
                    r for r in dashboard_data.recommendations if "CRITICAL" not in r
                ],
                "monitoring_focus_areas": self._identify_monitoring_focus_areas(
                    dashboard_data
                ),
            },
    ...
    def _calculate_performance_trend(
        self, history: List[DashboardData]
    ) -> Dict[str, str]:
        ...
        early_processing_p95 = sum(
            d.performance_metrics["processing"]["p95_duration"] for d in early_data
        ) / len(early_data)
        recent_processing_p95 = sum(
            d.performance_metrics["processing"]["p95_duration"] for d in recent_data
        ) / len(recent_data)

        early_search_p95 = sum(
            d.performance_metrics["search"]["p95_duration"] for d in early_data
        ) / len(early_data)
        recent_search_p95 = sum(
            d.performance_metrics["search"]["p95_duration"] for d in recent_data
        ) / len(recent_data)

        # Determine trends
        processing_trend = (
            "improving"
            if recent_processing_p95 < early_processing_p95 * 0.9
            else (
                "degrading"
                if recent_processing_p95 > early_processing_p95 * 1.1
                else "stable"
            )
        )
        ...
        search_trend = (
            "improving"
            if recent_search_p95 < early_search_p95 * 0.9
            else "degrading" if recent_search_p95 > early_search_p95 * 1.1 else "stable"
        )

        return {"processing": processing_trend, "search": search_trend}

    def _calculate_companion_trend(
        self, history: List[DashboardData]
    ) -> Dict[str, str]:
        ...
        early_satisfaction = sum(
            d.companion_metrics["quality_metrics"]["average_satisfaction"]
            for d in early_data
        ) / len(early_data)
        ...
        early_empathy = sum(
            d.companion_metrics["quality_metrics"]["empathy_accuracy"]
            for d in early_data
        ) / len(early_data)
        recent_empathy = sum(
            d.companion_metrics["quality_metrics"]["empathy_accuracy"]
        ...
        satisfaction_trend = (
            "improving"
            if recent_satisfaction > early_satisfaction * 1.05
            else (
                "degrading"
                if recent_satisfaction < early_satisfaction * 0.95
                else "stable"
            )
        )
        ...
        empathy_trend = (
            "improving"
            if recent_empathy > early_empathy * 1.05
            else "degrading" if recent_empathy < early_empathy * 0.95 else "stable"
        )

        return {"satisfaction": satisfaction_trend, "empathy": empathy_trend}

    def _identify_monitoring_focus_areas(
        ...
        if (
            dashboard_data.companion_metrics["quality_metrics"]["average_satisfaction"]
            < 0.7
        ):
            focus_areas.append("User satisfaction improvement")

        if (
            dashboard_data.companion_metrics["quality_metrics"]["empathy_accuracy"]
            < 0.8
        ):
