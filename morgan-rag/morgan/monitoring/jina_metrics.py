...
class JinaMetricsCollector:
    ...
    def __init__(self, base_metrics_collector: MetricsCollector):
        ...
        # Throughput tracking
        self.throughput_windows: Dict[str, deque] = defaultdict(
    ...
    def record_model_performance(
        ...
        logger.debug(
            "Recorded Jina model performance",
            model_type=model_type.value,
    ...
    def record_model_load_time(self, model_type: JinaModelType, load_time: float):
        ...
        logger.info(
            "Recorded model load time", model_type=model_type.value, load_time=load_time
        )
    ...
    def _update_model_calculated_metrics(self, model_type: JinaModelType):
        ...
        if metrics.total_requests > 0:
            ...
            recent_requests = [
                ...
                for timestamp, batch_size in self.throughput_windows.get(
                    throughput_key, []
                )
    ...
    def get_model_metrics(
        self, model_type: Optional[JinaModelType] = None
    ) -> Dict[str, JinaModelMetrics]:
        ...
        for model_type, stats in model_stats.items():
            if stats["latencies"]:
                ...
                summary[model_type] = {
                    ...
                    "average_latency": stats["total_processing_time"]
                    / len(stats["latencies"]),
                    ...
                    "tokens_per_second": (
                        stats["total_tokens"] / time_window.total_seconds()
    ...
    def get_quality_summary(
        ...
        for key, stats in quality_stats.items():
            summary[key] = {
                ...
                "fallback_rate": (
                    stats["fallback_count"] / stats["total_operations"]
                ...
                "average_relevance_improvement": (
                    sum(stats["relevance_improvements"])
                    / len(stats["relevance_improvements"])
                    if stats["relevance_improvements"]
                    else None
                ),
                "average_user_satisfaction": (
                    sum(stats["user_satisfactions"]) / len(stats["user_satisfactions"])
                    if stats["user_satisfactions"]
                    else None
                ),
                "average_confidence": (
    ...
    def get_optimization_insights(self) -> Dict[str, Any]:
        ...
        for model_type, metrics in self.model_metrics.items():
            if metrics.total_requests > 0:
                insights["model_performance"][model_type.value] = {
                    "health_status": (
                        "healthy" if self.model_health[model_type] else "unhealthy"
                    ),
        ...
        if batch_performance:
            ...
            optimal_batch = max(
                batch_performance.items(), key=lambda x: x[1]["throughput"]
            )
    ...
    def get_real_time_metrics(self) -> Dict[str, Any]:
        ...
        if recent_performance:
            ...
            avg_latency = (
                sum(r.processing_time for r in recent_performance) / total_requests
            )
        ...
        return {
            ...
            "active_models": [
                ...
                and metrics.last_used >= current_time - timedelta(minutes=10)
            ],
        }
            ...
            prometheus_metrics.update(
                {
                    ...
                    f"jina_model_health_{model_name}": (
                        1 if self.model_health[model_type] else 0
                    ),
                }
            )

            if metrics.memory_usage_mb is not None:
                prometheus_metrics[f"jina_model_memory_usage_mb_{model_name}"] = (
                    metrics.memory_usage_mb
                )

            if metrics.model_load_time is not None:
                prometheus_metrics[f"jina_model_load_time_seconds_{model_name}"] = (
