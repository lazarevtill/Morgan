...
class MetricsCollector:
    ...
    def record_processing_time(
        ...
        with self._lock:
            ...
            self.documents_processed.labels(
                document_type=document_type, status="success" if success else "error"
            ).inc()
    ...
    def record_search_time(
        ...
        with self._lock:
            ...
            self.search_requests.labels(
                search_type=search_type, status="success" if success else "error"
            ).inc()
    ...
    def record_embedding_time(self, model_type: str, scale: str, duration: float):
        ...
        with self._lock:
            self.embedding_duration.labels(model_type=model_type, scale=scale).observe(
                duration
            )
            ...
            metric = MetricValue(
                ...
                value=duration,
                timestamp=datetime.now(),
                labels={"model_type": model_type, "scale": scale},
            )
            self._metrics_buffer["embedding_time"].append(metric)

        logger.debug(
            "Recorded embedding time",
    ...
    def record_search_relevance(self, search_type: str, relevance_score: float):
        ...
        with self._lock:
            self.search_relevance.labels(search_type=search_type).observe(
                relevance_score
            )
        ...
        logger.debug(
            "Recorded search relevance",
            search_type=search_type,
            relevance_score=relevance_score,
        )

    def record_user_satisfaction(
        self, interaction_type: str, satisfaction_score: float
    ):
        """Record user satisfaction rating."""
        with self._lock:
            self.user_satisfaction.labels(interaction_type=interaction_type).observe(
                satisfaction_score
            )
    ...
    def record_cache_hit_rate(self, cache_type: str, hit_rate: float):
        ...
        logger.debug(
            "Recorded cache hit rate", cache_type=cache_type, hit_rate=hit_rate
        )
    ...
    def record_git_cache_request(self, cache_hit: bool, source_type: str = "unknown"):
        ...
        with self._lock:
            ...
            metric = MetricValue(
                ...
                value=1,
                timestamp=datetime.now(),
                labels={"result": result, "source_type": source_type},
            )
            self._metrics_buffer["git_cache_requests"].append(metric)

        logger.debug(
            "Recorded Git cache request", cache_hit=cache_hit, source_type=source_type
        )
    ...
    def record_git_hash_calculation(self, source_type: str = "file"):
        ...
        logger.debug("Recorded Git hash calculation", source_type=source_type)
    ...
    def record_cache_invalidation(self, reason: str = "manual"):
        ...
        logger.info("Recorded cache invalidation", reason=reason)
    ...
    def record_memory_usage(self, component: str, memory_bytes: int):
        ...
        logger.debug(
            "Recorded memory usage", component=component, memory_bytes=memory_bytes
        )

    def record_database_connections(self, database_type: str, connection_count: int):
        """Record active database connections."""
        with self._lock:
            self.database_connections.labels(database_type=database_type).set(
                connection_count
            )
        ...
        logger.debug(
            "Recorded database connections",
            database_type=database_type,
            connection_count=connection_count,
        )

    def record_error(
        self, error_type: str, component: str, error_details: Optional[str] = None
    ):
    ...
    def record_emotional_analysis_time(self, analysis_type: str, duration: float):
        ...
        with self._lock:
            self.emotional_analysis_duration.labels(
        ...
        logger.debug(
            "Recorded emotional analysis time",
            analysis_type=analysis_type,
            duration=duration,
        )
    ...
    def record_relationship_milestone(self, milestone_type: str):
        ...
        logger.info("Recorded relationship milestone", milestone_type=milestone_type)

    def record_empathy_score(self, response_type: str, empathy_score: float):
        """Record empathy score for a response."""
        with self._lock:
            self.empathy_score.labels(response_type=response_type).observe(
                empathy_score
            )
        ...
        logger.debug(
            "Recorded empathy score",
            response_type=response_type,
            empathy_score=empathy_score,
        )

    def get_metrics_summary(
        self, metric_name: str, time_window: timedelta = timedelta(hours=1)
        ...
        with self._lock:
            ...
            error_count = sum(
                1 for m in recent_metrics if m.labels.get("success") == "False"
            )
    ...
    def get_system_health_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            total_operations = sum(
                len(timings) for timings in self._operation_timings.values()
            )
            ...
            return {
                ...
                "search_performance": {
                    ...
                    "p95_duration": (
                        search_summary.percentile_95 if search_summary else 0
                    ),
