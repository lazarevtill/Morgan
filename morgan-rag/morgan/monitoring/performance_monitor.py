...
class PerformanceMonitor:
        ...
        try:
            ...
            if operation_type == "processing":
                self.metrics_collector.record_processing_time(
                    ...
                    duration=duration,
                    document_type=(
                        labels.get("document_type", "unknown") if labels else "unknown"
                    ),
                    success=False,
                )
                ...
                self.metrics_collector.record_search_time(
                    ...
                    duration=duration,
                    collection=(
                        labels.get("collection", "default") if labels else "default"
                    ),
                    success=False,
                )
    ...
    def start_system_monitoring(self, interval_seconds: float = 30.0):
        ...
        self._system_monitor_thread = threading.Thread(
            target=self._system_monitor_loop, args=(interval_seconds,), daemon=True
        )
        ...
        while self._system_monitor_active:
            try:
                ...
                self.metrics_collector.record_memory_usage(
                    "system", stats.memory_used_bytes
                )
    ...
    def _check_application_performance_alerts(self, alerts: List[PerformanceAlert]):
        ...
        if (
            processing_summary
            and processing_summary.percentile_95 > self.thresholds.processing_time_p95
        ):
            alerts.append(
                PerformanceAlert(
        ...
        if (
            search_summary
            and search_summary.percentile_95 > self.thresholds.search_time_p95
        ):
            alerts.append(
                PerformanceAlert(
        ...
        if (
            processing_summary
            and processing_summary.error_rate > self.thresholds.error_rate_max / 100
        ):
            alerts.append(
                PerformanceAlert(
    ...
    def get_active_alerts(self) -> List[PerformanceAlert]:
        ...
        self._active_alerts = [
            alert for alert in self._active_alerts if alert.timestamp >= cutoff_time
        ]
    ...
    def get_system_stats_history(self, hours: int = 1) -> List[SystemResourceUsage]:
        ...
        return [
            stats
            for stats in self._system_stats_history
        ...
        return {
            ...
            "application_performance": {
                ...
                "search": {
                    ...
                    "p99_duration": (
                        search_summary.percentile_99 if search_summary else 0
                    ),
                    "error_rate": search_summary.error_rate if search_summary else 0,
                    "total_operations": (
                        search_summary.total_count if search_summary else 0
                    ),
                },
            },
            "active_operations": {
                ...
                "longest_running_duration": (
                    max(active_ops.values()) if active_ops else 0
                ),
            ...
            "alerts": {
                ...
                "critical_count": len(
                    [a for a in self._active_alerts if a.severity == "critical"]
                ),
                "warning_count": len(
                    [a for a in self._active_alerts if a.severity == "warning"]
                ),
            },
            "thresholds": {
