...
from morgan.emotional.models import (
    EmotionalState, EmotionType, MoodPattern, UserPreferences,
    RelationshipMilestone, MilestoneType, EmpatheticResponse,
    ConversationContext, InteractionData, CompanionProfile,
...
class EmotionalIntelligenceEngine:
    ...
    def track_mood_patterns(
        ...
        recent_emotions = [
            emotion for emotion in self.mood_patterns[user_id]
        ...
        pattern_confidence = min(1.0, len(recent_emotions) / 20.0)  # Full confidence at 20+ data points
    ...
    def generate_empathetic_response(
        ...
        response_text = self._generate_empathetic_text(user_emotion, context, emotional_tone)
        ...
        confidence_score = user_emotion.confidence * 0.8  # Slightly lower than emotion confidence
    ...
    def _detect_emotions_llm(
        ...
        try:
            ...
            return EmotionalState(
                ...
                secondary_emotions=[EmotionType(e) for e in emotion_data.get("secondary_emotions", [])],
    ...
    def _combine_emotion_results(
        ...
        return EmotionalState(
            ...
            emotional_indicators=llm_emotion.emotional_indicators + ["hybrid detection"]
        )
    
    def _analyze_emotional_trends(self, emotions: List[EmotionalState]) -> Dict[str, Any]:
    ...
    def _select_emotional_tone(self, user_emotion: EmotionalState) -> str:
        ...
        return emotion_tones.get(user_emotion.primary_emotion, "empathetic and supportive")
    ...
    def _update_emotional_patterns(
        ...
        if len(profile.emotional_patterns["recent_emotions"]) > 20:
            profile.emotional_patterns["recent_emotions"] = \
                profile.emotional_patterns["recent_emotions"][-20:]
    ...
    def _update_communication_preferences(
        ...
        if msg_length > 200:
            ...
            profile.communication_preferences.preferred_response_length = ResponseLength.DETAILED
        elif msg_length < 50:
            # User prefers brief communication
            profile.communication_preferences.preferred_response_length = ResponseLength.BRIEF
    ...
    def _update_relationship_scores(
        ...
        if interaction_data.conversation_context.user_feedback:
            engagement_factors.append(interaction_data.conversation_context.user_feedback / 5.0)
        
        if engagement_factors:
            avg_engagement = sum(engagement_factors) / len(engagement_factors)
            # Smooth update (weighted average)
            profile.engagement_score = (profile.engagement_score * 0.8) + (avg_engagement * 0.2)
