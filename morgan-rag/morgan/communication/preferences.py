...
class UserPreferenceLearner:
    ...
    def __init__(self):
        ...
        self.learning_history: Dict[str, List[PreferenceLearningResult]] = defaultdict(
            list
        )
    ...
    def _analyze_interaction_patterns(self, user_id: str) -> List[InteractionPattern]:
        ...
        if avg_length > 150:
            patterns.append(
                InteractionPattern(
                    ...
                    frequency=len(
                        [msg_len for msg_len in message_lengths if msg_len > 150]
                    ),
                    confidence=0.8,
                    examples=[],
                )
            ...
            patterns.append(
                InteractionPattern(
                    ...
                    frequency=len(
                        [msg_len for msg_len in message_lengths if msg_len < 50]
                    ),
                    confidence=0.8,
                    examples=[],
                )
    ...
    def _learn_communication_style(
        ...
        for interaction in interactions[-10:]:  # Last 10 interactions
            ...
            casual_count += sum(
                1
                for indicator in formality_indicators["casual"]
                if indicator in text_lower
            )
        ...
        emotional_expression = (
            "expressive" if emotional_state.intensity > 0.6 else "reserved"
        )
    ...
    def _learn_response_length_preferences(
        self, user_id: str, context: ConversationContext, feedback: Optional[float]
    ) -> Dict[str, Any]:
        """Learn response length preferences."""
        interactions = self.user_interactions[user_id]
        ...
        avg_length = (
            sum(message_lengths) / len(message_lengths) if message_lengths else 100
        )
    ...
    def _learn_topic_interests(
        ...
        top_interests = sorted(
            weighted_topics.items(), key=lambda x: x[1], reverse=True
        )[:10]
        ...
        if cached_data:
            return UserPreferences(
                ...
                communication_style=CommunicationStyle(
                    cached_data.get("communication_style", "friendly")
                ),
                preferred_response_length=ResponseLength(
                    cached_data.get("preferred_response_length", "moderate")
                ),
    ...
    def _analyze_preference_stability(
        self, learning_results: List[PreferenceLearningResult]
    ) -> Dict[str, Any]:
        ...
        styles = [
            result.learned_preferences.communication_style
            for result in learning_results
        ]
        style_changes = sum(
            1 for i in range(1, len(styles)) if styles[i] != styles[i - 1]
        )

        # Check response length stability
        lengths = [
            result.learned_preferences.preferred_response_length
            for result in learning_results
        ]
        length_changes = sum(
    ...
    def _identify_preference_drift(
        ...
        topic_overlap = len(first_topics.intersection(last_topics)) / max(
            1, len(first_topics.union(last_topics))
        )
    ...
    def _predict_response_length_needs(
        self, preferences: UserPreferences, context: ConversationContext
    ) -> Optional[Dict[str, Any]]:
        """Predict response length needs."""
    ...
    def _predict_topic_interest_alignment(
        ...
        aligned_topics = [
            topic for topic in message_topics if topic in preferences.topics_of_interest
        ]

        alignment_score = (
            len(aligned_topics) / len(message_topics) if message_topics else 0
        )

        return {
            "need_type": "topic_alignment",
            "predicted_need": (
                "high_interest" if alignment_score > 0.5 else "moderate_interest"
            ),
    ...
    def _predict_emotional_support_needs(
        self, preferences: UserPreferences, emotional_state: EmotionalState
    ) -> Optional[Dict[str, Any]]:
        """Predict emotional support needs."""
