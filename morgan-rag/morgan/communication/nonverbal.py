...
class NonVerbalCueDetector:
    ...
    def _detect_punctuation_emphasis(self, text: str) -> List[NonVerbalCue]:
        ...
        if exclamation_matches:
            intensity = (
                ...
                if len(max(exclamation_matches, key=len)) > 3
                else EmotionalIntensity.MODERATE
            )
            cues.append(
                NonVerbalCue(
        ...
        if question_matches:
            intensity = (
                ...
                if len(max(question_matches, key=len)) > 2
    ...
    def _detect_timing_patterns(
        self, timing_data: Dict[str, Any], context: ConversationContext
    ) -> List[NonVerbalCue]:
            ...
            if typing_duration > 60 and message_length < 50:
                cues.append(
                    NonVerbalCue(
                        ...
                        indicators=[
                            f"Typing: {typing_duration}s, Length: {message_length}"
                        ],
    ...
    def _calculate_overall_intensity(
        self, cues: List[NonVerbalCue]
    ) -> EmotionalIntensity:
        ...
        total_score = sum(
            intensity_scores[cue.intensity] * cue.confidence for cue in cues
        )
        ...
        for cue in cues:
            ...
            if cue.cue_type == NonVerbalCueType.PUNCTUATION_EMPHASIS:
                if "exclamation" in cue.description:
                    indicators["excitement"] = (
                        indicators.get("excitement", 0.0) + cue.confidence * 0.8
                    )
                elif "question" in cue.description:
                    indicators["confusion"] = (
                        indicators.get("confusion", 0.0) + cue.confidence * 0.7
                    )
                elif "ellipsis" in cue.description:
                    indicators["hesitation"] = (
                        indicators.get("hesitation", 0.0) + cue.confidence * 0.6
                    )

            elif cue.cue_type == NonVerbalCueType.CAPITALIZATION:
                indicators["intensity"] = (
                    indicators.get("intensity", 0.0) + cue.confidence * 0.9
                )
                if "mixed case" in cue.description:
                    indicators["sarcasm"] = (
                        indicators.get("sarcasm", 0.0) + cue.confidence * 0.7
                    )

            elif cue.cue_type == NonVerbalCueType.REPETITION:
                indicators["emphasis"] = (
                    indicators.get("emphasis", 0.0) + cue.confidence * 0.8
                )

            elif cue.cue_type == NonVerbalCueType.EMOJI_USAGE:
                indicators["emotional_expression"] = (
                    indicators.get("emotional_expression", 0.0) + cue.confidence * 0.9
                )

            elif cue.cue_type == NonVerbalCueType.TIMING_PATTERNS:
                if "rapid" in cue.description:
                    indicators["urgency"] = (
                        indicators.get("urgency", 0.0) + cue.confidence * 0.8
                    )
    ...
    def _calculate_communication_urgency(
        self, cues: List[NonVerbalCue], timing_data: Optional[Dict[str, Any]]
    ) -> float:
        ...
        for cue in cues:
            if (
                cue.cue_type == NonVerbalCueType.PUNCTUATION_EMPHASIS
                and "exclamation" in cue.description
            ):
                urgency_score += 0.3 * cue.confidence
            elif cue.cue_type == NonVerbalCueType.CAPITALIZATION:
                urgency_score += 0.4 * cue.confidence
            elif (
                cue.cue_type == NonVerbalCueType.TIMING_PATTERNS
                and "rapid" in cue.description
            ):
                urgency_score += 0.5 * cue.confidence
            elif (
                cue.cue_type == NonVerbalCueType.MESSAGE_LENGTH
                and "short" in cue.description
            ):
