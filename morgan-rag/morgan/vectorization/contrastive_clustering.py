...
class ContrastiveClusteringEngine:
    ...
    def apply_contrastive_bias(
        self, embedding: List[float], category: str, scale: str
    ) -> List[float]:
        """
    ...
    def generate_category_bias(self, category: str, embedding_dim: int) -> np.ndarray:
        """
        ...
        logger.debug(
            "Generated category bias: category=%s, dim=%d, " "seed=%d, norm=%.3f",
            category,
            embedding_dim,
            category_seed,
    ...
    def apply_batch_contrastive_bias(
        self, embeddings: List[List[float]], categories: List[str], scales: List[str]
    ) -> List[List[float]]:
        """
        ...
        if not (len(embeddings) == len(categories) == len(scales)):
            raise ValueError("embeddings, categories, and scales must have same length")

        if not embeddings:
            return []

        logger.debug(
            "Applying batch contrastive bias to %d embeddings", len(embeddings)
        )

        # Process each embedding
        biased_embeddings = []
        for embedding, category, scale in zip(embeddings, categories, scales):
            biased_embedding = self.apply_contrastive_bias(embedding, category, scale)
            biased_embeddings.append(biased_embedding)

        return biased_embeddings

    def compute_category_separation(
        self, embeddings: List[List[float]], categories: List[str]
    ) -> Dict[str, float]:
        """
        ...
        if len(embeddings) != len(categories):
            raise ValueError("embeddings and categories must have same length")

        if len(embeddings) < 2:
            return {
                "intra_category_distance": 0.0,
            ...
            if len(group_embeddings) > 1:
                ...
                for i in range(len(group_array)):
                    for j in range(i + 1, len(group_array)):
                        distance = np.linalg.norm(group_array[i] - group_array[j])
