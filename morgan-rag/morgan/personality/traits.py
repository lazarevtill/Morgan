...
class PersonalityProfile:
    ...
    user_id: str
    trait_scores: Dict[PersonalityTrait, TraitScore] = field(default_factory=dict)
    ...
    def _update_overall_confidence(self):
        ...
        total_confidence = sum(score.confidence for score in self.trait_scores.values())
...
class PersonalityTraitModeler:
    ...
    def analyze_personality(
        ...
        for trait in PersonalityTrait:
            trait_score = self._analyze_trait(trait, text_data, interactions, user_id)
            if trait_score:
        ...
        logger.info(
            "Personality analysis complete for user %s. " "Overall confidence: %.2f",
            user_id,
    ...
    def _extract_text_data(self, interactions: List[InteractionData]) -> List[str]:
    ...
    def _analyze_trait(
        ...
        low_score = self._calculate_pattern_score(text_data, indicators.get("low", []))

        # Calculate behavioral indicators
        behavioral_score = self._analyze_behavioral_indicators(trait, interactions)
        ...
        combined_score = (
            linguistic_weight * base_score + behavioral_weight * behavioral_score
        )
        ...
        confidence = min((len(text_data) / 10.0) * combined_score, 1.0)
        ...
        return TraitScore(
            trait=trait, score=final_score, confidence=confidence, evidence=evidence
        )
    ...
    def _analyze_behavioral_indicators(
        self, trait: PersonalityTrait, interactions: List[InteractionData]
    ) -> float:
        ...
        if trait == PersonalityTrait.EXTRAVERSION:
            ...
            avg_session_length = self._calculate_avg_session_length(interactions)
            ...
            emotional_volatility = self._calculate_emotional_volatility(interactions)
            ...
            positive_sentiment = self._calculate_positive_sentiment(interactions)
    ...
    def _calculate_feedback_ratio(self, interactions: List[InteractionData]) -> float:
    ...
    def _calculate_emotional_volatility(
        ...
        variance = sum(
            (intensity - mean_intensity) ** 2 for intensity in emotional_states
        ) / len(emotional_states)

        # Higher variance = higher volatility
        return min(variance * 2, 1.0)

    def _calculate_topic_diversity(self, interactions: List[InteractionData]) -> float:
    ...
    def _calculate_positive_sentiment(
        ...
        for interaction in interactions:
            if interaction.emotional_state:
                total_count += 1
                if (
                    interaction.emotional_state.primary_emotion.value
    ...
    def get_trait_description(self, trait: PersonalityTrait, level: TraitLevel) -> str:
