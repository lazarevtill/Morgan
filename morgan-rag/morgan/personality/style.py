...
from ..emotional.models import (
    CommunicationStyle, ConversationContext, EmotionalState
)
...
class CommunicationStyleAdapter:
    ...
    def adapt_style(
        ...
        logger.info(
            ...
            len(adaptations), user_id, overall_confidence
        )
    ...
    def _generate_base_style(
        ...
        for trait, trait_score in personality_profile.trait_scores.items():
            if trait in self.TRAIT_STYLE_MAPPINGS:
                ...
                style_mappings = self.TRAIT_STYLE_MAPPINGS[trait].get(
                    trait_level, {}
                )
    ...
    def _apply_contextual_adjustments(
        ...
        if emotional_state:
            adjusted_style = self._adjust_for_emotion(
                adjusted_style, emotional_state
            )
    ...
    def _adjust_for_emotion(
        ...
        if emotion in ['sadness', 'fear', 'anger']:
            ...
            if emotion in ['sadness', 'fear']:
                style.directness = self._decrease_intensity(
                    style.directness, intensity
                )
        
        elif emotion == 'joy':
            # Match enthusiasm for positive emotions
            style.enthusiasm = self._increase_intensity(
                style.enthusiasm, intensity
            )
            style.warmth = self._increase_intensity(style.warmth, intensity)
    ...
    def _generate_adaptations(
        ...
        for dimension in StyleDimension:
            ...
            if current_intensity != target_intensity:
                ...
                intensity_diff = abs(
                    current_intensity.value - target_intensity.value
                )
    ...
    def _generate_adaptation_reasoning(
        ...
        direction = "increase" if to_intensity.value > from_intensity.value else "decrease"
    ...
    def _calculate_overall_confidence(
        ...
        adaptation_confidence = sum(
            a.confidence for a in adaptations
        ) / len(adaptations)
        
        # Combine with weights
        overall_confidence = (
            0.6 * base_confidence + 0.4 * adaptation_confidence
        )
    ...
    def _generate_reasoning(
        ...
        if hasattr(context, 'context_type'):
            reasoning_parts.append(
                f"Adapted for {context.context_type} context"
            )
        
        # Adaptation count
        if adaptations:
            reasoning_parts.append(
                f"Applied {len(adaptations)} style adjustments"
            )
        
        return "; ".join(reasoning_parts) if reasoning_parts else "Default style applied"
