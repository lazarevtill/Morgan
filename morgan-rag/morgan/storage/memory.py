...
class MemoryStorage:
    ...
    def _load_data(self) -> None:
        ...
        try:
            ...
            if self.profiles_file.exists():
                with open(self.profiles_file, "r", encoding="utf-8") as f:
                    ...
                    for user_id, profile_data in profiles_data.items():
                        ...
                        self._profile_cache[user_id] = UserProfile(**profile_data)
            ...
            logger.info(
                "Loaded %d profiles and %d memories",
        ...
        try:
            ...
            for memory in self._memory_cache.values():
                if (
                    ...
                    and memory.importance_score >= min_importance
                ):

                    if query is None or query.lower() in memory.content.lower():
                ...
                with open(self.memories_file, "r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        if (
                            ...
                            and memory_data["importance_score"] >= min_importance
                            and memory_data["id"] not in self._memory_cache
                        ):

                            if (
                                query is None
                                or query.lower() in memory_data["content"].lower()
                            ):
                                memory_data["timestamp"] = datetime.fromisoformat(
                                    memory_data["timestamp"]
                                )
    ...
    def store_emotional_state(self, emotional_state: EmotionalState) -> bool:
        ...
        try:
            ...
            logger.debug("Stored emotional state for user: %s", emotional_state.user_id)
            return True
    ...
    def store_user_profile(self, profile: UserProfile) -> bool:
        ...
        try:
            ...
            logger.error("Error storing profile for user %s: %s", profile.user_id, e)
            return False
    ...
    def get_memory_stats(self, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if user_id:
                user_memories = [
                    m for m in self._memory_cache.values() if m.user_id == user_id
                ]
                stats["user_memories"] = len(user_memories)
                if user_memories:
                    stats["avg_importance"] = sum(
                        m.importance_score for m in user_memories
                    ) / len(user_memories)
                else:
