...
class EmotionalChatInterface:
    ...
    def _process_message(self, user_id: str, message: str):
        ...
        if response.milestone_celebration:
            ...
            self.display.display_milestone_celebration(
                response.milestone_celebration, celebration_msg
            )
    ...
    def _handle_goodbye(self, user_id: str):
        ...
        if session and session.message_count > 0:
            goodbye_text += (
                f"I enjoyed our {session.message_count} message conversation. "
            )
        ...
        goodbye_panel = Panel(
            goodbye_text, title="👋 Until Next Time!", border_style="blue"
        )
...
def start_emotional_chat(
    user_id: Optional[str] = None, morgan_assistant: Optional[MorganAssistant] = None
):
