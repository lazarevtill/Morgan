...
from ..emotional.models import (
    ...
    CompanionProfile, EmotionalState
)
...
class UserPreferenceManager:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        ...
        self.storage_path = Path(storage_path) if storage_path else Path("data/preferences")
        self.storage_path.mkdir(parents=True, exist_ok=True)
        
        self.preference_history: Dict[str, List[PreferenceChange]] = {}
        self.validation_rules = self._setup_validation_rules()
        
        logger.info(f"User preference manager initialized with storage: {self.storage_path}")
        ...
        try:
            ...
            if pref_file.exists():
                ...
                preferences = UserPreferences(
                    ...
                    communication_style=CommunicationStyle(data.get('communication_style', 'friendly')),
                    preferred_response_length=ResponseLength(data.get('preferred_response_length', 'detailed')),
    ...
    def update_user_preferences(
        ...
        try:
            ...
            if not validation_result.is_valid:
                logger.warning(f"Invalid preferences for user {user_id}: {validation_result.errors}")
            ...
            # Track changes
            if current_prefs:
                self._track_preference_changes(user_id, current_prefs, preferences, reason)
    ...
    def validate_preferences(self, preferences: UserPreferences) -> PreferenceValidationResult:
        ...
        if len(preferences.topics_of_interest) > 20:
            warnings.append("You have many topics of interest. Consider focusing on your top priorities.")
        
        # Check for duplicate topics
        unique_topics = set(topic.lower() for topic in preferences.topics_of_interest)
        if len(unique_topics) < len(preferences.topics_of_interest):
            warnings.append("Some topics of interest are duplicated.")
            suggestions.append("Remove duplicate topics to keep your interests organized.")
        
        # Validate communication style consistency
        if (preferences.communication_style == CommunicationStyle.FORMAL and 
            preferences.preferred_response_length == ResponseLength.BRIEF):
            suggestions.append("Formal communication often benefits from detailed responses.")
        
        # Validate learning goals
        if len(preferences.learning_goals) > 10:
            warnings.append("You have many learning goals. Consider prioritizing the most important ones.")
        
        # Check for empty or very short goals
        short_goals = [goal for goal in preferences.learning_goals if len(goal.strip()) < 10]
        if short_goals:
            suggestions.append("Consider adding more detail to your learning goals for better assistance.")
        ...
        if preferences.personal_context:
            ...
            for key in preferences.personal_context.keys():
                if any(sensitive in key.lower() for sensitive in sensitive_keys):
                    errors.append(f"Personal context should not contain sensitive information like '{key}'.")
        ...
        try:
            ...
            if 'topics_discussed' in interaction_data:
                ...
                for topic in new_topics:
                    if (topic not in current_prefs.topics_of_interest and 
                        len(current_prefs.topics_of_interest) < 15):
            ...
            if 'message_style_indicators' in interaction_data:
                ...
                if indicators.get('formal_language_ratio', 0) > 0.7:
                    if current_prefs.communication_style != CommunicationStyle.FORMAL:
                        ...
                        updated = True
                        logger.debug(f"Learned formal communication style for user {user_id}")
                
                # Detect technical language
                elif indicators.get('technical_terms_ratio', 0) > 0.5:
                    if current_prefs.communication_style != CommunicationStyle.TECHNICAL:
                        current_prefs.communication_style = CommunicationStyle.TECHNICAL
                        updated = True
                        logger.debug(f"Learned technical communication style for user {user_id}")
            ...
            if 'response_length_feedback' in interaction_data:
                ...
                if feedback == 'too_long' and current_prefs.preferred_response_length != ResponseLength.BRIEF:
                    current_prefs.preferred_response_length = ResponseLength.BRIEF
                    updated = True
                elif feedback == 'too_short' and current_prefs.preferred_response_length != ResponseLength.COMPREHENSIVE:
                    current_prefs.preferred_response_length = ResponseLength.COMPREHENSIVE
                    updated = True
            ...
            if 'emotional_preferences' in interaction_data:
                ...
                if 'empathy_preference' in emotional_prefs:
                    current_prefs.interaction_preferences['empathy_level'] = emotional_prefs['empathy_preference']
                    updated = True
                
                if 'formality_preference' in emotional_prefs:
                    current_prefs.interaction_preferences['formality_level'] = emotional_prefs['formality_preference']
                    updated = True
            ...
            logger.error(f"Failed to learn preferences from behavior for user {user_id}: {e}")
            return None
    ...
    def get_preference_suggestions(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            style_changes = [change for change in history if change.preference_type == 'communication_style']
            ...
            logger.error(f"Failed to generate preference suggestions for user {user_id}: {e}")
    ...
    def import_preferences(
        ...
        try:
            ...
            imported_prefs = UserPreferences(
                ...
                communication_style=CommunicationStyle(pref_data.get('communication_style', 'friendly')),
                preferred_response_length=ResponseLength(pref_data.get('preferred_response_length', 'detailed')),
            ...
            if merge_with_existing:
                ...
                if existing_prefs:
                    imported_prefs = self._merge_preferences(existing_prefs, imported_prefs)
            
            # Validate imported preferences
            validation_result = self.validate_preferences(imported_prefs)
            if not validation_result.is_valid:
                logger.error(f"Invalid imported preferences: {validation_result.errors}")
                return False
    ...
    def rollback_preferences(self, user_id: str, steps: int = 1) -> bool:
        ...
        try:
            ...
            if len(history) < steps:
                logger.warning(f"Not enough history to rollback {steps} steps for user {user_id}")
                return False
    ...
    def _merge_preferences(
        ...
        merged_topics = list(set(existing.topics_of_interest + imported.topics_of_interest))
    ...
    def _setup_validation_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'min_goal_length': 10,
            'forbidden_context_keys': ['password', 'ssn', 'credit_card', 'bank_account'],
