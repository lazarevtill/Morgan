    ...
    def _setup_routes(self):
        ...
        async def home(request: Request):
            ...
            return self.templates.TemplateResponse(
                "chat.html", 
                {"request": request, "title": "Morgan - Your AI Companion"}
            )
            ...
            try:
                ...
                return {
                    ...
                    "milestone_celebration": (
                        ...
                        } if response.milestone_celebration else None
                    ),
        ...
        async def update_preferences(preferences: PreferenceUpdate):
            ...
            try:
                profile = self.morgan.relationship_manager.profiles.get(preferences.user_id)
                if not profile:
                    # Create new profile if it doesn't exist
                    profile = self.morgan.emotional_processor.get_or_create_user_profile(preferences.user_id)
                
                # Update preferences
                if preferences.communication_style:
                    from ..emotional.models import CommunicationStyle
                    profile.communication_preferences.communication_style = CommunicationStyle(
                        preferences.communication_style
                    )
                
                if preferences.response_length:
                    from ..emotional.models import ResponseLength
                    profile.communication_preferences.preferred_response_length = ResponseLength(
                        preferences.response_length
                    )
                
                if preferences.topics_of_interest:
                    profile.communication_preferences.topics_of_interest = preferences.topics_of_interest
