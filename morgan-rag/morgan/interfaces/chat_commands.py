...
class ChatCommandHandler:
    ...
    def handle_preferences_command(self, user_id: str):
        ...
        if not user_profile:
            self.display.display_error(
                "No profile found. Preferences will be created after our first conversation."
            )
            return
        ...
        self.display.console.print(
            Panel("⚙️ Preference Management", border_style="blue")
        )
        ...
        current_prefs.add_row(
            "Communication Style", prefs.communication_style.value.title()
        )
        current_prefs.add_row(
            "Response Length", prefs.preferred_response_length.value.title()
        )
        ...
        while rating is None:
            try:
                ...
                rating_input = Prompt.ask(
                    "Rate our conversation (1-5 stars)", default="5"
                )
    ...
    def handle_stats_command(self, user_id: str):
        ...
        try:
            ...
            if isinstance(insights, dict) and "error" not in insights:
                stats_table.add_row(
                    "Relationship Age",
                    f"{insights.get('relationship_age_days', 0)} days",
                )
                stats_table.add_row(
                    "Total Conversations", str(insights.get("interaction_count", 0))
                )
                stats_table.add_row(
                    "Trust Level", f"{insights.get('trust_level', 0):.1%}"
                )
                stats_table.add_row(
                    "Engagement Score", f"{insights.get('engagement_score', 0):.1%}"
                )
            ...
            if isinstance(milestone_stats, dict) and "error" not in milestone_stats:
                stats_table.add_row(
                    "Total Milestones", str(milestone_stats.get("total_milestones", 0))
                )
            ...
            stats_panel = Panel(
                stats_table, title="📊 Your Statistics", border_style="blue"
            )
    ...
    def handle_topics_command(self, user_id: str):
        ...
        try:
            ...
            topics_panel = Panel(
                topics_table, title="💡 Suggested Topics", border_style="yellow"
            )
    ...
    def _update_preferences_interactive(self, user_profile: CompanionProfile):
        ...
        style_choice = Prompt.ask(
            "Choose communication style (1-5)",
            default=str(style_options.index(current_style) + 1),
        )
        try:
            new_style_index = int(style_choice) - 1
            if 0 <= new_style_index < len(style_options):
                user_profile.communication_preferences.communication_style = (
                    CommunicationStyle(style_options[new_style_index])
                )
                self.display.display_success(
                    f"Updated communication style to '{style_options[new_style_index]}'"
                )
        ...
        current_length = (
            user_profile.communication_preferences.preferred_response_length.value
        )
        ...
        length_choice = Prompt.ask(
            "Choose response length (1-3)",
            default=str(length_options.index(current_length) + 1),
        )
        try:
            ...
            if 0 <= new_length_index < len(length_options):
                user_profile.communication_preferences.preferred_response_length = (
                    ResponseLength(length_options[new_length_index])
                )
                self.display.display_success(
                    f"Updated response length to '{length_options[new_length_index]}'"
                )
        except (ValueError, IndexError):
            self.display.display_error("Invalid choice")
        ...
        if self.display.confirm("Would you like to add new topics of interest?"):
            new_topics_input = Prompt.ask(
                "Enter topics separated by commas", default=""
            )
            if new_topics_input.strip():
                ...
                for topic in new_topics:
                    if (
                        topic
                        and topic
                        not in user_profile.communication_preferences.topics_of_interest
                    ):
                        user_profile.communication_preferences.topics_of_interest.append(
                            topic
                        )
