...
def test_integration_with_existing_services():
    ...
    try:
        ...
        embeddings = embedding_service.generate_embeddings(
            texts=test_texts, model_name="jina-clip-v2", batch_size=2
        )
        ...
        processor = MultimodalContentProcessor(
            embedding_service=embedding_service, ocr_enabled=True
        )
        ...
        print(
            f"   Processed document with {len(doc.text_embeddings)} text embedding dims"
        )
    ...
    try:
        ...
        for query in search_queries:
            results = processor.search_multimodal_content(
                query=query, documents=documents, include_images=True, max_results=2
            )

            print(f"   Query: '{query}' -> {len(results)} results")
            for i, result in enumerate(results):
                print(
                    f"     Result {i+1}: score={result.relevance_score:.3f}, images={len(result.images)}"
                )
