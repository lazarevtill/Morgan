...
class TestEmotionalIntelligenceEngine:
    ...
    def test_relationship_milestone_detection(self, engine):
        ...
        first_conv_milestones = [
            m for m in milestones if m.milestone_type.value == "first_conversation"
        ]
    ...
    def test_empathetic_response_generation(self, engine):
        ...
        mock_llm.generate.return_value = Mock(
            content="I understand you're feeling happy! That's wonderful to hear."
        )
    ...
    def test_user_profile_creation_and_update(self, engine):
        ...
        assert (
            "machine learning" in profile.communication_preferences.topics_of_interest
        )
    ...
    def test_fallback_empathetic_response(self, engine):
        ...
        assert len(response.response_text) > 0
        assert (
            "support" in response.response_text.lower()
            or "difficult" in response.response_text.lower()
        )
    ...
    def test_communication_preference_learning(self, engine):
        ...
        long_message = (
            "This is a very detailed message where I explain my thoughts thoroughly and provide extensive context about what I'm thinking and feeling. I like to be comprehensive in my communication and provide lots of details about my situation and needs."
            * 2
        )
        ...
        emotional_state = EmotionalState(
            primary_emotion=EmotionType.NEUTRAL, intensity=0.5, confidence=0.7
        )

        interaction_data = InteractionData(
            conversation_context=context, emotional_state=emotional_state
        )
