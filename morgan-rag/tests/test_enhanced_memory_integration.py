...
def test_enhanced_memory_search_integration():
    ...
    try:
        ...
        for i, result in enumerate(companion_results[:3], 1):
            ...
            print(
                f"    Relationship Significance: {result.relationship_significance:.3f}"
            )
        ...
        relationship_memories = companion_search.get_relationship_memories(
            user_id="test_user", max_results=3
        )
...
def test_requirements_compliance():
    ...
    try:
        ...
        doc_results = [
            r
            for r in mixed_results.results
            if r.result_type in ["knowledge", "document"]
        ]
        memory_results = [
            r
            for r in mixed_results.results
        ...
        print(
            f"✅ Both types included: {len(doc_results) > 0 and len(memory_results) > 0}"
        )
        ...
        temporal_results = companion_search.search_with_emotional_context(
            query="Recent programming challenges", user_id="test_user", max_results=5
        )

        # Check if results are properly weighted by recency
        if temporal_results:
            recent_count = sum(
                1 for r in temporal_results if "recent" in r.personalization_factors
            )
            print(
                f"✅ Recent conversations prioritized: {recent_count}/{len(temporal_results)}"
            )
        ...
        for result in mixed_search.results[:3]:
            source_type = (
                "Memory"
                if "Memory" in result.source
                or result.result_type
                in ["memory", "enhanced_memory", "companion_memory"]
                else "Document"
            )
...
if __name__ == "__main__":
    ...
    print(
        "Testing implementation of task 8.2: Integrate memory search with companion features"
    )
    print()
    ...
    if integration_success and requirements_success:
        ...
        print(
            "✅ Enhanced memory integration with companion features is working correctly"
        )
        print("✅ All requirements 10.1-10.5 are properly implemented")
