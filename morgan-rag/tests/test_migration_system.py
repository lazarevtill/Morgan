...
class TestRollbackManager:
    ...
    def test_validate_backup_for_rollback_valid(self):
        ...
        try:
            with patch.object(self.rollback_manager.vector_db, 'collection_exists') as mock_exists:
...
class TestMigrationIntegration:
    ...
    def test_full_migration_workflow(self, mock_hierarchical_service, mock_vector_db):
        ...
        mock_embedding = Mock()
        mock_embedding.get_embedding.side_effect = lambda scale: [0.1, 0.2] if scale == "coarse" else [0.1, 0.2, 0.3, 0.4]
        mock_embedding.texts = {"coarse": "test", "medium": "test", "fine": "test"}
        mock_embedding.metadata = {}
        mock_hierarchical_emb_service.create_hierarchical_embeddings.return_value = mock_embedding
