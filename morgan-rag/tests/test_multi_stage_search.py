...
class TestMultiStageSearchEngine:
    ...
    def mock_dependencies(self):
        ...
        with patch('morgan.search.multi_stage_search.get_embedding_service') as mock_embedding, \
             patch('morgan.search.multi_stage_search.get_hierarchical_embedding_service') as mock_hierarchical, \
             patch('morgan.search.multi_stage_search.VectorDBClient') as mock_vector_db:
            
            # Setup embedding service mock
            mock_embedding_service = Mock()
            mock_embedding_service.encode.return_value = [0.1] * 384
            mock_embedding_service.encode_batch.return_value = [[0.1] * 384, [0.2] * 384, [0.3] * 384]
    ...
    def test_basic_search_execution(self, search_engine, mock_dependencies, sample_db_results):
    ...
    def test_multiple_strategy_search(self, search_engine, mock_dependencies, sample_db_results):
        """Test search with multiple strategies and fusion."""
        # Setup mocks
        mock_dependencies['vector_db'].search.return_value = sample_db_results
        mock_dependencies['vector_db'].search_with_filter.return_value = sample_db_results
        
        # Execute search with multiple strategies
        results = search_engine.search(
            query="Docker installation",
            max_results=5,
            strategies=[SearchStrategy.SEMANTIC, SearchStrategy.KEYWORD, SearchStrategy.CATEGORY]
        )
    ...
    def test_hierarchical_search_filtering(self, search_engine, mock_dependencies, sample_db_results):
    ...
    def test_search_result_structure(self, search_engine, mock_dependencies, sample_db_results):
    ...
    def test_keyword_extraction(self, search_engine):
        ...
        keywords = search_engine._extract_keywords("How to install Docker on Ubuntu server")
    ...
    def test_category_detection(self, search_engine):
        ...
        code_category = search_engine._detect_query_category("function definition in Python")
        assert code_category == "code"
        
        # Test documentation query
        doc_category = search_engine._detect_query_category("how to install Docker tutorial")
        assert doc_category == "documentation"
        
        # Test API query
        api_category = search_engine._detect_query_category("REST API endpoint authentication")
        assert api_category == "api"
        
        # Test troubleshooting query
        trouble_category = search_engine._detect_query_category("Docker error connection refused")
    ...
    def test_result_deduplication(self, search_engine, mock_dependencies):
        ...
        with patch.object(search_engine, '_calculate_similarity') as mock_sim:
            mock_sim.side_effect = [0.99, 0.1]  # First comparison is duplicate, second is not
    ...
    def test_rrf_fusion(self, search_engine, mock_dependencies):
        ...
        strategy2_results = [
            SearchResult("Result B", "source2.md", 0.85, "knowledge", {}),  # Same as strategy1[1]
        ...
        with patch.object(search_engine, '_get_result_signature') as mock_sig:
            ...
            fused_results = search_engine._fusion_results([strategy1_results, strategy2_results], 10)
    ...
    def test_performance_stats_tracking(self, search_engine, mock_dependencies, sample_db_results):
    ...
    def test_search_results_methods(self):
        ...
        results = [
            SearchResult("Content A", "source1.md", 0.9, "knowledge", {}, strategy="semantic"),
            SearchResult("Content B", "source2.md", 0.8, "knowledge", {}, strategy="keyword"),
            SearchResult("Content C", "source3.md", 0.7, "knowledge", {}, strategy="semantic"),
        ]
    ...
    def test_memory_search_formatting(self, search_engine, mock_dependencies):
        ...
        results = search_engine.search(
            query="Docker installation",
            strategies=[SearchStrategy.MEMORY]
        )
