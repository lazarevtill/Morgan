...
def test_git_hash_cache_with_metrics():
    ...
    try:
        ...
        (source_dir / "doc1.txt").write_text(
            "This is document 1 content for testing cache"
        )
        (source_dir / "doc2.txt").write_text(
            "This is document 2 content for testing cache"
        )
        (source_dir / "doc3.txt").write_text(
            "This is document 3 content for testing cache"
        )
        ...
        collection_data = {
            ...
            "metadata": {"total_docs": 3, "processed_at": "2025-01-01T12:00:00"},
        }
        ...
        print(f"   Cache valid: {cache_status.is_valid}")
        print(
            f"   Stored hash: {cache_status.stored_hash[:12] if cache_status.stored_hash else 'None'}..."
        )
        print(f"   Current hash: {cache_status.current_hash[:12]}...")
        ...
        print(
            f"   Total documents in cache: {cache_stats['collections']['total_documents']}"
        )
...
def test_knowledge_base_integration():
    ...
    try:
        ...
        test_file.write_text(
            "This is a test document for knowledge base integration testing."
        )
...
if __name__ == "__main__":
    ...
    print(
        f"   Cache Implementation: {'✅ PASSED' if cache_test_passed else '❌ FAILED'}"
    )
    print(
        f"   KnowledgeBase Integration: {'✅ PASSED' if integration_test_passed else '❌ FAILED'}"
    )
