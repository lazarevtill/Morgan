...
def test_raw_punctuation_does_not_raise():
    ...
    assert to_match_query('what about RISKDEV-14802 "quoted" AND?') != ""
...
async def test_current_facts_is_project_scoped(gate: MemoryGate):
    await gate.upsert_fact(
        TemporalFact(user_id="u", project="plata", subject="s", predicate="p", object="o")
    )
...
async def test_consolidation_does_not_hardcode_the_default_project(consolidator, gate):
    ...
    await gate.store(Memory(user_id="u", project="plata", content="harbor mirror note"))
    await consolidator.run(user_id="u", project="plata")
...
async def test_forget_is_project_scoped(tmp_path):
    path = str(tmp_path / "m.db")
    m = _module(path)
    await m.store(Memory(user_id="u", project="plata", content="harbor"))
    await m.store(Memory(user_id="u", project="personal", content="harbor"))
    await m.forget(user_id="u", project="plata")
...
def test_cross_repo_recall_after_restart(tmp_path):
    data = tmp_path / "brain"
    repo_a, repo_b = tmp_path / "plata", tmp_path / "personal"
