...
class ConversationContext(BaseModel):
    ...
    id: Optional[Any] = Field(
        default=None, description="Database UUID (when persisted)"
    )
...
class AudioChunk(BaseModel):
    ...
    timestamp: Optional[datetime] = Field(
        default_factory=datetime.now, description="Audio timestamp"
    )
...
class LLMRequest(BaseModel):
    ...
    max_tokens: Optional[int] = Field(
        default=None, gt=0, le=8192, description="Maximum tokens to generate"
    )
...
class TTSRequest(BaseModel):
    ...
    speed: Optional[float] = Field(
        default=None, ge=0.1, le=3.0, description="Speech speed"
    )
...
class STTResponse(BaseModel):
    ...
    confidence: Optional[float] = Field(
        default=None,
        description="Transcription confidence (0.0-1.0, where 1.0 is highest confidence)",
    )
    duration: Optional[float] = Field(
        default=None, description="Audio duration in seconds"
    )
    segments: Optional[List[Dict[str, Any]]] = Field(
        default=None, description="Detailed transcription segments"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Additional metadata"
    )
...
class VADRequest(BaseModel):
    ...
    threshold: Optional[float] = Field(
        default=None, ge=0.0, le=1.0, description="Detection threshold"
    )
    sample_rate: Optional[int] = Field(
        default=None, gt=0, description="Audio sample rate"
    )
...
class VADResponse(BaseModel):
    ...
    speech_segments: Optional[List[Dict[str, Any]]] = Field(
        default=None, description="Speech segments"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Additional metadata"
    )
...
class Intent(BaseModel):
    ...
    parameters: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Intent parameters"
    )
    entities: Optional[List[Dict[str, Any]]] = Field(
        default=None, description="Named entities"
    )
...
class Command(BaseModel):
    ...
    context: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Command context"
    )
...
class Response(BaseModel):
    ...
    actions: Optional[List[Command]] = Field(
        default=None, description="Actions to execute"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Additional metadata"
    )
    confidence: Optional[float] = Field(
        default=None, ge=0.0, le=1.0, description="Response confidence"
    )
