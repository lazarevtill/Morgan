...
def test_the_default_session_bucket_does_not_leak_between_projects() -> None:
    ...
    store.append(key, _msg("the plata registry credentials rotate on Fridays"), project="plata")
    store.append(key, _msg("buy milk"), project="personal")

    plata = [m.content for m in store.recent(key, project="plata")]
    personal = [m.content for m in store.recent(key, project="personal")]

    assert plata == ["the plata registry credentials rotate on Fridays"]
...
def test_an_explicit_session_id_is_still_project_scoped() -> None:
    ...
    store.append(key, _msg("from plata"), project="plata")
    store.append(key, _msg("from personal"), project="personal")

    assert [m.content for m in store.recent(key, project="plata")] == ["from plata"]
