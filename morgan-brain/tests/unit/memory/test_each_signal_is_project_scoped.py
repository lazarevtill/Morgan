...
def test_fts_search_does_not_cross_projects(conn: object) -> None:
    ...
    idx.add("a", "the Harbor mirror blocked the deploy", user_id="u", project="acme")
    idx.add("b", "the Harbor mirror blocked the deploy", user_id="u", project="personal")

    assert idx.search("harbor", user_id="u", top_k=5, project="acme") == ["a"]
...
def test_entity_search_does_not_cross_projects(conn: object) -> None:
    ...
    idx.add("a", ["harbor"], user_id="u", project="acme")
...
async def test_vector_search_does_not_cross_projects() -> None:
    ...
    acme = await idx.search(user_id="u", vector=vector, top_k=5, project="acme")
...
def test_fts_project_scoping_survives_a_user_collision(conn: object) -> None:
    ...
    idx = FtsIndex(conn)  # type: ignore[arg-type]
    idx.add("a", "harbor", user_id="u1", project="acme")
    idx.add("b", "harbor", user_id="u2", project="acme")

    assert idx.search("harbor", user_id="u1", top_k=5, project="acme") == ["a"]
    assert idx.search("harbor", user_id="u2", top_k=5, project="acme") == ["b"]
