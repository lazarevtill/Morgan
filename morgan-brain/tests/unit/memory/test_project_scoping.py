...
async def test_recall_defaults_to_the_query_project(tmp_path):
    ...
    m = _module(path)
    await m.store(Memory(user_id="u", project="plata", content="harbor mirror note"))
    await m.store(Memory(user_id="u", project="personal", content="harbor sailing note"))
    got = await m.recall(MemoryQuery(user_id="u", project="plata", text="harbor"))
...
async def test_recall_defaults_to_the_default_project_when_unspecified(tmp_path):
    ...
    await m.store(Memory(user_id="u", project="plata", content="scoped note about harbor"))
...
async def test_facts_are_project_scoped(tmp_path):
    ...
    await m.upsert_fact(
        TemporalFact(
            user_id="u", project="plata", subject="user", predicate="works_on", object="RISKDEV"
        )
    )
    await m.upsert_fact(
        TemporalFact(
    ...
    plata_facts = await m.current_facts(user_id="u", project="plata")
    assert [f.object for f in plata_facts] == ["RISKDEV"]
    all_facts = await m.current_facts(user_id="u", project=None)
    assert {f.object for f in all_facts} == {"RISKDEV", "a boat"}
...
def test_vector_scoping_happens_inside_the_knn_not_after(tmp_path):
    ...
    async def run() -> None:
        await idx.upsert(
            VectorRecord(id="plata-exact", user_id="u", project="plata", vector=[1, 0, 0, 0])
        )
        await idx.upsert(
            VectorRecord(id="personal-exact", user_id="u", project="personal", vector=[1, 0, 0, 0])
        )
        await idx.upsert(
            VectorRecord(id="plata-near", user_id="u", project="plata", vector=[0.9, 0.1, 0, 0])
        )
        await idx.upsert(
            VectorRecord(
        ...
        hits = await idx.search(user_id="u", vector=[1, 0, 0, 0], top_k=2, project="plata")
        assert [h.id for h in hits] == ["plata-exact", "plata-near"]
