"""E2E test: /api/chat (blocking) and /api/chat/stream (SSE) via httpx AsyncClient.

Tests:
- POST /api/chat → 200 + {response, model_used, turn_id}.
- POST /api/chat/stream → SSE chunks with data: {...} lines ending with data: [DONE].
- Missing API key → 401 when key is configured.
- GET /health → 200, no auth required.
- /api/chat requires auth when key is enforced.
"""

from __future__ import annotations

from datetime import datetime

import pytest
from fastapi import Depends, FastAPI
from fastapi.responses import StreamingResponse
from httpx import ASGITransport, AsyncClient

from morgan_brain.apps.brain_api.app import ChatRequest, ChatResponse
from morgan_brain.apps.brain_api.auth import require_api_key
from morgan_brain.apps.brain_api.routes import build_router
from morgan_brain.composition import _assemble
from morgan_brain.config import Settings
from morgan_brain.learning.history import SessionHistoryStore
from morgan_brain.modules.memory.indexing.embedder import FakeEmbedder
from morgan_brain.providers.adapters.fake import FakeChatClient
from morgan_brain.providers.capability import CapabilityRegistry
from morgan_brain.providers.router import Binding, RoleRouter
from morgan_brain import __version__

import json as _json
from typing import AsyncIterator

CLOCK = lambda: datetime(2026, 1, 1)  # noqa: E731
API_KEY = "e2e-chat-key"
AUTH_HEADERS = {"Authorization": f"Bearer {API_KEY}"}


def _build_app(reply: str = "hello from fake") -> FastAPI:
    """Build a minimal create_app()-equivalent using fakes (no live services)."""
    settings = Settings(api_key=API_KEY, llm_model="test-model", llm_fast_model="test-model")
    fake_client = FakeChatClient(reply=reply)
    reg = CapabilityRegistry.from_seed(
        {
            "fake/test-model": {
                "supports_tools": True,
                "json_mode": "json_schema",
                "context_window": 32768,
            }
        }
    )
    router = RoleRouter(
        reg=reg,
        bindings={"strong": [Binding("fake", "test-model", fake_client)]},
    )
    history_store = SessionHistoryStore()
    orch, _, signal_store, recorder, executor, skills, learner = _assemble(
        embedder=FakeEmbedder(dim=16),
        router=router,
        settings=settings,
        clock=CLOCK,
        temporal_path=":memory:",
        history_store=history_store,
    )

    app = FastAPI(title="morgan-brain", version=__version__)
    _auth = Depends(require_api_key(settings))

    @app.get("/health")
    async def health() -> dict[str, str]:
        return {"status": "ok", "version": __version__, "event_bus": settings.event_bus}

    @app.post("/api/chat", response_model=ChatResponse, dependencies=[_auth])
    async def chat(req: ChatRequest) -> ChatResponse:
        uid = req.user_id or settings.owner_user_id
        hist = history_store.recent(req.session_id or "default")
        result, turn_id = await orch.handle_turn_with_id(
            user_id=uid,
            project=req.project,
            text=req.message,
            session_id=req.session_id,
            history=hist,
        )
        return ChatResponse(response=result.text, model_used=result.model_used, turn_id=turn_id)

    @app.post("/api/chat/stream", dependencies=[_auth])
    async def chat_stream(req: ChatRequest) -> StreamingResponse:
        uid = req.user_id or settings.owner_user_id

        async def _event_stream() -> AsyncIterator[str]:
            hist = history_store.recent(req.session_id or "default")
            async for delta in orch.stream_turn(
                user_id=uid,
                project=req.project,
                text=req.message,
                session_id=req.session_id,
                history=hist,
            ):
                payload = _json.dumps({"delta": delta})
                yield f"data: {payload}\n\n"
            yield "data: [DONE]\n\n"

        return StreamingResponse(_event_stream(), media_type="text/event-stream")

    app.include_router(
        build_router(
            orchestrator=orch,
            signal_recorder=recorder,
            executor=executor,
            skills=skills,
            learner=learner,
            settings=settings,
        )
    )

    return app


# ---------------------------------------------------------------------------
# Tests
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_chat_returns_200_with_response_model_used_turn_id() -> None:
    """/api/chat returns 200 + {response, model_used, turn_id}."""
    app = _build_app("test answer")
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        resp = await c.post(
            "/api/chat",
            json={"message": "What is 2+2?", "project": "default", "session_id": "s1"},
            headers=AUTH_HEADERS,
        )

    assert resp.status_code == 200
    body = resp.json()
    assert body["response"] == "test answer"
    assert isinstance(body["model_used"], str) and len(body["model_used"]) > 0
    assert isinstance(body["turn_id"], str) and len(body["turn_id"]) > 0


@pytest.mark.asyncio
async def test_chat_stream_yields_sse_lines_ending_with_done() -> None:
    """/api/chat/stream returns SSE lines ending with data: [DONE]."""
    app = _build_app("streamed reply")
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        async with c.stream(
            "POST",
            "/api/chat/stream",
            json={"message": "stream me", "project": "default", "session_id": "s1"},
            headers=AUTH_HEADERS,
        ) as resp:
            assert resp.status_code == 200
            assert "text/event-stream" in resp.headers.get("content-type", "")
            raw = await resp.aread()

    text = raw.decode()
    lines = [ln for ln in text.splitlines() if ln.startswith("data:")]
    assert len(lines) >= 1, f"No data: lines in SSE response: {text!r}"

    # Last line must be the [DONE] sentinel.
    last_line = lines[-1]
    assert last_line == "data: [DONE]", (
        f"Expected last SSE line to be 'data: [DONE]', got {last_line!r}"
    )

    # Non-terminal lines must be parseable JSON dicts with a 'delta' key.
    for line in lines[:-1]:
        payload_str = line[len("data:") :].strip()
        payload = _json.loads(payload_str)
        assert "delta" in payload, f"SSE data line missing 'delta' key: {payload}"


@pytest.mark.asyncio
async def test_chat_missing_auth_returns_401() -> None:
    """/api/chat without Authorization header → 401 when key is enforced."""
    app = _build_app()
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        resp = await c.post(
            "/api/chat",
            json={"message": "hello", "project": "default"},
            # No auth header
        )
    assert resp.status_code == 401


@pytest.mark.asyncio
async def test_chat_wrong_key_returns_401() -> None:
    """/api/chat with wrong key → 401."""
    app = _build_app()
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        resp = await c.post(
            "/api/chat",
            json={"message": "hello", "project": "default"},
            headers={"Authorization": "Bearer wrong-key"},
        )
    assert resp.status_code == 401


@pytest.mark.asyncio
async def test_health_returns_ok_without_auth() -> None:
    """GET /health → 200 {status: ok} without any auth header."""
    app = _build_app()
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        resp = await c.get("/health")
    assert resp.status_code == 200
    body = resp.json()
    assert body["status"] == "ok"
    assert "version" in body


@pytest.mark.asyncio
async def test_chat_returns_unique_turn_ids_per_request() -> None:
    """Each /api/chat call must return a distinct turn_id."""
    app = _build_app()
    turn_ids = []
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        for _ in range(3):
            resp = await c.post(
                "/api/chat",
                json={"message": "hi", "project": "default"},
                headers=AUTH_HEADERS,
            )
            turn_ids.append(resp.json()["turn_id"])

    assert len(set(turn_ids)) == 3, f"Expected 3 unique turn_ids, got: {turn_ids}"
