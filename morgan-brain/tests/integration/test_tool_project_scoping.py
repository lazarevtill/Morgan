...
layers carried a project, so an assistant answering a question in the `plata` repo searched
`default` instead: it missed every `plata` memory *and* pulled `default`-bucket content into a
`plata` conversation. Both directions of the "every read is project-scoped" invariant, on the
...
async def test_the_tool_searches_the_turns_project_not_default(
    ...
    await _seed(gate, project="plata", content="the RISKDEV credentials rotate on Fridays")
    await _seed(gate, project="default", content="DEFAULT-BUCKET credentials note")

    result = await executor.execute(
        "memory_search", user_id="u", project="plata", query="credentials"
    )

    assert result.ok, result.error
    assert result.output == ["the RISKDEV credentials rotate on Fridays"], result.output
...
async def test_the_model_cannot_choose_the_project(
    ...
    result = await executor.execute("memory_search", user_id="u", project="plata", **stripped)
