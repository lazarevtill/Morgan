...
def test_health():
    ...
    if response.status_code == 200:
        ...
        print(
            f"✓ Services: LLM={data['services']['llm']}, TTS={data['services']['tts']}, STT={data['services']['stt']}"
        )
        return True
    else:
...
def test_tts_direct():
    ...
    payload = {"text": "Hello, this is a test", "voice": "af_heart"}
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            print(
                f"  - First 4 bytes (ascii): {test_bytes[:4].decode('ascii', errors='ignore')}"
            )
...
def test_text_to_audio():
    ...
    payload = {"text": "What is the weather today?", "user_id": "test_user"}
...
def test_audio_metadata():
    ...
    tts_response = requests.post(
        f"{TTS_URL}/generate", json={"text": "Test", "voice": "af_heart"}
    )
