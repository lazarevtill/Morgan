...
def _no_env_files(
    ...
    what a test must never see: a result that changes with the contents of the developer's
    home directory, or of the checkout the suite runs in, depends on install state, not on
    the code. So an in-process test reads no file, except one marked ``reads_env_files``,
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line(
        "markers",
        "reads_env_files: settings_for reads the real .env files each surface names "
        "(the root conftest's guard stands aside)",
    )
