...
sys.path.insert(
    0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../../morgan-rag"))
)
...
async def test_emotional_analysis_flow():
    ...
    emotional_states, metrics = (
        await integration.batch_optimizer.optimize_emotional_processing_batch(
            texts=[user_message],
...
async def test_embedding_flow():
    ...
    embeddings, metrics = (
        await integration.batch_optimizer.optimize_jina_embedding_batch(
            texts=[user_message],
...
async def test_reranking_flow():
    ...
    mock_results = [
        SearchResult(
            content="Deep learning architecture for AI",
            score=0.9,
            metadata={},
            source="mock",
        ),
        SearchResult(
            content="Cooking recipes for dinner", score=0.1, metadata={}, source="mock"
        ),
    ]
