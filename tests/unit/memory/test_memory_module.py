...
async def test_store_then_recall_finds_memory():
    ...
    hits = (await m.recall(MemoryQuery(user_id="u1", text="hiking Berlin", top_k=5))).memories
...
async def test_recall_is_user_scoped():
    ...
    hits = (await m.recall(MemoryQuery(user_id="u2", text="secret", top_k=5))).memories
    assert hits == []
...
async def test_current_facts_are_surfaced_by_recall():
    ...
    hits = (await m.recall(MemoryQuery(user_id="u1", text="where do I live", top_k=5))).memories
...
async def test_entity_overlap_boosts_recall():
    ...
    hits = (await m.recall(MemoryQuery(user_id="u1", text="Alice", top_k=2))).memories
...
async def test_recall_facts_are_user_scoped():
    ...
    hits = (await m.recall(MemoryQuery(user_id="u2", text="anything", top_k=5))).memories
    assert hits == []
