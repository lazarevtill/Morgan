...
def test_recent_empty_session_returns_empty_list(store: SessionHistoryStore) -> None:
    assert store.recent("s1", project="personal") == []


def test_append_and_recent_one_message(store: SessionHistoryStore) -> None:
    msg = _user_msg("hello")
    store.append("s1", msg)
    result = store.recent("s1", project="personal")
    assert len(result) == 1
...
def test_recent_limit_truncates_oldest(store: SessionHistoryStore) -> None:
    ...
    result = store.recent("s1", project="personal", limit=3)
...
def test_sessions_are_isolated(store: SessionHistoryStore) -> None:
    ...
    assert store.recent("s1", project="personal")[0].content == "session one"
    assert store.recent("s2", project="personal")[0].content == "session two"
    assert len(store.recent("s1", project="personal")) == 1
    assert len(store.recent("s2", project="personal")) == 1
